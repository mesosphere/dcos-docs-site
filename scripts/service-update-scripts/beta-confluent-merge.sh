...
for i in $( ls ./services/beta-confluent-kafka/*/index.md );
do
...
mkdir service-docs/beta-confluent-kafka/$directory
mkdir service-docs/beta-confluent-kafka/$directory/img
echo "New directories created: service-docs/beta-confluent-kafka/$directory and service-docs/beta-confluent-kafka/$directory/img"
...
cp -r docs/* service-docs/beta-confluent-kafka/$directory

done <scripts/merge-lists/beta-confluent-merge-list.txt

git rm -rf docs/

# Add version information to latest index file

sed -i '' -e "2s/.*/post_title: Beta Confluent $directory/g" ./services/beta-confluent-kafka/$directory/index.md
