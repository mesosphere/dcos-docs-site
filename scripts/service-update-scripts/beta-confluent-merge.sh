...
#
echo "------------------------------"
echo " Merging Beta Confluent Kafka"
echo "------------------------------"

# Update sort order of index files

for i in $( ls ./pages/services/beta-confluent-kafka/*/index.md );
do
...
mkdir ./pages/services/beta-confluent-kafka/$directory
mkdir ./pages/services/beta-confluent-kafka/$directory/img
echo "New directories created: /pages/services/beta-confluent-kafka/$directory and /pages/services/beta-confluent-kafka/$directory/img"
...
do
  ...
  if [ ${p: -3} == ".md" ]; then
    ...
    awk -v directory="$directory" '{gsub(/\(img/,"(/services/beta-confluent-kafka/"directory"/img");}{print;}' $p > tmp && mv tmp $p
...
done <scripts/service-update-scripts/service-update-scripts/merge-lists/beta-confluent-kafka-merge-list.txt

git rm -rf frameworks

# Add version information to latest index file

sed -i '' -e "2s/.*/navigationTitle: Beta Confluent Kafka $directory/g" ./pages/services/beta-confluent-kafka/$directory/index.md
sed -i '' -e "2s/.*/title: Beta Confluent Kafka $directory/g" ./pages/services/beta-confluent-kafka/$directory/index.md
