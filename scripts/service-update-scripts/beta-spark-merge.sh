...
#
echo "------------------------------"
echo " Merging Beta spark-build"
echo "------------------------------"

# Update sort order of index files

for i in $( ls ./pages/services/beta-spark/*/index.md );
do
...
mkdir ./pages/services/beta-spark/$directory
mkdir ./pages/services/beta-spark/$directory/img
echo "New directories created: /pages/services/beta-spark/$directory and /pages/services/beta-spark/$directory/img"
...
do
  ...
  if [ ${p: -3} == ".md" ]; then
    ...
    awk -v directory="$directory" '{gsub(/\(img/,"(/services/beta-spark/"directory"/img");}{print;}' $p > tmp && mv tmp $p
...
cp -r docs/* ./pages/services/beta-spark/$directory

done <scripts/service-update-scripts/merge-lists/beta-spark-build-merge-list.txt
...
sed -i '' -e "2s/.*/navigationTitle: Beta Spark $directory/g" ./pages/services/beta-spark/$directory/index.md
sed -i '' -e "2s/.*/title: Beta Spark $directory/g" ./pages/services/beta-spark/$directory/index.md

echo "------------------------------"
echo " Beta spark-build merge complete"
echo "------------------------------"
