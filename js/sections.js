...
const contentHeadingsArray = [...contentHeadings].filter(
  (heading) => !heading.dataset.hide
);
...
function scrollTop() {
  if (tableOfContents) {
    if (
      windowOffset <
      articleContainer.getBoundingClientRect().top + headerOffset
    ) {
    ...
    contentHeadingsArray.forEach((el) => {
      ...
      if (elTopOffset < 1) {
        ...
        if (activeLink) {
          ...
          if (curActiveLink) {
            if (
              curActiveLink.getAttribute("href") !== `#${el.getAttribute("id")}`
            ) {
