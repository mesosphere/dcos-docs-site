...
function popTip(e) {
  ...
  if (window.innerWidth > 500) {
    if (measureLeft(e.target) > window.innerWidth - 300) {
      ...
      tip.style.right =
        window.innerWidth -
        (measureLeft(e.target) + e.target.offsetWidth) +
        "px";
      nub.style.right = tip.offsetWidth / 2 - 10 + "px";
    } else {
      tip.style.left =
        measureLeft(e.target) +
        (e.target.offsetWidth / 2 - tip.offsetWidth / 2) +
        "px";
      nub.style.left = tip.offsetWidth / 2 - 10 + "px";
    }
  } else {
    nub.style.left =
      measureLeft(e.target) - 5 + (e.target.offsetWidth / 2 - 10) + "px";
    tip.style.left = 0;
  }
