...
1.  Verify that you have a valid IP detect﻿⁠⁠⁠⁠script, functioning DNS resolvers to bind the DC/OS services to, and that all nodes are synchronized with NTP.
    ...
    You must have a valid [ip-detect](/1.8/administration/installing/custom/advanced/) script. You can manually run `ip-detect` on all the nodes in your cluster or check `/opt/mesosphere/bin/detect_ip` on an existing installation to ensure that it returns a valid IP address. A valid IP address does not have:
    
    - extra lines
    - white space
    - special or hidden characters 
    
    It is recommended that you use the `ip-detect` [examples](/1.8/administration/installing/custom/advanced/).
...
*   SSH to your master node and enter this command to view the logs from boot time:
 ...
 [1]: /1.8/administration/installing/custom/configuration-parameters/#scrollNav-5
 [2]: https://open.mesosphere.com/reference/mesos-master/
 [3]: /1.8/administration/installing/custom/configuration-parameters/#scrollNav-7
 [4]: /1.8/overview/architecture/#boot
 [5]: /1.8/administration/installing/custom/configuration-parameters
