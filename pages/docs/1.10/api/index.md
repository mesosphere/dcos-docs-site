...
For details on how to obtain and use an authentication token, see [Authentication HTTP API Endpoint](/docs/1.10/security/ent/iam-api/).
...
- **Course-grained permissions** are [enforced by Admin Router](/docs/1.10/security/ent/perms-reference/#admin-router) at the route level.
- **Fine-grained permissions** are enforced by individual backend component services.

[Permission management](/docs/1.10/security/ent/permissions/) can be performed by users with the [Superuser permission](/docs/1.10/security/ent/permissions/superuser-perm/) using the [Identity and Access Management API](/docs/1.10/security/ent/iam-api/). Users with the Superuser permission also have implicit permission to access all routes.
