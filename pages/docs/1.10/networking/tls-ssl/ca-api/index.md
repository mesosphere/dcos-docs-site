...
To get an authentication token, pass the user name and password of a user with the necessary permissions in the body of a request to the `/auth/login` endpoint of the [Identity and Access Management Service API](/1.10/security/iam-api/). It returns an authentication token as shown below.
...
[api-explorer api='/1.10/api/certificate-authority.yaml']
