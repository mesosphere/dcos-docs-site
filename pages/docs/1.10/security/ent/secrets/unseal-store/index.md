...
- [After being manually sealed.](/docs/1.10/security/ent/secrets/seal-store/)
- After a power outage.

A sealed Secret Store cannot be accessed from the GUI. Secret values cannot be retrieved using the [Secrets API](/docs/1.10/security/ent/secrets/secrets-api/). Services that depend on values provisioned to them via environment variables may fail to deploy. 
...
1. From the new tab, [SSH into your master](/docs/1.10/administering-clusters/sshcluster/) and launch the ZooKeeper command line interface as follows.

   ```bash
   /opt/mesosphere/packages/exhibitor--*/usr/zookeeper/bin/zkCli.sh
   ```

1. Execute the following ZooKeeper command to gain additional privileges, specifying the user name and password of the ZooKeeper superuser. By default, this is set to `super:secret` but we recommend [changing the default](/docs/1.10/installing/ent/custom/configuration/configuration-parameters/#zk-superuser).
...
1. Use secure copy to transfer the new file to your master, as shown below. Before executing the command, replace `<master-IP>` with the IP address of the master.
   ...
   **Tip:** If you used GPG to generate the custom GPG keypair as described in [Reinitializing the Secret Store with a custom GPG keypair](/docs/1.10/security/ent/secrets/custom-key/) and you have multiple masters, use the IP address of the master that you used to generate the keypair.

1. [SSH into your master](/docs/1.10/administering-clusters/sshcluster/)
...
1. If your GPG private key is not already loaded into GPG, go ahead and load it now.
   ...
   **Tip:** If you recently completed the [Reinitializing the Secret Store with your own GPG key](/docs/1.10/security/ent/secrets/custom-key/) procedure, your private key should already be loaded in GPG. 
