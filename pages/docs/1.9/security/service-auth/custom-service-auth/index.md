...
For more information, see the [permissions reference](/docs/1.9/security/perms-reference/).

## Assign the Permissions
Using the [permissions reference](/docs/1.9/security/perms-reference/) and the log output, assign permissions to your service.
...
Generate a [service login token](/docs/1.9/security/service-auth/)), where the service account (`<service-account-id>`) and private key (`<private-key>.pem`) are specified.
...
After the service has successfully logged in, an [authentication token](/docs/1.9/security/service-auth/) is created. The authentication token should used in subsequent requests to DC/OS endpoints. You can reference the authentication token as a shell variable, for example:
