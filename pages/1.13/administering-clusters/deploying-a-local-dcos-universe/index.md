...
You can install and run DC/OS services on a datacenter without internet access with a local [Catalog](https://github.com/mesosphere/universe). You can deploy a local Catalog that includes all Certified packages (basic), or a local Catalog that includes selected packages (advanced).
...
# <a name="certified"></a>Certified Catalog packages
...
1.  Use [secure copy](https://linux.die.net/man/1/scp) (scp) to transfer the Catalog and registry files to a master node, replacing `<master-IP>` with the public IP address of a master before issuing the following commands. (You can find the public IP address of a master in the top left corner of the DC/OS UI.)
...
1.  Load the Catalog into the local Docker instance. This may take some time to complete.
...
If you only have one master, skip to [Linking local Catalog to master](#linking) below. If you have multiple masters, continue with the following procedure.
...
1.  Use [secure copy](https://linux.die.net/man/1/scp) to transfer the Catalog and registry files to one of the other masters. Replace `<master-IP>` with the IP address of the other master.
...
variable. To minimize the container size and download time, you can select only what you need. If you do not use the `DCOS_PACKAGE_INCLUDE` variable, all Certified Catalog packages are
...
1.  Perform all of the steps as described in [Certified Catalog packages](#certified).
