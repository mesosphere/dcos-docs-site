...
- `persistent.profileName`: (not seen above) The storage [volume profile](/services/storage/1.0.0/terminology-and-concepts/#volume-profile). Only volumes with the specified profile are used to launch an application. It this option is not given, any volume (with or without a profile) will be used for launching.
