...
|----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| [agent_list](#agent-list)                                              | A YAML nested list (`-`) of IPv4 addresses to your [private agent](/mesosphere/dcos/1.13/overview/concepts/#private-agent-node) host names. |
...
| [aws_template_storage_bucket](#aws-template-storage-bucket)                | The name of an S3 bucket to contain [customized advanced AWS templates](/mesosphere/dcos/1.13/installing/evaluation/community-supported-methods/aws/advanced/#create-your-templates). |
...
For more information, see the [examples](/mesosphere/dcos/1.13/installing/ent/custom/configuration/examples/#docker-credentials).
...
*  `dcos_overlay_enable: 'true'` Enable the DC/OS virtual network. This is the default value. After the virtual network is enabled, you can also specify the following parameters:
    ...
    *  `dcos_overlay_network` This group of parameters defines a virtual network for DC/OS.  The default configuration of DC/OS provides a virtual network named `dcos` with this YAML configuration:
        ...
        *  `overlays`
            *  `name` The canonical name (see [limitations](/mesosphere/dcos/1.13/networking/virtual-networks/) for constraints on naming virtual networks).
 ...
 For more information, see the [example](/mesosphere/dcos/1.13/installing/ent/custom/configuration/examples/#overlay) and [documentation](/mesosphere/dcos/1.13/networking/virtual-networks/).
...
*  `enable_docker_gc: 'true'` Run the docker-gc scripts once every hour. This is the default value for [cloud](/mesosphere/dcos/1.13/installing/ent/cloud/) template installations.
*  `enable_docker_gc: 'false'` Do not run the docker-gc scripts once every hour. This is the default value for [custom](/mesosphere/dcos/1.13/installing/ent/custom/) installations.
...
-  If you do not have internal hostnames to resolve, you can set this to a public nameserver like Google or AWS. For example, you can specify the [Google Public DNS IP addresses (IPv4)](https://developers.google.com/speed/public-dns/docs/using):
    ...
    - 8.8.8.8
    ```
-  If you do not have a DNS infrastructure and do not have access to internet DNS servers, you can specify `resolvers: []`. By specifying this setting, all requests to non-`.mesos` will return an error. For more information, see the Mesos-DNS [documentation](/mesosphere/dcos/1.13/networking/mesos-dns/).
...
*  `use_proxy: 'true'` Configure DC/OS [components](/mesosphere/dcos/1.13/overview/architecture/components/) to use a custom proxy. If you specify `use_proxy: 'true'`, you can also specify these parameters:
    ...
    **Note:** Wildcard characters (`*`) are not supported.</p>
    ...
    For more information, see the [examples](/mesosphere/dcos/1.13/installing/ent/custom/configuration/examples/#http-proxy).
