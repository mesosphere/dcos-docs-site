...
- A node that meets the bootstrap node [system requirements](/1.13/installing/ent/custom/system-requirements/).
...
For parameters descriptions and configuration examples, see the [documentation](/1.13/installing/ent/custom/configuration/configuration-parameters/).
...
- On the **Select Template** page, upload the [Zen](/1.13/installing/evaluation/aws/) template from your workstation and click **Next**.
...
2.  On the **Specify Details** page, specify these values and and click **Next**.
    ...
    ![AWS UI](/1.13/img/aws-advanced-1.png)

     Figure 1. AWS Advanced web interface

    *  **Stack name** Specify the cluster name.
    *  **CustomAMI** Optional: Specify the AMI ID. For more information, see [Installing using a Custom AMI](/1.13/installing/production/system-requirements/custom-ami/).
...
*  The cluster stack spins up over a period of 15 to 20 minutes. You will have a stack created for each of these, where `<stack-name>` is the value you specified for **Stack name** and `<stack-id>` is an auto-generated ID.
   ...
   ![AWS UI](/1.13/img/aws-advanced-2.png)
...
1.  Click on the **Outputs** tab and copy/paste the Mesos Master hostname into your browser to open the DC/OS web interface. The interface runs on the standard HTTP port 80, so you do not need to specify a port number after the hostname.
    ...
    **Note:** You might need to resize your window to see this tab. You can find your DC/OS hostname any time from the <a href="https://console.aws.amazon.com/cloudformation/home" target="_blank">AWS CloudFormation Management</a> page.
    ...
    ![Monitor stack creation](/1.13/img/dcos-aws-step3a.png)
...
1.  Click **Log In To DC/OS**.
    ...
    ![UI installer success](/1.13/img/gui-installer-success-ee.gif)
...
1.  Enter the username and password of the superuser account.
    ...
    **Note:** The default username is `bootstrapuser` and default password is `deleteme`.
    ...
    ![alt text](/1.13/img/ui-installer-auth2.png)

    Figure 5. Sign-in screen

    You are done!

    ![UI dashboard](/1.13/img/dashboard-ee.png)
...
You can add more agent nodes by creating a new stack. Use the [advanced-priv-agent.json](/1.13/installing/evaluation/aws/) or [advanced-pub-agent.json](/1.13/installing/evaluation/aws/) templates. These templates create agents which are then attached to the `PrivateAgentStack` or `PublicAgentStack` as a part of an AutoScalingGroup.
...
For all of the advanced configuration options, see the template reference [documentation](/1.13/installing/evaluation/aws/).
...
# Template reference
For the complete advanced configuration options, see the template reference [documentation](/1.13/installing/evaluation/aws/).
