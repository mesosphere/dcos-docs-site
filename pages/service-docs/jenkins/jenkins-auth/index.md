...
Whether you can or must provision Jenkins with a service account varies by [security mode](/docs/1.9/overview/security/security-modes/).
...
**Prerequisite:** You must have the [DC/OS CLI installed](/docs/1.9/cli/install/) and the [Enterprise DC/OS CLI 0.4.14 or later installed](/docs/1.9/cli/enterprise-cli/#ent-cli-install).
...
**Prerequisite:** You must have the [DC/OS CLI installed](/docs/1.9/cli/install/), the [Enterprise DC/OS CLI 0.4.14 or later installed](/docs/1.9/cli/enterprise-cli/#ent-cli-install), and be logged in as a superuser via `dcos auth login`.
...
1. If you have [jq 1.5 or later](https://stedolan.github.io/jq/download) installed, you can also use the following command to retrieve the secret and ensure that it contains the correct service account ID and private key.
   ...
   **Important:** While reviewing the secret, ensure that the `login_endpoint` URL uses HTTPS if you are in `strict` mode and HTTP if you are in `permissive` mode. If the URL begins with `https` and you are in `permissive` mode, try [upgrading the Enterprise DC/OS CLI](/docs/1.9/cli/enterprise-cli/#ent-cli-upgrade), deleting the secret, and recreating it.
...
**Prerequisite:** If your [security mode](/docs/1.9/overview/security/security-modes/) is `permissive` or `strict`, you must follow the steps in [Obtaining and passing the DC/OS certificate in curl requests](/docs/1.9/networking/tls-ssl/get-cert/) before issuing the curl commands in this section. If your [security mode](/docs/1.9/overview/security/security-modes/) is `disabled`, you must delete `--cacert dcos-ca.crt` from the commands before issuing them.
