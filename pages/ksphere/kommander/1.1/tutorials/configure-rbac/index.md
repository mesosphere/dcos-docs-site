...
- _Workspace viewer_: these users can access the workspaces they have specifically been given access to in a read-only capacity.
...
The two roles for this persona are:
...
3. Select the _Cluster Roles_ tab. Kommander creates several default roles, the ones to select for this persona are _Cluster Admin Role_ and _Kommander Global Admin Role_.
...
Assuming `kubectl` is attached to your Kommander cluster and you have a `VirtualGroup` named `global-admin`. You can apply these two `VirtualGroupKommanderClusterRoleBinding` to link the group to roles:
...
The two cluster roles for this persona are:
...
1. Select _Global_ in the header drop-down.
2. Select _Administration_ > _Access Control_.
3. Select the _Cluster Roles_ tab. Kommander creates several default roles, the ones to select for this persona are _View Role_ and _Global Kommander View Role_.
...
Assuming `kubectl` is attached to your Kommander cluster and you have a `VirtualGroup` named `global-viewer`. You can apply these two `VirtualGroupKommanderClusterRoleBinding` to link the group to roles:
...
- _Kommander Workspace Admin Role_: a `KommanderWorkspaceRole` that applies to the management cluster. This role grants admin access to the workspace namespace on the management cluster. When a `VirtualGroupKommanderWorkspaceRoleBinding` with this role gets created, a `ClusterRole` and `VirtualGroupKommanderClusterRoleBinding` will automatically be created at the global level to allow access to this workspace specifically, instead of all workspaces.
...
1. Select _Global_ in the header drop-down.
2. Select _Administration_ > _Access Control_.
3. Select the _Cluster Roles_ tab. The two roles for this persona are _Workspace Admin Role_ and _Kommander Workspace Admin Role_.
4. Select the Cluster Policies tab and assign these two roles to the _Workspace Admin_ group.

#### Using kubectl

Assuming `kubectl` is attached to your Kommander cluster and you have a `VirtualGroup` named `workspace-admin`. You can apply this template to link the group to roles:
...
The _Workspace Viewer_ persona will grant users in a speific group read access to the Default Workspace. Users in the group can view all namespaced resources in the workspace namespace on the management cluster, as well as all cluster resources on the workspace’s target clusters.

The two roles for this persona are:

- _Workspace View Role_: a `WorkspaceRole` that applies to all target clusters in all workspaces
- _Kommander Workspace View Role_: a `KommanderWorkspaceRole` that applies to the management cluster. This role grants read access to the workspace namespace on the management cluster. When a `VirtualGroupKommanderWorkspaceRoleBinding` with this role gets created, a `ClusterRole` and `VirtualGroupKommanderClusterRoleBinding` will automatically be created at the global level to allow access to this workspace specifically, instead of all workspaces.
...
1. Select _Global_ in the header drop-down.
2. Select _Administration_ > _Access Control_.
3. Select the _Cluster Roles_ tab. The two roles for this persona are _Workspace View Role_ and _Kommander Workspace View Role_.
4. Select the Cluster Policies tab and assign these two roles to the _Workspace Viewer_ group.

#### Using kubectl

Assuming `kubectl` is attached to your Kommander cluster and you have a `VirtualGroup` named `workspace-viewer`. You can apply this template to link the group to roles:
