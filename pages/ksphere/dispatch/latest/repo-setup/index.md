...
After you have saved the file as `Dispatchfile`, run `dispatch print-pipeline` to output the pipeline rendered as YAML to check for any syntax errors.
...
If you want to preview your build, you can set up a locak 'kind' Kubernetes cluster, run the command `dispatch build local`, and then destroy the cluster afterwards.

To run your Dispatchfile, run:

```bash
dispatch build local --tasks test team-1
```

Alternatively, you can run the `dispatch build tekton` command in a running Dispatch cluster:
...
```bash
dispatch build local --tasks build team-1
```

Alternatively, run it in a running Dispatch cluster with:

```bash
dispatch build tekton --tasks build team-1 --follow
```

The result should look like this:

```bash
cicd-hello-world/ Tue Jan 28$ dispatch build tekton --tasks test --service-account team-1 --follow
...
The `dispatch create repository` command creates a `Repository` object in Kubernetes for your
