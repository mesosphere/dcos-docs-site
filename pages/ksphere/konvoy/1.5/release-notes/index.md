...
### Version v1.5.0-rc.3 - Released 14 July 2020
...
- CLI: Fix a bug where an air-gapped upgrade would fail due to the missing `kubernetes-cni` package, when installing `kubeadm`.
- CLI: Fix a bug where using `--verbose` flag could cause stack trace errors.
- CLI: Fix a bug where the `--v` flag was not respected for many commands.
- Ansible: Fix a bug where configuring with `imageRegistries` username and password did not work with with Containerd `1.3.4`.

#### Component version changes

- kubernetes-base-addons `stable-1.17-2.0.2`
- Autoprovisioning(YAKCL) `v0.1.6`
- Kommander `testing-1.17-1.1.0-rc.7`
- Dispatch `stable-1.17-1.2.2`
- Go `1.13.13`

### Version v1.5.0-rc.2 - Released 30 June 2020
...
- Kubernetes `v1.17.8`
- Autoprovisioning(YAKCL) `v0.1.4`
- Kommander `testing-1.17-1.1.0-rc.5`

### Version v1.5.0-rc.1 - Released 26 June 2020
...
- CLI: Fix a bug where the cluster state was not pushed when running `konvoy deploy kubernetes` or `konvoy deploy addons` commands.
- CLI: Support the autoscaler functionality when cluster name contains special characters `_` and `.`.

#### Improvements

- CLI: New command `konvoy deploy auto-provisioning` to deploy only the auto-provisioning functionality.
- CLI: New command `konvoy run playbook` to run custom Ansible playbooks using.
- CLI: New flag `--force` for `konvoy pull` command to fetch cluster information even if another provisioning process is running in the cluster.
- CLI: Extend the usage of `--target-node-pool` flag for node installation and not only upgrades.
- CLI: Execute the Calico preflight checks in Go code to greatly improve the installation speed on large clusters.
- Ansible: Only run preflight checks on nodes that will be modified, greatly improving the installation of additional nodes or upgrade speeds on large clusters.
- Ansible: The `autoscaler` chart is now deployed using Helm, allowing to override parameters using the `autoProvisioning` field in cluster.yaml.
...
- Fix a bug in the kubeaddons KUDO driver which can lead to Addons not being deployed.
- Fix a bug leading to the kubeaddons controller crash-looping when deploying KUDO based addons.
- The kubeaddons controller now supports `v1beta2` addons, allowing to configure multiple addons with a single data source.

#### Known issues and limitations

- Using the autoscaling functionality when the cluster names begins with numbers is not supported.

#### Component version changes

- Kubernetes `v1.17.7`
- Kubeaddons `v0.16.2`
- kubernetes-base-addons `testing-2.0.0-5`
- Kommander `v1.1.0-rc.4`
- Dispatch `v1.2.1`
...
- CLI: Allow symlinks in the `extras/provisioner` directory.
- CLI: The `konvoy pull` command will now create the state directory if it does not exist.

#### Addon Improvements

- Prometheus: the upgrade from a previous version will no longer fail.

#### Component version changes

- kubernetes-base-addons `testing-2.0.2`
...
- Upgrading Containerd from `1.2.x` to `1.3.4` will fail.

### Version v1.5.0-beta.6 - Released 4 June 2020
...
- CLI: Fix `konvoy completion` command containing extra carriage-return in line termination.
- CLI: Fix a bug when `konvoy reset` may fail when the Kubernetes apiserver is not responding.
- CLI: Support deploying KUDO based addons on airgapped clusters.

#### Improvements

- API: Rename field `v1beta2.helmRepository` to `v1beta2.addonRepository`, `cluster.yaml` files generated by previous versions of Konvoy `v1.4.x` with `v1beta1` API will not be impacted.
- CLI: the `--force-upgrade` flag now also applies to the control-plane nodes.
- Ansible: no longer assume `PATH` settings on the nodes, which caused issues in certain environments not being able execute some Ansible tasks.
...
- Keepalived `v2.0.20`
- Kubeaddons `v0.15.3`
- kubernetes-base-addons `testing-2.0.1`
- Kommander `v1.1.0-rc.2`
