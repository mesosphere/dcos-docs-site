...
The following setup and configuration are required:
...
This opens a browser window requesting your credentials. After supplying your credentials, the command line displays some of your account information (including your accesible subscriptions).
...
If your account has access to multiple subscriptions, select a subscription to use. Enter the following command. It requires your subscription name which can be found in the output of the `az login` command.

  ```bash
  az account set --subscription "Your Subscription Name"
  ```

You must be authorized to the roles of a `Contributor` and a `User Access Administrator` in your Azure account. Your account must be able to create and assign roles to a user. To do this, you or your Azure Administrator must run the following command:

```bash
az role assignment create --assignee YOUR_USER_LOGIN --role "User Access Administrator"
```

You can validate your own roles with following command:
...
After verifying your prerequisites, enter the following command to create all of the default configuration files to launch an Azure Kubernetes cluster. THis current directory where you create your configuration files and cluster will be the directory where you run any subsequent Konvoy commands for future administration.
...
The current directory now contains two certificate files (`.pub` and `.pem`) used in the provisioning of the cluster. It also contains a file named `cluster.yaml` which has all of the default configurations for building a Konvoy cluster on Azure.
...
If your cluster requires additional configuration, you can specify it by creating a `extras/cloud-provider/cloud.conf` file in your working directory.
...
In the case when both files are specified, the remote `/root/kubernetes/cloud.conf` file is used.

### Install the cluster

To install the cluster, ensure you are in the correct directory with the certificates and `cluster.yaml` file, and run the following command:

  ```bash
  konvoy up -y
  ```

This starts the creation of the Azure VM instances and environment. This installs Kubernetes and all of the addons specified in the `cluster.yaml` file.  With the default configuration, it should take about 30 minutes for the Konvoy cluster to be provisioned.

When the deployment is complete, you should see output similar to the following:
...
To verify all of the Konvoy components are installed and running correctly, run this command:
...
You can use the user interface to monitor and operate your cluster through the [Operations Portal][ops_portal]. The URL, username, and password to connect are in the output from the `konvoy up` command. If you need to access this information again, you can run the following command:
...
To uninstall Konvoy and destroy all of the artifacts created in Azure by the deployment, enter the following command:
...
As mentioned previously in the [Preparing the Cluster Configurations][preparing_cluster_configs] section, the directory where you ran `konvoy up` remains the directory you need, to run any subsequent `konvoy` CLI commands.  The `konvoy` CLI is used to administer the cluster (upgrade, deprovision, scale, change configs, install Addons, etc.).  This directory is important because it now contains the following generated files:
