...
This section describes different networking components that come together to form the Konvoy networking stack. It assumes that you are familiar with Kubernetes networking.
...
High availability is provided through the cloud provider load balancer.
...
Further, you can set `spec.kubernetes.controlPlane.keepalived.vrid` to specify the [Virtual Router ID][keepalived_conf] used by Keepalived.
...
A detailed explanation of all these plugins can be found [here][coredns_plugins].

You can modify the CoreDNS configuration by updating the `configmap` named `coredns` in `kube-system` namespace.

# Load Balancing

A discussion around Load Balancing can be split into two areas:
...
For on-premise deployments, Konvoy ships with [MetalLB][metallb].
...
In on-premise deployments, by default, it uses MetalLB. MetalLB can be configured as discussed earlier.
