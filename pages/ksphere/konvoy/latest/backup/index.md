...
<p class="message--note"><strong>NOTE: </strong>If you intend to use the cluster without an external storage volume for Minio, you should <a href="#fetching-a-backup-archive">fetch the latest backup</a> and store it in a known, secure location at a regular interval. For example, if you are not using an external storage volume, you should back up and archive the cluster on a weekly basis.</p>
