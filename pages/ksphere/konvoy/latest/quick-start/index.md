...
-   You have [kubectl][install_kubectl] v1.15.3 or later for interacting with the running cluster.
...
1. Verify that you have valid **AWS security credentials** to deploy the cluster on AWS.     This step is not required if you are installing Konvoy on an on-premise environment. For information about installing in an on-premise environment, see [Install on-premise](../install/install_onprem/).
...
You should copy the cluster URL and login information and paste it into a text file, then save the file in a secure, shared location on your network.
...
If there were issues with installing or bringing the Kubernetes cluster online, the add-on installations would fail.
