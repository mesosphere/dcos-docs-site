...
* [kubectl][install_kubectl] v1.15.4 or later (for interacting with the running cluster)
...
And then it installs the Kubernetes cluster in those Docker containers, and installs default add-ons to support the Kubernetes cluster.
...
The following add-ons are not installed by default.
However, you could enable those add-ons by editing `cluster.yaml` and run `konvoy up` again.
