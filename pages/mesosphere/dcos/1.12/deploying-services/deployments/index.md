...
Every change in the definition of applications or groups in DC/OS is performed as a Marathon deployment. A [deployment](/mesosphere/dcos/1.12/deploying-services/marathon-api/) is a set of actions, including:
...
![dependency diagram](/mesosphere/dcos/1.12/img/dependency.png)
...
- **Health checks** are specified in each application and are run against tasks. If health check fails for a task, DC/OS will replace the task. For more information, see the [documentation](/mesosphere/dcos/1.12/deploying-services/creating-services/health-checks/).
