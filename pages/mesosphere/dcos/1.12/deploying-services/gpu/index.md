...
- Review the [prerequisites](/mesosphere/dcos/1.12/installing/evaluation/) of the DC/OS Universal Installer.
- Review the prerequisites of your cloud provider, such as [AWS](/mesosphere/dcos/1.12/installing/evaluation/aws/#prerequisites), [Azure](/mesosphere/dcos/1.12/installing/evaluation/azure/#prerequisites), or [GCP](/mesosphere/dcos/1.12/installing/evaluation/gcp/#prerequisites).

### Customize your main.tf

In the main.tf file you are using to deploy DC/OS, ensure at least one agent is being deployed with one or more GPUs. Also ensure the agent meets all other [agent node requirements](/mesosphere/dcos/1.12/installing/production/system-requirements/#agent-node-requirements).
...
-  NVIDIA GPU support is only available for tasks launched using the [DC/OS Universal Container Runtime](/mesosphere/dcos/1.12/deploying-services/containerizers/). Docker is not supported.
