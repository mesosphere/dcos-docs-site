...
1. Use the [DC/OS CLI](/mesosphere/dcos/1.12/cli/) to add the service to DC/OS.
...
1. Choose whether to use the Universal Container Runtime (UCR) or Docker Engine runtime. See [Using Containerizers](/mesosphere/dcos/1.12/deploying-services/containerizers/).
...
1. Use the [Marathon API](/mesosphere/dcos/1.12/deploying-services/marathon-api/) to deploy the app `basic-3-docker`. Refer to [Authentication HTTP API Endpoint](/mesosphere/dcos/1.12/security/ent/iam-api/) to learn more about the API token required in the command below.
...
1. Scroll down to the **Marathon Task Configuration** section and note the PORTS property.
   ...
   ![container port](/mesosphere/dcos/1.12/img/container-port.png)

   Figure 1. Container port
   
1. Determine the [IP address of the public node](/mesosphere/dcos/1.12/administering-clusters/locate-public-agent/).
