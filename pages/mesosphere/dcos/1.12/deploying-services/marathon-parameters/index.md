...
- **type** The containerizer runtime type, either `MESOS` or `DOCKER`. For more information, see [Using Containerizers](/mesosphere/dcos/1.12/deploying-services/containerizers/).
...
- **docker** The Docker container information.
    ...
    - **pullConfig** A secret whose value is a stringified JSON object in a Secret Store. See [Using a Private Docker Registry](/mesosphere/dcos/1.12/deploying-services/private-docker-registry/#secret-store-instructions).

- **volumes** The volumes accessible to the container.
    - **containerPath** The path where your container will read and write data.
    - **external** An external persistent volume. See [External Persistent Volumes](/mesosphere/dcos/1.12/storage/external-storage/).
        - **name** Name that your volume driver uses to look up the external volume.
        - **provider** The storage provider.
        - **options** Which Docker volume driver to use for storage. The only Docker volume driver supported by DC/OS is [REX-Ray](/mesosphere/dcos/1.12/storage/external-storage/).
    ...
    - **persistent** A local persistent volume. See [Local Persistent Volumes](/mesosphere/dcos/1.12/storage/persistent-volume/).
...
- If you use the [Universal Container Runtime](/mesosphere/dcos/1.12/deploying-services/containerizers/ucr/), pass zeros as port values to generate one or more arbitrary free ports for each application instance.

For more information, see [Containerizers](/mesosphere/dcos/1.12/deploying-services/containerizers/).
...
### residency
Set up a stateful application. For more information, see [local persistent volumes](/mesosphere/dcos/1.12/storage/persistent-volume/). **Deprecated**.
