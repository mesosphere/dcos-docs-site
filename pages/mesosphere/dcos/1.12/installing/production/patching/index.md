...
<p class="message--note"><strong>NOTE: </strong>These instructions are only appropriate for a change to the cluster configuration or the maintenance version number. For example, from DC/OS 1.12.1 to 1.12.2.</p>

- To update to a newer major or minor version (for example, from 1.11 to 1.12), refer to the instructions for [upgrading](/mesosphere/dcos/1.12/installing/production/upgrading/).
...
- Review the [release notes](/mesosphere/dcos/1.12/release-notes/) before patching DC/OS.
...
See the security [mode](/mesosphere/dcos/1.12/installing/production/advanced-configuration/configuration-reference/#security-enterprise) for information on different security modes.
...
- You must be familiar with the [DC/OS Installation Guide](/mesosphere/dcos/1.12/installing/production/deploying-dcos/installation/).
- [Take a snapshot of ZooKeeper](/mesosphere/dcos/1.12/installing/installation-faq/#q-how-do-i-backup-zookeeper-using-guano) prior to patching. Marathon supports rollbacks, but does not support downgrades.
- [Take a snapshot of the IAM database](/mesosphere/dcos/1.12/installing/installation-faq/#q-how-do-i-backup-the-iam-database-enterprise) prior to patching.
...
- [Back up your cluster](/mesosphere/dcos/1.12/administering-clusters/backup-and-restore/).
- **Optional** You can add custom [node and cluster health checks](/mesosphere/dcos/1.12/installing/production/deploying-dcos/node-cluster-health-check/#custom-health-checks) to your `config.yaml`.
...
This procedure patches a DC/OS 1.12 cluster without changing the cluster's [security mode](/mesosphere/dcos/1.12/1.12/installing/production/advanced-configuration/configuration-reference/#security-enterprise).
...
1.  Build your installer package.
    ...
    1.  Run the [nginx](/mesosphere/dcos/1.12/installing/production/deploying-dcos/installation/) container to serve the installation files.

1.  Go to the DC/OS Master [procedure](/mesosphere/dcos/1.12/installing/production/patching/#masters) to complete your installation.

# <a name="strict"></a>Patching to DC/OS 1.12 in strict mode
This procedure patches to DC/OS 1.12 in strict [security mode](/mesosphere/dcos/1.12/installing/production/advanced-configuration/configuration-reference/#security-enterprise).
...
1.  Build your installer package.
    ...
    1.  Generate the installation files. Replace `<installed_cluster_version>` in the below command with the DC/OS version currently running on the cluster you intend to patch, for example `1.8.8`.
        ...
        ```
    1.  The command in the previous step will produce a URL in the last line of its output, prefixed with `Node patch script URL:`. Record this URL for use in later steps. It will be referred to in this document as the "Node patch script URL".
    2.  Run the [nginx](/mesosphere/dcos/1.12/installing/production/deploying-dcos/installation/) container to serve the installation files.
...
1.  Validate the patch:
    ...
    1.  Verify that `/opt/mesosphere/bin/mesos-master --version` indicates that the patched master is running the version of Mesos specified in the [release notes](/mesosphere/dcos/1.12/release-notes/), for example `1.5.1`.
...
If your patch fails because of a [custom node or cluster check](/mesosphere/dcos/1.12/installing/production/deploying-dcos/node-cluster-health-check/#custom-health-checks), run these commands for more details:
```bash
