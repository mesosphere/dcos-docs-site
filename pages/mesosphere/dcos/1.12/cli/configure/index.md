...
You can access DC/OS CLI configuration with the [dcos cluster](/mesosphere/dcos/1.12/cli/command-reference/dcos-cluster/) and [dcos config](/mesosphere/dcos/1.12/cli/command-reference/dcos-config/) command groups.
...
To set the [attached cluster](/mesosphere/dcos/1.12/cli/command-reference/dcos-cluster/dcos-cluster-attach/), set the variable with the command:
...
* Define `no_proxy` for domains that you do not want to use the proxy for:
   ...
   This setting generates and updates per cluster configuration under `$DCOS_DIR/clusters/<cluster_id>`. Generates a newly setup cluster [as seen here](/mesosphere/dcos/1.12/cli/#setting-up-a-cluster).
