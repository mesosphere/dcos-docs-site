...
For more information on the `dcos task exec` commands, see the [CLI Reference section](/mesosphere/dcos/1.12/cli/command-reference/dcos-task/dcos-task-exec/).
...
- A container launched by using the [DC/OS Universal container runtime](/mesosphere/dcos/1.12/deploying-services/containerizers/)
...
In this example, a long running [job](/mesosphere/dcos/1.12/deploying-jobs/) is launched by using the `dcos job run` command, and the `dcos task exec` command is used to launch an interactive Bash shell inside the container of that job.
