...
Each service task has some number of environment variables, which are used to configure the task. These environment variables are set by the service Scheduler. While it is possible to use these environment variables in ad hoc scripts (for instance, via `dcos task exec`), the name of a given environment variable may change between versions of a service and should not be considered a public API of the service.

## {{ model.techName }} Specific
- DC/OS Enterprise only
- Rosco and Kayenta services are currently not included.
- Fiat service based authorization is currently not supported.
- Connection to Redis currently does not support TLS
