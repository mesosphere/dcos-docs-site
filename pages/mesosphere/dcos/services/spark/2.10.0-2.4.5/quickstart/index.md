...
* Depending on your [security mode](/mesosphere/dcos/1.12/security/ent/), {{ model.techShortName }} requires service authentication for access to DC/OS. See [Provisioning a service account](/mesosphere/dcos/services/spark/2.10.0-2.4.5/security/#provision-a-service-account) for more information.
...
1. Install the {{ model.techShortName }} package. This may take a few minutes. This step installs the {{ model.techShortName }} DC/OS service, {{ model.techShortName }} CLI, dispatcher, and, optionally, the history server. See the [History Server](/mesosphere/dcos/services/spark/2.10.0-2.4.5/history-server/#installing-hdfs) section for information about how to install the history server.
    ...
    ```bash
    Installing Marathon app for package [{{ model.packageName }}] version [2.10.0-2.4.5]
    Installing CLI subcommand for package [{{ model.packageName }}] version [2.10.0-2.4.5]
...
1. Run the sample SparkPi jar for DC/OS.
    ...
    1. Use the following command to run a {{ model.techShortName }} job which calculates the value of Pi.
        ...
        ```bash
        2017/08/24 15:42:07 Using docker image mesosphere/spark:2.10.0-2.4.5-scala-2.11-hadoop-2.9 for drivers
        2017/08/24 15:42:07 Pulling image mesosphere/spark:2.10.0-2.4.5-scala-2.11-hadoop-2.9 for executors, by default. To bypass set spark.mesos.executor.docker.forcePullImage=false
...
2. Run a Python SparkPi jar. You can view the example source [here](https://downloads.mesosphere.com/spark/examples/pi.py).
    ...
    1. Use the following command to run a Python {{ model.techShortName }} job which calculates the value of Pi.
        ...
        2017/08/24 15:44:23 Using docker image mesosphere/spark:2.10.0-2.4.5-scala-2.11-hadoop-2.9 for drivers
        2017/08/24 15:44:23 Pulling image mesosphere/spark:2.10.0-2.4.5-scala-2.11-hadoop-2.9 for executors, by default. To bypass set spark.mesos.executor.docker.forcePullImage=false
...
3. Run an R job. You can view the example source [here](https://downloads.mesosphere.com/spark/examples/dataframe.R).
    ...
    1. Use the following command to run an R job.
        ...
        2017/08/24 15:45:23 Using docker image mesosphere/spark:2.10.0-2.4.5-scala-2.11-hadoop-2.9 for drivers
        2017/08/24 15:45:23 Pulling image mesosphere/spark:2.10.0-2.4.5-scala-2.11-hadoop-2.9 for executors, by default. To bypass set spark.mesos.executor.docker.forcePullImage=false
