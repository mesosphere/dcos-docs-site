...
- **Jenkins Master**: The Jenkins master only runs on Linux agents. To prevent the Jenkins master from being deployed on Windows agents, the new configuration option `service.os-anti-affinity` was introduced. The default value of `windows` prevents Marathon from launching the Jenkins Master on any nodes with this Mesos attribute.
