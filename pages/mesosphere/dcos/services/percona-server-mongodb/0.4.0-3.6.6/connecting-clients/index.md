...
- CLI:
  - List endpoint types: `dcos percona-server-mongodb endpoints`
  - View endpoints for an endpoint type: `dcos percona-server-mongodb endpoints <endpoint>`
- API:
  - List endpoint types: `<dcos-url>/service/percona-server-mongodb/v1/endpoints`
...
In general, the `.autoip.dcos.thisdcos.directory` endpoints will only work from within the same DC/OS cluster. From outside the cluster you can either use direct IPs or set up a proxy service that acts as a frontend to your Percona-Server-MongoDB instance. For development and testing purposes, you can use [DC/OS Tunnel](/mesosphere/dcos/latest/administration/access-node/tunnel/) to access services from outside the cluster, but this option is not suitable for production use.
