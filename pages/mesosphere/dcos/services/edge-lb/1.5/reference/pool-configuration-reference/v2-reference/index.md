...
| role                        | string   |             |                 | Apache&reg; Mesos&reg;  role for load balancers. Defaults to "slave_public" so that load balancers will be run on public agents. Use "*" to run load balancers on private agents. Read more about Mesos roles at http://mesos.apache.org/documentation/latest/roles/.  |
| cpus                        | number   |             |                 |                |
...
| constraints                 | string   | true        |                 | Marathon&trade; style constraints for load balancer instance placement. |
...
| bindPort        | integer |                | The port (for exmaple port 80 for HTTP or 443 for HTTPS) to which this frontend will bind.  |   | int32  |
...
| linkBackend     | object  | <ul><li>defaultBackend</li><li>map</li></ul>  | This describes which backends to which to send traffic. This can be expressed with a variety of filters such as matching on the hostname or the HTTP URL path.<br />Default: map: []   |   |   |
...
| path            | object  | <ul><li>[fromPath](#path-prop)</li><li>[toPath](#path-prop)</li></ul>  | Rewrite the HTTP URL path. All fields required, otherwise it is ignored.  |
| request         |         |                |                |
| response        |         |                |                |
| sticky          | object  | <ul><li>[enabled](#sticky-prop)</li><li>[customStr](#sticky-prop)</li></ul>  | Sticky sessions through use of a cookie.<br />To use the default values (recommended), set this field to the empty object.  |
...
| check       | object    | Enable health checks.  These are required for DNS resolution to function properly, and are, by default, TCP health checks.  For more options see "customCheck". |
