...
<tr><td>balance</td><td>string</td><td>Specifies the load balancing strategy. For example, `roundrobin` or `leastconn`. For more information about load balancing algorithms you can use, see `HAPROXY_BACKEND_HEAD` in <a href="/mesosphere/dcos/services/edge-lb/1.5/reference/haproxy-templates/#backend-template-settings"> HAProxy templates and labels</a>.</td></tr>
