...
1. Use `add-stub-universe.sh` script to add to the Universe the JSON definitions obtained during the [Obtaining package artifacts](/mesosphere/dcos/services/edge-lb/1.2/installing/#obtaining-package-artifacts) step.  Each run of the `add-stub-universe.sh` script will process the JSON file, generate the necessary JSON and Mustache files, and add them to `stub-repo/packages/<X>/<packagename>`.
