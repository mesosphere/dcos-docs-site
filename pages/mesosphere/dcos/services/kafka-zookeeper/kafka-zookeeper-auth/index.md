...
This topic describes how to configure DC/OS access for Kafka Zookeeper. Depending on your [security mode](/mesosphere/dcos/latest/security/ent/#security-modes/), Kafka Zookeeper requires [service authentication](/mesosphere/dcos/1.10/security/ent/service-auth/) for access to DC/OS.
...
```bash
dcos security org service-accounts create -p <your-public-key>.pem -d "Kafka Zookeeper service account" <service-account-id>
```
...
**Tip:** If you store your secret in a path that matches the service name (e.g. service name and secret path are `kafka-zookeeper`), then only the service named `kafka-zookeeper` can access it.
...
1.  Create the permission.
    ...
    **Important:** These commands use the default Kafka Zookeeper `role` value of `kafka-zookeeper-role`. If you're running multiple instances of Kafka Zookeeper, replace the instances of `kafka-zookeeper-role` with the correct name (`<name>-role`). For example, if you have a Kafka Zookeeper instance named `kafka-zookeeper2`, you would replace each role value in the code samples to `kafka-zookeeper2-role`.
...
Create a custom configuration file that will be used to install Kafka Zookeeper and save as `config.json`.

Specify the service account (`<service-account-id>`) and secret path (`kafka-zookeeper/<secret-name>`).

```json
{
  "service": {
    "service_account": "<service-account-id>",
    "service_account_name": "kafka-zookeeper/<secret-name>"
  }
}
```

## <a name="install-kafka-zookeeper"></a>Install Kafka Zookeeper
Install Kafka Zookeeper with this command:
