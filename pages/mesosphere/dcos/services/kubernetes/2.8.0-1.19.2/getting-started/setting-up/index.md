...
You will need [superuser access](/mesosphere/dcos/1.12/security/ent/users-groups/reset-superuser/) to an unutilized cluster of virtual machines (VMs). The following is the minimal setup for this current tutorial.
...
## Kubernetes https prerequisite

The Mesosphere Kubernetes Engine (MKE) requires access over https to connect to the kubernetes-apiserver using `kubectl`. To finish setting up the cluster and before moving on to installing the MKE, go ahead and authenticate to the DC/OS CLI using https:
...
3. **Moreover, if the [TLS certificate](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/connecting-clients/) used by DC/OS is not trusted in your case, you can run the following command to disable TLS verification for the purposes of completing this tutorial:**
...
Now that you have your DC/OS cluster all set up, you are ready to [install the Mesosphere Kubernetes Engine](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/getting-started/installing-mke/).
