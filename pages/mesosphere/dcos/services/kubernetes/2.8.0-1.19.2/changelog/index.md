...
### Changelog since 2.7.0-1.18.6

#### Improvements

* Add option to enable custom Docker daemon configuration. See [Custom Docker daemon configuration](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/docker-custom-config) for more details.
* Kubernetes v1.19.2
* etcd v3.4.13
* Calico v3.16.1
* Docker v19.03.13
* Update Debian base container images to version `stable-20200908-slim`.
* Enable extensive Calico metrics
...
* Fix intermittent bug that prevented etcd forming a cluster by removing unnecessary SANs from etcd certificates to reduce amount of DNS queries.

### Changelog since 2.5.0-1.16.9
...
* Calico 3.10.1
* CoreDNS v1.6.5
* Add option to enable AlwaysPullImages Kubernetes admission controller. See [Admission Controllers](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/admission-controllers/) for more details.
* Add option to enable Kubernetes secret encryption. See [Kubernetes secret encryption](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/encrypt-data) for more details.
...
* Add instructions to install [Gatekeeper](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/gatekeeper) and how to use it as a replacement for Kubernetes PodSecurity policies.
...
* dcos-commons 0.57.0
* Adds configuration options for DC/OS Quota support. See [Quota Support](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/quota-support/) for more details.
* Control Plane tasks are now launched with labels that support auto exposure of Kubernetes API via [EdgeLB](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/exposing-the-kubernetes-api-marathonlb-edgelb/)
...
* Kubernetes 1.15.3
* dcos-commons 0.56.3
* Adds [configuration options to enable Kubernetes auditing](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/kubernetes-audit/). Kubernetes auditing provides a security-relevant chronological set of records. These document the sequence of activities that have affected the system by users, administrators or other components of the system.
...
* Fix a bug where a custom OIDC certificate file is not available to the Kubernetes API server when OIDC support is enabled.
...
* Fix a bug where Kubernetes clusters created under a DC/OS folder do not export metrics with the correct Kubernetes cluster name.
...
* Downgrade to Kubernetes 1.14.1 to mitigate https://github.com/kubernetes/kubernetes/issues/78308.
...
* Allow for defining the maximum amount of disk space taken by pods' containers' log files (defaults to 1MB).
  The default can be overridden by setting the  `kubernetes.maximum_container_log_size` configuration option.

#### Documentation

* Fix a few items in the [limitations section](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/limitations/).
...
* Enable unauthenticated etcd /metrics endpoint on port 2381 by default. Available using DC/OS VIP `http://etcd-N-peer.${KUBERNETES_CLUSTER_NAME}.autoip.dcos.thisdcos.directory:2381/metrics` where N is the task instance index.
...
* Fix the [`etcd` snapshot](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/troubleshooting/) instructions.
...
* Add a [Storage](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/storage/) page documenting Container Storage Interface (CSI).
...
* Support relative paths in `--path-to-custom-ca` for `cluster kubeconfig` command, for example `--path-to-custom-ca=./my-custom-ca.pem`.
...
* Documentation section on how to [upgrade](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/upgrade/#Mesosphere-Kubernetes-Engine) the `kubernetes` package.
...
* Add an [Overview](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/overview/) page explaining in detail what changed since the 1.x series of releases.
* Add a [CLI](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/cli/) page detailing the new Mesosphere Kubernetes Engine CLI.
* Merge `Advanced Installation` page merging its content into [Customizing your Installation](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/customizing-install/).
* Add a [Private Docker Registry](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/private-docker-registry/) page explaining how to configure it.
