...
Hence, to access the Kubernetes API for one or more Kubernetes clusters from outside the DC/OS cluster, you can set up a proxy that can reach the DC/OS VIP(s) exposing the Kubernetes API instance(s) inside the DC/OS cluster.
...
In the next sections we present two examples you can follow to expose the Kubernetes API for a given Kubernetes cluster to outside the DC/OS cluster.
...
Alternately, if you are running Marathon-LB or Edge-LB in your DC/OS cluster, you may wish to expose the Kubernetes API for a given Kubernetes cluster via one of those. Details on how to do this are documented [here](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/exposing-the-kubernetes-api-marathonlb-edgelb/).
...
![Exposing the Kubernetes API using HAProxy](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/img/haproxy.png)
...
to achieve this, you can use `openssl`:
...
Run the following command (replacing the `<SERVICE_NAME>` placeholder as appropriate) to create a working HAProxy configuration:
...
<p class="message--note"><strong>NOTE: </strong>In the instructions above it is assumed that <tt>&lt;user&gt;</tt> has permissions to create the <tt>&lt;path-to-haproxy-config-directory&gt;</tt>. If this is not the case in your setup, you will probably need your systems administrator to perform this step and set up adequate permissions on said directory.
</p>
...
To configure `kubectl` to access the Kubernetes API using this setup, follow the steps described in the ["Without TLS verification"](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/connecting-clients#without-tls-verification) subsection of the [Connecting Clients](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/connecting-clients/) page.
...
In a production scenario, the above points should be addressed, both to increase the overall security of the solution and to be able to expose the Kubernetes API for multiple Kubernetes clusters using the same HAProxy instance.

This example builds on the previous one and focuses on addressing all these shortcomings. In what follows, `<SERVICE_NAME_1>` and `<SERVICE_NAME_2>` represent the names of two different Kubernetes clusters (for example `dev/kubernetes01` and `dev/kubernetes02`).
...
This is done by letting HAProxy know that the certificate authority that signs each of the certificates is trustworthy. To do this, retrieve each of these certificate authorities:
...
In DC/OS, these instructions must be repeated for `<SERVICE_NAME_2>`. In DC/OS Enterprise it is enough to run this step once, and `<SERVICE_NAME_1>-ca.pem` can be renamed to something more generic (for example `dcos-ca.pem`).
...
* DNS is configured in such a way that each of the domains resolve to the IP address(es) of the public DC/OS agent(s) where HAProxy will be running (for example, `devkubernetes01.example.com` and `devkubernetes02.example.com` both resolve to `35.233.213.97s`).
...
to expose the Kubernetes API for both `<SERVICE_NAME_1>` and `<SERVICE_NAME_2>` using the same instance of HAProxy, we will use HAProxy's [Server Name Indication](https://en.wikipedia.org/wiki/Server_Name_Indication) feature.
...
To configure `kubectl` to access the Kubernetes API using this setup, follow the steps described in the ["With TLS verification"](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/connecting-clients#with-tls-verification) subsection of the [Connecting Clients](/mesosphere/dcos/services/kubernetes/2.8.0-1.19.2/operations/connecting-clients/) page.
