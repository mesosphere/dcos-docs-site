...
</br></br>In case the returned URL does not start with <tt>https://</tt> run:
...
to configure `kubectl` to access the Kubernetes API without validating the presented TLS certificate, run the following command, replacing `https://kube-apiserver.example.com:6443` with the URL at which the Kubernetes API is exposed to outside the DC/OS cluster:
...
to configure `kubectl` to access the Kubernetes API while validating the presented TLS certificate run the `dcos kubernetes cluster kubeconfig` command.
...
To create the DC/OS Kubernetes configuration without switching the context, run:
...
kube-control-plane-0-instance.kubernetes-cluster.mesos   Ready     master    44m       v1.19.2
kube-node-0-kubelet.kubernetes-cluster.mesos             Ready     <none>    43m       v1.19.2
```
