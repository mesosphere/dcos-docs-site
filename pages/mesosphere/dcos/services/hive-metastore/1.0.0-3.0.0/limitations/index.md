...
### Compatible modes
Using percona-pxc-mysql under mode `PERMISSIVE` or `DISABLED` works with Hive Metastore out of the box.

### Non-Compatible modes
Using percona-pxc-mysql under mode `ENFORCING` or `MASTER` is not supported in the current Hive Metastore version.
