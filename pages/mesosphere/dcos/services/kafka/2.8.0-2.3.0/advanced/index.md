...
When [Kerberos](/mesosphere/dcos/services/kafka/2.7.0-2.3.0/security/#authentication) and or [Transport Encryption](mesosphere/dcos/services/kafka/2.7.0-2.3.0/security/#transport-encryption) is enabled the health check produces only a single random message to a topic and will attempt to consume that same first message of the topic at each health-check interval.
