...
- It is possible to upgrade directly to `2.1.0` from version `2.0.0`. However, you'll need to run the following command to successfully upgrade your {{ model.techName }} package: 

  ```
  dcos {{ model.serviceName }} update start --package-version=2.1.0 --replace
  ```
