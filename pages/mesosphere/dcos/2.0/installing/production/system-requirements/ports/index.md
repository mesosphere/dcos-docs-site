...
[DC/OS components](/mesosphere/dcos/1.14/overview/architecture/components/) listen on multiple ports on each node. These ports must be available for installation to succeed.
...
- You must use appropriate network mechanisms to prevent unauthorized access to cluster nodes. Refer to the documentation on [network security](/mesosphere/dcos/1.14/administering-clusters/securing-your-cluster/#network-security).
