...
The DC/OS&trade; GUI Update Service updates the current version of the DC/OS GUI using new versions published to the {{ model.packageRepo }}. This service allows you to update your GUI without having to update your version of DC/OS.
...
When processing an update request, the master that receives the API call downloads the package, extracts its contents and starts serving the new version. If the cluster has multiple masters, the DC/OS GUI update will be eventually consistent. 
