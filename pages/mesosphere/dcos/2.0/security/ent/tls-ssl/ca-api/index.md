...
To get an authentication token, pass the user name and password of a user with the necessary permissions in the body of a request to the `/auth/login` endpoint of the [Identity and Access Management Service API](/mesosphere/dcos/2.0/security/ent/iam-api/). It returns an authentication token as shown below.
...
When you log into the [DC/OS CLI](/mesosphere/dcos/2.0/cli/) using `dcos auth login`, it stores the authentication token value locally. You can reference this value as a variable in cURL commands (discussed in the next section).
...
[swagger api='/mesosphere/dcos/2.0/api/certificate-authority.yaml']
