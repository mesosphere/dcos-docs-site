...
If you run the `dcos node list` command without this permission, the command returns node information but does not include the Public IP address column in the output. For more information about setting and managing permissions, see [Permissions Management](/mesosphere/dcos/2.1/security/ent/perms-management/) and [Permissions Reference](/mesosphere/dcos/2.1/security/ent/perms-reference/).
