...
Using the [DC/OS CLI](/mesosphere/dcos/2.2/cli/) one can log in as a local DC/OS user by specifying the `dcos-users` login provider.

1. To log in via the DC/OS CLI, replace `uid` and `password` in the following [auth login](/mesosphere/dcos/2.2/cli/command-reference/dcos-auth/dcos-auth-login/) command:
...
Local users can log in using [Identity and Access Management (IAM) API](/mesosphere/dcos/2.2/security/oss/iam-api/).
