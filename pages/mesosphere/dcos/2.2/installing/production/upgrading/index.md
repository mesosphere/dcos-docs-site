...
- The Production installation method is the **only** recommended upgrade path for DC/OS. It is recommended that you familiarize yourself with the [DC/OS Deployment Guide](/mesosphere/dcos/{{ model.folder_version }}/installing/production/deploying-dcos/) before proceeding.
- Review the [release notes](/mesosphere/dcos/{{ model.folder_version }}/release-notes/) before upgrading DC/OS.
...
- Take a [snapshot of ZooKeeper](/mesosphere/dcos/{{ model.folder_version }}/installing/installation-faq/#q-how-do-i-backup-zookeeper-using-guano) prior to upgrading. Marathon supports rollbacks, but does not support downgrades.
- Take a [snapshot of the IAM database](/mesosphere/dcos/{{ model.folder_version }}/installing/installation-faq/#q-how-do-i-backup-the-iam-database-enterprise) prior to upgrading. **This is very easy to do and should be considered a necessity.**
...
- [Back up your cluster](/mesosphere/dcos/{{ model.folder_version }}/administering-clusters/backup-and-restore/). [enterprise type="inline" size="small" /]
- Optional: You can add custom [node and cluster health checks](/mesosphere/dcos/{{ model.folder_version }}/installing/production/deploying-dcos/node-cluster-health-check/) to your `config.yaml`.
...
1.  Verify that `/opt/mesosphere/bin/mesos-master --version` indicates that the upgraded master is running the version of Mesos specified in the [release notes](/mesosphere/dcos/{{ model.folder_version }}/release-notes/), for example `1.9.1`.
...
If your upgrade fails because of a [custom node or cluster check](/mesosphere/dcos/{{ model.folder_version }}/installing/production/deploying-dcos/node-cluster-health-check/), run these commands for more details:
```bash
