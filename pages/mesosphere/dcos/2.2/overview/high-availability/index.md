...
With DC/OS, you can distribute masters across racks for HA. Agents can be distributed across regions, and it is recommended that you tag agents with attributes to describe their location. Synchronous services like ZooKeeper should also remain within the same region to reduce network latency. For more information, see [Configuring zones and regions](/mesosphere/dcos/2.2/installing/production/advanced-configuration/configuring-zones-regions/) for high-availability.
