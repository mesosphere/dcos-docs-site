...
- [Deploying non-native instances of Marathon](/mesosphere/dcos/2.2/deploying-services/marathon-on-marathon/)
- [Spark Quota](/mesosphere/dcos/services/spark/latest/job-scheduling/#quota-for-drivers-and-executors)

In the examples below, it is recommended to run the application from a host with [DC/OS CLI](/mesosphere/dcos/2.2/cli/) installed.
...
The DC/OS {{ model.packageRepo }} includes Marathon, which can be used to deploy a MoM. It should be noted that this is only useful for DC/OS OSS installations, as it does not provide support for Strict mode, Secrets or ACLs. See the [Marathon on Marathon documentation](/mesosphere/dcos/2.2/deploying-services/marathon-on-marathon/basic/).

To install DC/OS Enterprise MoM, you must contact Mesosphere Support for the Enterprise MoM tarball, then deploy it using the root Marathon. See the [custom non-native Marathon documentation](/mesosphere/dcos/2.2/deploying-services/marathon-on-marathon/advanced/).
