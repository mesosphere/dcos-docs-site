...
You can also [configure Marathon's behavior when a task is unreachable](/mesosphere/dcos/2.2/deploying-services/task-handling/configure-task-handling/).

![Task Handling Flow](/mesosphere/dcos/2.2/img/task-handling-corrected.png)
...
If the task was configured to use [Local Persistent Volumes](/mesosphere/dcos/2.2/storage/persistent-volume), these will be abandoned given that the agent is considered to be gone and not able to offer those volumes. A new task will be created as a replacement, with new volumes for its use.
