...
This topic provides field definitions and usage examples for pods. For more details on field definitions, see [Marathon Configuration Reference](/mesosphere/dcos/2.2/deploying-services/marathon-parameters/).
...
For an example of a pod that uses a persistent volume, see [Create a pod with a local persistent volume](/mesosphere/dcos/2.2/storage/persistent-volume/#create-a-pod-with-a-local-persistent-volume).

## IP-per-Pod Networking

The following pod definition specifies a virtual (user) network named `dcos`. The `networks:mode:container` field creates the virtual network. The `name` field is optional. If you have installed DC/OS using [our AWS templates](/mesosphere/dcos/2.2/installing/evaluation/community-supported-methods/aws/), the default virtual network name is `dcos`. <!-- Validated by suzanne 6-23-17 -->
