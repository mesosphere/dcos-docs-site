...
The Logging API is backed by the [DC/OS Log component](/mesosphere/dcos/2.2/overview/architecture/components/#dcos-log), which runs on all nodes in the cluster. For more information about using the Logging API, see [Logging](/mesosphere/dcos/2.2/monitoring/logging/). For usage examples, see [Logging API Examples](/mesosphere/dcos/2.2/monitoring/logging/logging-api-examples/).
...
In versions of DC/OS prior to 1.11, node and component logs were managed by `journald`. However, the [Mesos task journald log sink was disabled due to [journald performance issues](/mesosphere/dcos/2.2/installing/production/advanced-configuration/configuration-reference/#mesos-container-log-sink). So container log files for older versions are only accessible via the [Mesos task sandbox files API](http://mesos.apache.org/documentation/latest/sandbox/).
...
To determine the address of your cluster, see [Cluster Access](/mesosphere/dcos/2.2/api/access/).
...
All routes may also be reached by users with the _dcos:superuser_ permission. To assign permissions to your account, see [Permissions Reference](/mesosphere/dcos/2.2/security/ent/perms-reference/).
...
## Resources
 ...
 [swagger api='/mesosphere/dcos/2.2/api/logs2.yaml']
