...
Your cluster will become more secure as you move from `disabled` to `permissive` to `strict` [security modes](/mesosphere/dcos/1.9/security/ent/#security-modes).
...
- <a name="secure-flag"></a>In `permissive` and `strict` modes, set the [`auth_cookie_secure_flag`](/mesosphere/dcos/1.9/installing/ent/custom/configuration/configuration-parameters/#auth-cookie-secure-flag-enterprise) to `true`.
...
- [Provision services with service accounts even when optional](/mesosphere/dcos/1.9/security/ent/service-auth/).

- [Use secrets to store and pass sensitive information to services](/mesosphere/dcos/1.9/security/ent/secrets/).

- [Use spaces to restrict service access to secrets](/mesosphere/dcos/1.9/security/ent/#spaces).

- Tightly restrict the distribution of SSH keys. For debugging, consider using [`dcos task exec`](/mesosphere/dcos/1.9/monitoring/debugging/) instead. 
...
- [Override the Linux user account of your services](/mesosphere/dcos/1.9/security/ent/users-groups/config-linux-user/) to use a less privileged account such as `nobody`.
