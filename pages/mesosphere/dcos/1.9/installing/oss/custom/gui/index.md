...
3.  Click **Begin Installation**.
    ...
    ![Begin Install](/mesosphere/dcos/1.9/img/dcos-gui-install.png)

4.  Specify your Deployment and DC/OS Environment settings:

    ![preflight](/mesosphere/dcos/1.9/img/dcos-gui-preflight.png)
...
#### IP Detect Script
    Choose an IP detect script from the dropdown to broadcast the IP address of each node across the cluster. Each node in a DC/OS cluster has a unique IP address that is used to communicate between nodes in the cluster. The IP detect script prints the unique IPv4 address of a node to STDOUT each time DC/OS is started on the node. For more information about IP detect scripts, see the advanced install [documentation](/mesosphere/dcos/1.9/installing/oss/custom/advanced/#ip-detect-script).
...
#### Enable Authentication
    Indicate whether to enable authentication for your DC/OS cluster. For more information, see the [documentation](/mesosphere/dcos/1.9/security/).
...
5.  Click **Run Pre-Flight**. The preflight script installs the cluster prerequisites and validates that your cluster is installable. For a list of cluster prerequisites, see the scripted installer [prerequisites][3]. This step can take up to 15 minutes to complete. If errors any errors are found, fix and then click **Retry**.
   ...
   **Important:* If you exit your GUI installation before launching DC/OS, you must do this before reinstalling:
    ...
    *   SSH to your bootstrap master node and run `rm -rf /var/lib/dcos/exhibitor/zookeeper`
    ...
    ![preflight](/mesosphere/dcos/1.9/img/dcos-gui-run-preflight.png)

6.  Click **Deploy** to install DC/OS on your cluster. If errors any errors are found, fix and then click **Retry**.

    ![deploy](/mesosphere/dcos/1.9/img/ui-installer-deploy.png)
...
7.  Click **Run Post-Flight**. If errors any errors are found, fix and then click **Retry**.
    ...
    ![postflight](/mesosphere/dcos/1.9/img/ui-installer-post-flight.png)

    **Tips:**

    *  If you encounter errors such as `Time is marked as bad`, `adjtimex`, or `Time not in sync` during Post-Flight, verify that Network Time Protocol (NTP) is enabled on all nodes. For more information, see the [system requirements](/mesosphere/dcos/1.9/installing/oss/custom/system-requirements/#port-and-protocol-configuration).
...
8.  Click **Log In To DC/OS**. If this doesn't work, take a look at the [troubleshooting docs][9].
    ...
    ![login](/mesosphere/dcos/1.9/img/dcos-gui-login.png)
...
[2]: /mesosphere/dcos/1.9/networking/
[3]: /mesosphere/dcos/1.9/installing/oss/custom/system-requirements/
[4]: /mesosphere/dcos/1.9/administering-clusters/convert-agent-type/
[5]: /mesosphere/dcos/1.9/cli/install/
[7]: /mesosphere/dcos/1.9/installing/oss/custom/uninstall/
[9]: /mesosphere/dcos/1.9/installing/oss/troubleshooting/
