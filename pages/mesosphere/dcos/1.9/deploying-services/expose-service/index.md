...
* DC/OS is [installed](/mesosphere/dcos/1.9/installing/oss/)
* DC/OS CLI is [installed](/mesosphere/dcos/1.9/cli/install/)
...
1.  Create a Marathon app definition with the required `"acceptedResourceRoles":["slave_public"]` parameter specified. For example:
    ...
    For more information about the `acceptedResourceRoles` parameter, see the Marathon API [documentation](/mesosphere/dcos/1.9/deploying-services/marathon-api/).
...
1.  Add the your app to Marathon by using this command, where `myApp.json` is your app:
     ...
     **Tip:** You can also add your app by using the **Services** tab of DC/OS [GUI](/mesosphere/dcos/1.9/gui/services/).
...
1.  Verify that the app is added with this command:
    ...
    **Tip:** You can also view deployed apps by using the **Services** tab of DC/OS [GUI](/mesosphere/dcos/1.9/gui/services/).

1.  Configure an edge load balancer and service discovery mechanism.

    - AWS users: If you installed DC/OS by using the [AWS CloudFormation templates](/mesosphere/dcos/1.9/installing/oss/cloud/aws/), an ELB is included. However, you must reconfigure the health check on the public ELB to expose the app to the port specified in your app definition (e.g. port 80).
...
1.  Go to your public agent to see the site running and to find your public agent IP. Type it into your browser.
    ...
    ![Hello Brave World](/mesosphere/dcos/1.9/img/helloworld.png)
