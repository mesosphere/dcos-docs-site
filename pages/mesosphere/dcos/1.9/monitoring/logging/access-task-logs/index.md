...
**Important:** The functionality described in this document is only available in [strict security mode](/mesosphere/dcos/1.9/security/ent/#security-modes).
...
Here is an overview of the [permissions](/mesosphere/dcos/1.9/security/ent/perms-reference/) that are required:
...
- DC/OS and DC/OS CLI are [installed](/mesosphere/dcos/1.9/installing/ent/) and you are logged in as a superuser.
...
1.  Select **Organization** and choose **Groups**.
    ...
    ![New group](/mesosphere/dcos/1.9/img/new-user-group.png)

1.  Create a new group.

    ![Prod group](/mesosphere/dcos/1.9/img/new-user-group-prod.png)

    1.  Select the group name and from the **Permissions** tab, click **ADD PERMISSION**.

        ![Add permission to prod-group](/mesosphere/dcos/1.9/img/new-user-group-prod-permission.png)
    ...
    1.  Click **INSERT PERMISSION STRING** to toggle the dialog, and then paste in the following permissions and click **ADD PERMISSIONS**.
        ...
        ![Add permission](/mesosphere/dcos/1.9/img/new-user-group-prod-permission-string.png)

        The permissions tab should now look like this:

        ![prod-group permissions complete](/mesosphere/dcos/1.9/img/new-user-group-prod-permission-done.png)

### Create the Users and Grant Permission

1.  Select **Organization** and choose **Users**. Select an existing or create a new user.

    ![New user](/mesosphere/dcos/1.9/img/new-user-generic.png)
...
1.  From the **Group Membership** tab, type in the search box and choose the group name. This will grant the group permissions to an individual user.
    ...
    ![Add alice to security group](/mesosphere/dcos/1.9/img/new-user-alice-add-group.png)
...
1.  Deploy a simple application in your group.
    ...
    1.  Select **Single Container** and define your service as:
        ...
        ![Define nested service](/mesosphere/dcos/1.9/img/new-user-alice-service-group.png)

    1.  Click **REVIEW & RUN** to complete your installation. You should now see a service that is running in a group.

        ![Service running within group](/mesosphere/dcos/1.9/img/new-user-alice-service-done.png)
...
1. Use the following command to deploy a simple application (`<service-name>`) inside of `<group-name>`.
...
1. Use the following commands to grant the permissions to the user (`<username>`). These will allow her to view the task logs in `<group-name>`.
