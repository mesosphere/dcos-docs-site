...
# Steps
  * Install the DC/OS CLI
    * Follow the steps [here](/mesosphere/dcos/1.9/cli/install/) or the `Install CLI` instruction in the lower left corner of the DC/OS GUI.
    ...
    * You can also add/invite friends and co-workers to your cluster. See [user management documentation](/mesosphere/dcos/1.9/security/ent/users-groups/) for details
  ...
  * Explore the cluster:
      ...
      * To explore more CLI options, enter the `dcos help` command. There are also help options of the individual commands available e.g., `dcos node --help`. Alternatively, check the [CLI documentation](/mesosphere/dcos/1.9/cli/).
...
## DC/OS components
Here are the DC/OS components that are relevant to this tutorial. A full description of all components can be found in the [documentation](/mesosphere/dcos/1.9/overview/architecture/components/).
* [Marathon](/mesosphere/dcos/1.9/overview/architecture/components/#marathon) starts and monitors DC/OS applications and services.
* Apache [Mesos](/mesosphere/dcos/1.9/overview/architecture/components/#apache-mesos) is the kernel of DC/OS and responsible for low-level task maintenance.
* [Mesos DNS](/mesosphere/dcos/1.9/overview/architecture/components/#mesos-dns) provides service discovery within the cluster.
* [Minuteman](/mesosphere/dcos/1.9/overview/architecture/components/#minuteman) is the internal layer 4 load balancer.
* [Admin Router](/mesosphere/dcos/1.9/overview/architecture/components/#admin-router) is an open source NGINX configuration that provides central authentication and proxy to DC/OS services.
* [Universe](/mesosphere/dcos/1.9/overview/architecture/components/#dcos-package-manager) is the package repository that holds the DC/OS services (e.g. Apache Spark or Apache Cassandra) that you can install on your cluster directly from the DC/OS GUI and CLI.
