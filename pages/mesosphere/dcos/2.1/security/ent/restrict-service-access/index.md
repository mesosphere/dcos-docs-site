...
This tutorial demonstrates how to implement user permissions for DC/OS services in the permissive [security mode](/mesosphere/dcos/2.1/security/ent/#security-modes). When you are done you will have multi-tenancy by using DC/OS permissions.

**Prerequisites:**

- DC/OS Enterprise is [installed](/mesosphere/dcos/2.1/installing/) in permissive [mode](/mesosphere/dcos/2.1/security/ent/#security-modes).
...
1.  Create service groups from the **Services > Services > Create Group**.
    ...
    ![Services Create Group](/mesosphere/dcos/2.1/img/GUI-Services-No_Services_Running-1_12.png)
    ...
    ![Group folders](/mesosphere/dcos/2.1/img/GUI-Services-List_View_Groups_Empty-1_12.png)
...
1.  Create your users and groups and define the required permissions for each group.
    ...
    1.  Select **Organization > Users** and create a new user.  In this example, two users are created (`Cory` and `Nick`).
        ...
        ![Create user Cory](/mesosphere/dcos/2.1/img/GUI-Organization-Users-Create_User_Modal-1_12.png)

        Figure 3. Creating a new user

        When you are finished, you should see the two users.

        ![All users](/mesosphere/dcos/2.1/img/GUI-Organization-Users-Users_List_View_w_Users-1_12.png)
    ...
    1.  Select **New Group**. In this example, two groups are created:
        ...
        ![prod-a group](/mesosphere/dcos/2.1/img/GUI-Organization-Groups-Create_New_Group_Modal-1_12.png)
...
1.  Add each of these permissions for the prod-a-group and click **Close**.
    ...
    ![prod-a-group](/mesosphere/dcos/2.1/img/GUI-Organization-Groups-Add_Permission_String_w_Perms-1_12.png)

    Figure 6. Adding permissions for `prod-a-group`

    Here is what the permissions view should look like after adding:

    ![prod-a-group](/mesosphere/dcos/2.1/img/GUI-Organization-Groups-Group_Detail_w_Permissions-1_12.png)
...
1.  Select **Group Membership** and then type `prod-a-group` in the search box, then click to select.
    ...
    ![prod-a-group](/mesosphere/dcos/2.1/img/GUI-Organization-Groups-User_Add_Group_Membership-1_12.png)
...
1.  Log in as Cory to the DC/OS UI. You can see that user Cory only has access to the **Services** and **{{ model.packageRepo }}** tabs. Also, Cory can only see the **prod-a** services.
  ...
  ![prod-a-group](/mesosphere/dcos/2.1/img/GUI-Services-Limited_User_Access_List-1_12.png)
...
1.  Select **JSON Configuration** and paste in the following app definition:
    ...
    ![JSON view](/mesosphere/dcos/2.1/img/GUI-Services-Add_Service_With_JSON-1_12.png)
...
1.  While logged in as Cory or Nick, click on the NGINX launch icon to view the confirmation message.
    ...
    ![NGINX](/mesosphere/dcos/2.1/img/GUI-Services-Add_Service_Review_Install-1_12.png)
...
1.  Log out of the current user and then back in as a user with [superuser](/mesosphere/dcos/2.1/security/ent/perms-reference/#superuser) permission. You will see that both services are running in the `prod-a` and `prod-b-groups`.

    ![prod-a-group](/mesosphere/dcos/2.1/img/GUI-Services-Services_List_w_Groups-1_12.png)
