...
Details about the design and implementation of DC/OS Overlay can be found in the [Overlay introduction](/mesosphere/dcos/2.1/overview/design/overlay/). The default configuration of DC/OS Overlay provides an IPv4 virtual network, `dcos`, and an IPv6 virtual network `dcos6` whose YAML configuration is as follows:
...
![Virtual network address space](/mesosphere/dcos/2.1/img/overlay-network-address-space.png)
...
DC/OS provides FQDNs in different [zones](/mesosphere/dcos/2.1/networking/DNS/) that offer a clean way of accessing services through predictable URLs. If you are using DC/OS Overlay, you should use one of the FQDNs provided by the DC/OS DNS service to make it easy for clients to discover the location of your service.
...
* [Marathon health checks](/mesosphere/dcos/2.1/deploying-services/creating-services/health-checks/) will not work with certain DC/OS Overlay configurations. If you are not using the default DC/OS Overlay configuration and Marathon is isolated from the virtual network, health checks will fail consistently even if the service is healthy.
  ...
  * You use a [`command` health check](/mesosphere/dcos/2.1/deploying-services/creating-services/health-checks/).
