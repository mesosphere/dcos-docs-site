...
- The DC/OS monitoring service (`dcos-monitoring`) can be configured to use DC/OS storage service (DSS) volumes to store time-series data. <!--(DCOS-47725)-->
    ...
    For more information about working with the DC/OS monitoring service, see [DC/OS Monitoring Service](/mesosphere/dcos/services/dcos-monitoring/). For more information about using the DC/OS storage service, see [DC/OS Storage Service](/mesosphere/dcos/services/storage/latest/).
...
For more information about how to configure logging to integrate with Fluent Bit, see [Logging](/mesosphere/dcos/1.13/monitoring/logging/). -->

For more information about collecting metrics and configuring metrics plugins, see the following topics:
- [Metrics Plugin Architecture](/mesosphere/dcos/1.13/metrics/architecture/)
- [Mesos Metrics](/mesosphere/dcos/1.13/metrics/mesos/)
- [Configuration Reference](/mesosphere/dcos/1.13/installing/production/advanced-configuration/configuration-reference/)

## Command-line interface
- Identify the public-facing IP address for public agent nodes through the DC/OS CLI. <!--(DCOS-44697)-->

    With this release, you can retrieve the public-facing IP addresses for the nodes in a cluster by running the `dcos node list` command. For more information about using the new command for retrieving public IP addresses, see the [dcos node](/mesosphere/dcos/1.13/cli/command-reference/dcos-node/) command reference.
...
- Automatically install the DC/OS Enterprise command-line interface (CLI). <!--(DCOS-39775)-->
    ...
    For more information about installing the command-line interface (CLI) and CLI plugins, see [Installing the CLI](/mesosphere/dcos/1.13/cli/install) and [Installing the DC/OS Enterprise CLI](/mesosphere/dcos/1.13/cli/enterprise-cli/).
...
- Basic auto-completion using the TAB key. <!--(DCOS-39774)-->
    ...
    For more information about using auto-completion when you are working with the command-line interface (CLI) and CLI plugins, see [Enabling autocompletion for the CLI](/mesosphere/dcos/1.13/cli/autocompletion/).
...
- Add support for internationalization and localization (I18N and L10N - Chinese). <!--(DCOS-39557)-->
    ...
    For information about changing the language displayed, see the [UI](/mesosphere/dcos/1.13/gui/) documentation.

## Installation
- Multi-region support using the Universal Installer. <!--(DCOS-45727)-->

    Multi-region deployments enable higher availability for DC/OS clusters and support for multiple regions is crucial for customers who want to maintain uptime without being susceptible for regional outages. For more information, see the documentation for [multi-region deployment](/mesosphere/dcos/1.13/installing/evaluation/aws/aws-remote-region/).
...
- Enable Universal Installer and on-premise DC/OS life cycle management with Ansible. <!--(DCOS-45724)-->
    ...
    The DC/OS Ansible (`dcos-ansible`) component is a Mesosphere-provided version of the Ansible open-source provisioning, configuration management, and deployment tool that enables you to use supported Ansible roles for installing and upgrading DC/OS Open Source and DC/OS Enterprise clusters on the infrastructure you choose. For more information, see the documentation for [Ansible](/mesosphere/dcos/1.13/installing/production/dcos-ansible/).

<!-- not in 1.13 Docs with RN filter
- Universal Installer to provision Elastic Block Store (EBS) volumes. (DCOS-47221)
    The Universal Installer provides the ability to provision Amazon Elastic Block Store (Amazon EBS) volumes and attach them to the private agents within a DC/OS cluster. For more information about deploying extra storage volumes, see [Provision Extra Agent Volumes](/mesosphere/dcos/services/storage/latest/install/provision-extra-volumes/).
-->
...
- Update the Universal Container Runtime (UCR) to support Docker registry manifest specification v2_schema2 images. <!--(DCOS-43871)-->
    ...
    For more information, see [Universal Container Runtime](/mesosphere/dcos/1.13/deploying-services/containerizers/ucr/).
...
- Add a new networking API endpoint to retrieve the public-facing IP address for public agent nodes. <!--(DCOS-28127)-->
    ...
    This release introduces a new API endpoint for accessing public-facing IP addresses for the nodes in a cluster. For more information about retrieving and viewing public IP addresses, see [Finding the public IP address](/mesosphere/dcos/1.13/administering-clusters/locate-public-agent/).
...
- Extend the DC/OS authentication architecture to apply to both DC/OS Open Source (OSS) and DC/OS Enterprise clusters. <!--(DCOS-28672)-->
    ...
    For more information about authentication and managing accounts, see [Security](/mesosphere/dcos/1.13/security) and [User account management](/mesosphere/dcos/1.13/security/oss/user-account-management/).
...
- Support secure computing mode (seccomp) profiles. <!--(DCOS-28442, DCOS-49134)-->
    ...
    For more information about secure computing mode and the default secure computing profile, see [Secure computing profiles](/mesosphere/dcos/1.13/security/oss/secure-compute-profiles/).
