...
* DC/OS is [installed](/mesosphere/dcos/1.13/installing/)
...
1.  Create a Marathon app definition with the required `"acceptedResourceRoles":["slave_public"]` parameter specified. For example:
    ...
    For more information about the `acceptedResourceRoles` parameter, see the Marathon API [documentation](/mesosphere/dcos/1.13/deploying-services/marathon-api/).
...
1.  Add your app to Marathon by using this command, where `myApp.json` is the file containing your Marathon app definition.
    ...
    If this is added successfully, there is no output. You can also add your app by using the **Services** tab of the DC/OS [UI](/mesosphere/dcos/1.13/gui/services/).
...
1.  Verify that the app is added with this command:
    ...
    You can also view deployed apps by using the **Services** tab of the DC/OS [UI](/mesosphere/dcos/1.13/gui/services/).
