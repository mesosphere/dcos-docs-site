...
Service accounts are used in conjunction with public-private key pairs, secrets, permissions, and authentication tokens to provide access for DC/OS [services](/mesosphere/dcos/1.11/overview/concepts/#dcos-service) to DC/OS. Service accounts control the communications and DC/OS API actions that the services are permitted to make.
...
-  **Authentication token** After a service connects to DC/OS with the service login token, [the IAM service](/mesosphere/dcos/1.11/overview/architecture/components/#dcos-identity-and-access-manager-bouncer) creates an **authentication token** which the service can then use to authenticate its outgoing requests to DC/OS. An authentication token can be used for long-term access.
...
![Service authentication](/mesosphere/dcos/1.11/img/authn-service.png)
