...
enterprise: true
---
This tutorial demonstrates how to implement user permissions for DC/OS services in the permissive [security mode](/mesosphere/dcos/1.10/security/ent/#security-modes). When you are done you will have multi-tenancy by using DC/OS permissions.

**Prerequisites:**

- DC/OS Enterprise is [installed](/mesosphere/dcos/1.10/installing/) in permissive [mode](/mesosphere/dcos/1.10/security/ent/#security-modes).

## Create users and groups

1.  Create service groups from the **Services > Services > Create Group**.

    ![Services Create Group](/mesosphere/dcos/1.10/img/service-group1.png)
    ...
    ![Group folders](/mesosphere/dcos/1.10/img/service-group2.png)
...
1.  Create your users and groups and define the required permissions for each group.
    ...
    1.  Select **Organization > Users** and create a new user.  In this example, two users are created (`Cory` and `Nick`).
        ...
        ![All users](/mesosphere/dcos/1.10/img/service-group4.png)
    ...
    1.  Select **New Group**. In this example, two groups are created:
        ...
        ![prod-a group](/mesosphere/dcos/1.10/img/service-group5.png)
...
1.  Add each of these permissions for the prod-a-group and click **Close**.
    ...
    ![prod-a-group](/mesosphere/dcos/1.10/img/service-group7.png)

    Here's what the permissions view should look like after adding:

    ![prod-a-group](/mesosphere/dcos/1.10/img/service-group8.png)
...
1.  Select **Group Membership** and then type `prod-agroup` in the search box, then click to select.
    ...
    ![prod-a-group](/mesosphere/dcos/1.10/img/service-group9.png)
...
1.  Log in as Cory to the DC/OS GUI. You can see that user Cory only has access to the **Services** and **Universe** tabs. Also, Cory can only see the **prod-a** services.
    ...
    **Tip:** To log out of the current user, click on the cluster name in the top-left and select **Sign Out**.
    ...
    ![prod-a-group](/mesosphere/dcos/1.10/img/service-group10.png)
...
1.  Select **Services > Services** and the click the plus sign (**+**) to deploy a service.
    ...
    1.  Select **JSON Configuration** and paste in the following app definition:
        ...
        ![JSON view](/mesosphere/dcos/1.10/img/service-group15.png)
...
1.  While logged in as Cory or Nick, click on the NGINX launch icon to view the success message.
    ...
    ![NGINX](/mesosphere/dcos/1.10/img/service-group-nginx.png)
...
1.  Log out of the current user and then back in as a user with [superuser](/mesosphere/dcos/1.10/security/ent/perms-reference/#superuser) permission. You will see that both services are running in the prod-a and prod-b-groups.

    ![prod-a-group](/mesosphere/dcos/1.10/img/service-group14.png)
