...
2.  Click on the **Organization** tab and choose your action.
    ...
    **Tip:** Any user with access to DC/OS can invite more users. Each DC/OS user is an administrator, there is no explicit concept of privileges with DC/OS.
    ...
    ![new DC/OS user](/mesosphere/dcos/1.10/img/ui-add-user.gif)
    ...
    *   To log out of the DC/OS web interface, click on your username in the top left corner and select **Sign Out**.
        ...
        ![log out](/mesosphere/dcos/1.10/img/auth-enable-logout-user.gif)
...
Authentication is only supported for DC/OS CLI version 0.4.3 and above. See [here](/mesosphere/dcos/1.10/cli/update/) for upgrade instructions.
...
1.  Copy the URL in your terminal prompt and paste it into your browser.
    ...
    ![alt](/mesosphere/dcos/1.10/img/auth-login.png)
...
1.  Provide your credentials to the identity provider if prompted. If you have already authenticated to the identity provider during your current browser session, you won't need to do so again.
    ...
    ![alt](/mesosphere/dcos/1.10/img/auth-login-token.png)
...
If you are doing an [advanced installation](/mesosphere/dcos/1.10/installing/production/deploying-dcos/installation/), you can opt out of
Auth0-based authentication by adding this parameter to your configuration file (`genconf/config.yaml`). For more information, see the configuration [documentation](/mesosphere/dcos/1.10/installing/production/advanced-configuration/configuration-reference/).

```yaml
oauth_enabled: 'false'
```

If you are doing a cloud installation on [AWS](/mesosphere/dcos/1.10/installing/evaluation/community-supported-methods/aws/), you can set the `OAuthEnabled` option to `false` on the **Specify Details** step to disable authentication.

If you are doing a cloud installation on [Azure](/mesosphere/dcos/1.10/installing/evaluation/azure/), you currently cannot disable authentication. This will be added in a future release along with other
...
- [Understand DC/OS security](/mesosphere/dcos/1.10/administering-clusters/)
- [Learn how to monitor a DC/OS cluster](/mesosphere/dcos/1.10/monitoring/)
