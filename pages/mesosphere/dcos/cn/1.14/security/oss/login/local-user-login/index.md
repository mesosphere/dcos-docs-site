...
使用 [DC/OS CLI](/mesosphere/dcos/1.14/cli/)，可以通过指定 `dcos-users` 登录提供商，以本地 DC/OS 用户的身份登录。

1. 要通过 DC/OS CLI 登录，请替换以下 [auth login](/mesosphere/dcos/1.14/cli/command-reference/dcos-auth/dcos-auth-login/) 命令中的 `uid` 和 `password`：
...
本地用户可以使用 [身份和访问管理 (IAM) API] 登录(/mesosphere/dcos/1.14/security/oss/iam-api/)。
