...
[Docker Engine](https://www.docker.com/products/docker-engine) 从 Docker 镜像启动 Docker 容器。如需要 Docker 引擎的[功能](/mesosphere/dcos/1.14/deploying-services/containerizers/#container-runtime-features)，请使用 Docker 引擎。

如果您未使用针对 Docker 引擎的特定功能，则应考虑使用 DC/OS [通用容器运行时](/mesosphere/dcos/1.14/deploying-services/containerizers/ucr/)。DC/OS [通用容器运行时](/mesosphere/dcos/1.14/deploying-services/containerizers/ucr/) 提供一些功能，如对 Docker 容器不可用的 GPU 资源支持。
...
在 [Marathon 应用定义](/mesosphere/dcos/1.14/deploying-services/creating-services/#deploying-a-simple-docker-based-application-with-the-rest-api)中，将 `container.type` 参数设置为 `DOCKER`。
...
有关示例，请参阅[部署基于 Docker 的服务](/mesosphere/dcos/1.14/deploying-services/creating-services/deploy-docker-app/)。
