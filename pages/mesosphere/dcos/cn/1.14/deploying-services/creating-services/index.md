...
1. 使用 [DC/OS CLI](/mesosphere/dcos/1.14/cli/) 将服务添加到 DC/OS。
...
1. 选择是否使用 Universal Container Runtime (UCR) 或 Docker Engine 运行时间。请参阅[使用容器化工具](/mesosphere/dcos/1.14/deploying-services/containerizers/)。
...
1. 使用 [Marathon API](/mesosphere/dcos/1.14/deploying-services/marathon-api/) 部署应用程序 `basic-3-docker`。请参阅 [验证 HTTP API 端点](/mesosphere/dcos/1.14/security/ent/iam-api/) 以了解有关如下命令中所需的 API 令牌的更多信息。
...
1. 向下滚动到 **Marathon Task Configuration** 部分，并记录 PORTS 属性。
   ...
   ![容器端口](/mesosphere/dcos/1.14/img/container-port.png)

   图 1. 容器端口
   
1. 确定[公共节点的 IP 地址](/mesosphere/dcos/1.14/administering-clusters/locate-public-agent/)。
