...
按照以下步骤向 [DC/OS Enterprise 密钥存储库] 添加 Docker 注册表凭据(/mesosphere/dcos/1.14/security/ent/secrets/)，然后在服务定义中引用该密钥。

<p class="message--important"><strong>重要信息：</strong>此功能仅适用于<a href="/mesosphere/dcos/1.14/deploying-services/containerizers/ucr/">通用容器运行时</a>。如果需要使用 Docker Engine，请遵循上述 URI 说明。</p>
...
1. 添加 `config.json` 文件到 DC/OS 密钥存储库。[了解创建密钥的详细信息](/mesosphere/dcos/1.14/security/ent/secrets/create-secrets/)。
