...
1. 使用 [自定义高级安装说明] 安装 DC/OS (/mesosphere/dcos/1.14/installing/production/advanced-configuration/)。以下是 GPU 特定的配置参数：
    ...
    有关更多信息，请参阅 [配置参数文档] (/mesosphere/dcos/1.14/installing/production/advanced-configuration/configuring-gpu-nodes/) 和 Mesos [Nvidia GPU 支持文档] （http://mesos.apache.org/documentation/latest/gpu-support/#external-dependencies）。

## 带有 GPU 的 AWS EC2 DC/OS 安装

## 先决条件
- AWS DC/OS 高级模板 [系统要求](/mesosphere/dcos/1.14/installing/evaluation/community-supported-methods/aws/advanced/template-reference/)。
- 复制到本地机器的 `zen.sh` 脚本。脚本和说明在 [此处](/mesosphere/dcos/1.14/installing/evaluation/community-supported-methods/aws/advanced/)。
...
1. 运行 `zen.sh` 脚本以创建 Zen 模板依赖关系。这些依赖关系将用作在 CloudFormation 中创建堆栈的输入信息。
   ...
   ```
    ...
    <p class="message--important"><strong></strong>重要信息：在执行后续步骤前，必须运行“zen.sh”脚本。</p>

1. 请根据 [此处](/mesosphere/dcos/1.14/installing/production/advanced-configuration/configuring-gpu-nodes/#aws-ec2-dcos-installation-with-gpus/) 说明，使用以下 GPU 专用配置创建具有高级 AWS 模板的群集。

    在 **创建堆栈** > **指定详情**页面指定您的堆栈信息并单击 **下一步**。以下是 GPU 特定设置。
...
- NVIDIA GPU 支持仅适用于使用 [DC/OS 通用容器运行时间] 启动的任务(/mesosphere/dcos/1.14/deploying-services/containerizers/)。
...
1. 服务部署完成后，检查 `stdout` 内容，验证该服务是否采用 `nvidia-smi` 命令产生正确的输出。您会看到如下内容，并且每隔 5 秒重复一次。[通过 DC/OS  CLI](/mesosphere/dcos/1.14/monitoring/logging/quickstart/) 或在 DC/OS 仪表板上的服务 **健康** 页面访问日志。
...
## 基于 Docker 的应用定义
在本示例中部署了一个具有 GPU 的应用程序，用于指定 Docker 容器和 [DC/OS 通用容器运行时间 (UCR)](/mesosphere/dcos/1.14/deploying-services/containerizers/) （容器类型为 `MESOS`）。
