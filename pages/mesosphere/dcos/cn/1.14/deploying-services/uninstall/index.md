...
1. 导航至 DC/OS Web 界面中的 [**服务**](/mesosphere/dcos/1.14/gui/services/) 选项卡。
...
如需更多信息，请参阅 [命令名称](/mesosphere/dcos/1.14/cli/command-reference/#dcos-marathon)。
...
在 DC/OS [**服务**](/mesosphere/dcos/1.14/gui/) 选项卡中，使用 JSON 编辑器将以下内容添加为 Marathon 任务。根据需要清理的内容，替换传递到 `-r`/`-z` 的值。
