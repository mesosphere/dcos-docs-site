...
要验证 API 请求，请参阅 [获取认证令牌](/mesosphere/dcos/1.14/security/ent/iam-api/#obtaining-an-authentication-token) 和 [传递认证令牌](/mesosphere/dcos/1.14/security/ent/iam-api/#passing-an-authentication-token)。
...
要为您的帐户分配权限，请参阅 [权限参考](/mesosphere/dcos/1.14/security/ent/perms-reference/)。

# 资源
Mesos-DNS 实现了一个简单的 REST API，用于通过 HTTP 进行服务发现。这些示例假设您具有 [与节点的 SSH 连接](/mesosphere/dcos/1.14/administering-clusters/sshcluster/)。
...
<p class="message--note"><strong>注意：</strong>HTTP 界面仅解析 Mesos 域中的主机名。</p>
