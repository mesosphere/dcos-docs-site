...
您可以使用 [dcos cluster](/mesosphere/dcos/1.14/cli/command-reference/dcos-cluster/) 和 [dcos confi](/mesosphere/dcos/1.14/cli/command-reference/dcos-config/) 命令组访问 DC/OS CLI 配置。
...
要设置 [连接的群集](/mesosphere/dcos/1.14/cli/command-reference/dcos-cluster/dcos-cluster-attach/)，请使用以下命令设置变量：
...
* 为您不想使用代理的域定义 `no_proxy`：
   ...
   此设置根据 `$DCOS_DIR/clusters/<cluster_id>` 下的群集配置生成和更新。生成新设置群集 [如此处所示](/mesosphere/dcos/1.14/cli/#setupcluster)。
