...
- 要更新到较新的主要或次要版本（例如 1.11 到 1.12），请参阅 [升级](/mesosphere/dcos/1.14/installing/production/upgrading/) 的说明。
...
- 在修补 DC/OS 之前，请先查看 [版本注释](/mesosphere/dcos/1.14/release-notes/)。
...
有关不同安全模式的更多信息，请参阅安全 [模式](/mesosphere/dcos/1.14/installing/production/advanced-configuration/configuration-reference/#security-enterprise)。
...
- 所有主机（管理和代理节点）必须能够与所有其他主机通信，如 [网络安全](/mesosphere/dcos/1.14/administering-clusters/securing-your-cluster/#network-security) 所述。
...
- 您必须熟悉 DC/OS [生产安装](/mesosphere/dcos/1.14/installing/production/deploying-dcos/installation/) 说明。
- 修补之前要对 ZooKeeper 进行实时捕捉。Marathon 支持回滚，但不支持降级。
- *重要信息：*升级之前 [对 IAM 数据库进行快照](/mesosphere/dcos/1.14/installing/installation-faq/#q-how-do-i-backup-the-iam-database-enterprise)。这很容易做到，应该被视为必要的。
...
- [备份您的群集](/mesosphere/dcos/1.14/administering-clusters/backup-and-restore/)。
- **可选** 您可以将自定义 [节点和群集运行状况检查](/mesosphere/dcos/1.14/installing/production/deploying-dcos/node-cluster-health-check/#custom-health-checks) 添加到 `config.yaml`。
...
该程序修补 DC/OS 1.12 群集而不更改群集的 [安全模式](/mesosphere/dcos/1.14/installing/production/advanced-configuration/configuration-reference/#security-enterprise)。
...
1. 将旧的 `config.yaml` 合并为新的 `config.yaml` 格式。大多数情况下，区别会很小。
    ...
    <p class="message--note"><strong>注意：</strong>在修补期间，您无法更改 <code>exhibitor_zk_backend</code> 设置。<code>config.yaml</code> 的语法可能与早期版本不同。有关当前 <code>config.yaml</code> 语法和参数的详细说明，请参阅 <a href="/mesosphere/dcos/1.14/installing/production/advanced-configuration/configuration-reference/">文档</a>。</p>
...
1. 构建安装工具包。
    ...
    1. 运行 [nginx](/mesosphere/dcos/1.14/installing/production/deploying-dcos/installation/) 容器以为安装文件提供服务。

1. 转到 DC/OS 管理节点 [程序](/mesosphere/dcos/1.14/installing/production/patching/#masters) 完成安装。

# <a name="strict"></a>在严格模式下修补 DC/OS 1.12
该程序的在安全性严格 [安全模式](/mesosphere/dcos/1.14/installing/production/advanced-configuration/configuration-reference/#security-enterprise) 下修补到 DC/OS 1.12。
...
1. 构建安装工具包。
    ...
    1. 生成安装文件。将以下命令中的 `<installed_cluster_version>` 替换为当前要修补的群集上正在运行的 DC/OS 版本，例如 `1.8.8`。
        ...
        ```
    1. 上一步骤的命令将在其输出的最后一行产生 URL，前缀为 `Node patch script URL:`。记下此 URL 以供后续使用。它在本文档中被称为“节点补丁脚本 URL”。
    2. 运行 [nginx](/mesosphere/dcos/1.14/installing/production/deploying-dcos/installation/) 容器以为安装文件提供服务。
...
1. 验证补丁：
    ...
    1. 验证 `/opt/mesosphere/bin/mesos-master --version` 表示修补后的管理节点正在运行 [发行说明](/mesosphere/dcos/1.14/release-notes/) 中指定的 Mesos 版本，例如 `1.5.1`。
...
如果您的修补因为 [自定义节点或群集检查](/mesosphere/dcos/1.14/installing/production/deploying-dcos/node-cluster-health-check/#custom-health-checks) 而失败，运行以下命令可了解更多详细信息：
```bash
