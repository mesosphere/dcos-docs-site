...
2. SSH 至群集。
    ...
    **要 SSH 至代理节点：**
    ...
    要查找代理 ID，请选择 DC/OS [UI](/mesosphere/dcos/1.14/gui/) 中的 **节点** 选项卡并单击 **详细信息**。

    ![Web 界面节点 ID](/mesosphere/dcos/1.14/img/GUI-Nodes-Selected_Node_Detail_View-1_12.png)
...
2. SSH 至群集。
    ...
    * **要 SSH 至管理节点：**
        ...
        2. 打开 PuTTY 并在 **主机名（或 IP 地址）** 字段中输入管理节点 IP 地址。
            ...
            ![Putty 配置](/mesosphere/dcos/1.14/img/windowsputtybasic.png)

            图 3. PuTTY 配置

        3. 在 PuTTY 窗口左侧的 **类别** 窗格中，选择 **连接 > SSH > Auth**，单击 **浏览**，查找并选择您的 `.ppk` 文件，然后单击 **打开**。

            ![Putty SSH 选项](/mesosphere/dcos/1.14/img/windowsputtysshopt.png)

            图 4. PuTTY SSH 选项

        4. 如果您正在运行 CoreOS，则以“core”用户登录。CenTos 上的默认用户是“centos”。

            ![Windows 登录](/mesosphere/dcos/1.14/img/windowscore.png)
        ...
        **先决条件：** 您必须退出管理节点。
        ...
        1. 启用 PuTTY 中的代理转发。
            ...
            2. 单击 **浏览** 按钮并找到您之前使用 PuTTYgen 创建的 `.ppk` 文件。
                ...
                ![Windows 转发](/mesosphere/dcos/1.14/img/windowsforwarding.png)
