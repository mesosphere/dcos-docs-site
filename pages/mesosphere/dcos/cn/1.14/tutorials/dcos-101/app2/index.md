...
# 先决条件
* [正在运行的 DC/OS 群集](/mesosphere/dcos/1.14/tutorials/dcos-101/cli/)，[已安装 DC/OS CLI](/mesosphere/dcos/1.14/tutorials/dcos-101/cli/)。
* [app1](/mesosphere/dcos/1.14/tutorials/dcos-101/app1/) 已部署并在您的群集中运行。


# 目的
在本教程[前述部分](/mesosphere/dcos/1.14/tutorials/dcos-101/app1/)中，您部署了在群集内部运行的应用程序，与群集中的其他应用程序连接，而不是在外部进行交互。在此部分中，您将部署为用户提供 GUI 的应用程序。您还将本地部署此应用程序，而不依赖于 Docker 作为依赖项，从而降低复杂性。
...
DC/OS 使用[容器化工具](/mesosphere/dcos/1.14/deploying-services/containerizers/)在容器中运行任务。容器中的运行任务提供了许多好处，包括将任务彼此隔离并以编程方式控制任务资源的能力。DC/OS 支持两种类型的容器化工具 - DC/OS Universal Container Runtime 和 Docker 容器化工具。
...
这实际上可以使用 DC/OS Universal Container Runtime 来运行 Docker 镜像 - 有关详细信息，请查看 [DC/OS Universal Container Runtime](/mesosphere/dcos/1.14/deploying-services/containerizers/) 文档。
