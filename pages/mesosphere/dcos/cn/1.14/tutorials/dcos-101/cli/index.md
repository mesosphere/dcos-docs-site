...
# 步骤
  * 安装 DC/OS CLI
    ...
    * 您还可以向群集添加/邀请朋友和同事。有关详细信息，请参阅[用户管理文档](/mesosphere/dcos/1.14/security/ent/users-groups/)
  ...
  * 查看群集：
      ...
      * 要查看更多 CLI 选项，请输入 `dcos help` 命令。还有可用的各个命令的帮助选项，例如，`dcos node --help`。或者，查看 [CLI 文档](/mesosphere/dcos/1.14/cli/)。
...
## DC/OS 组件
以下是与本教程相关的 DC/OS 组件。有关所有组件的完整说明，请参阅[文档](/mesosphere/dcos/1.14/overview/architecture/components/)。
* [Marathon](/mesosphere/dcos/1.14/overview/architecture/components/#marathon) 启动并监控 DC/OS 应用程序和服务。
* Apache [Mesos](/mesosphere/dcos/1.14/overview/architecture/components/#apache-mesos) 是 DC/OS 的核心，负责低级别任务维护。
* [Mesos DNS](/mesosphere/dcos/1.14/overview/architecture/components/#mesos-dns) 在群集内提供服务发现。
* [Minuteman](/mesosphere/dcos/1.14/overview/architecture/components/#minuteman) 是内部第 4 层负载均衡器。
* [Admin Router](/mesosphere/dcos/1.14/overview/architecture/components/#admin-router) 是一种开源 NGINX 配置，为 DC/OS 服务提供集中身份认证和代理。
* [{{ model.packageRepo }}](/mesosphere/dcos/1.14/overview/architecture/components/#dcos-package-manager) 是包含 DC/OS 服务（例如，Apache Spark 或 Apache Cassandra）的包资源库，您可以直接从 DC/OS GUI 和 CLI 在群集上安装这些服务。
