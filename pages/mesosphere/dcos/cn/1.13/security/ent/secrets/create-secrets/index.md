...
有关如何在应用程序或 pod 定义中引用这些密钥的信息，请参阅[配置服务和 pod 以使用密钥](/mesosphere/dcos/cn/1.13/security/ent/secrets/use-secrets/)
...
密钥应包括路径，除非您希望允许所有服务访问其值。有关密钥路径的更多信息，请参阅[空间](/mesosphere/dcos/cn/1.13/security/ent/#spaces)
...
- 有关从 CLI 或 API 创建密钥所需的权限，请参阅[密钥存储库权限](/mesosphere/dcos/cn/1.13/security/ent/perms-reference/#secrets) 您配置的权限必须包括允许用户创建的密钥名称。每个密钥必须拥有一项权限。密钥名称和权限名称必须匹配。
...
1. 在 **ID** 框中，提供密钥名称及其路径（如果有）。
    ...
    ![密钥 ID 键对](/mesosphere/dcos/cn/1.13/img/GUI-Secrets-Create-New-Keypair.png)
...
<p class="message--note"><strong>注意：</strong>在发出本部分中的 curl 命令之前，必须遵循 <a href="/mesosphere/dcos/1.13/security/ent/tls-ssl/get-cert/">获取 DC/OS CA 捆绑包</a> 中的步骤。</p>
...
1. 使用 `dcos auth login` 登录到 CLI。您可在 [CLI 参考] 中找到有关此命令的更多信息。(/mesosphere/dcos/cn/1.13/cli/command-reference/dcos-auth/dcos-auth-login/).
...
1. 返回到密匙屏幕，可以看到密匙已经部署。
   ...
   ![已部署密匙](/mesosphere/dcos/cn/1.13/img/GUI-Secrets-Deployed.jpeg)
   图 7 - 已部署密钥
