...
本教程演示如何在宽容[安全模式](/mesosphere/dcos/1.13/security/ent/#security-modes)下实现 DC/OS 服务的用户权限。完成后，您将通过使用 DC/OS 权限获得多租户。

**前提条件：**

- DC/OS Enterprise [已安装](/mesosphere/dcos/1.13/installing/)在宽容[模式](/mesosphere/dcos/1.13/security/ent/#security-modes)下。
...
1. 注销当前用户，然后以具有[超级用户](/mesosphere/dcos/1.13/security/ent/perms-reference/#superuser)权限的用户身份返回。您将看到，两个服务都在 `prod-a` 和 `prod-b-groups` 中运行。
