...
若要获取认证令牌，请将请求正文中具有必要权限的用户的用户名和密码传递给[身份和访问管理服务 API]`/auth/login` 的 (/mesosphere/dcos/cn/1.13/security/ent/iam-api/). 端点。它将返回认证令牌，如下所示。
...
默认情况下，认证令牌在五天后过期。如果您的程序需要运行超过五天，则需要一个服务帐户。有关更多信息，请参阅 [调配自定义服务](/mesosphere/dcos/cn/1.13/security/ent/service-auth/custom-service-auth/)
