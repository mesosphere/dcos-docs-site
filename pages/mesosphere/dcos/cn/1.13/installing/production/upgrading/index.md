...
- 要更新到较新的维护版本（例如 1.12.1 到 1.12.2），请参阅 [修补](/mesosphere/dcos/1.13/installing/production/patching/) 的说明。
- 要修改群集配置，请参阅 [修补](/mesosphere/dcos/1.13/installing/production/patching/) 的说明。
...
- 生产安装方法是 DC/OS 的唯一建议升级路径。建议您熟悉 [DC/OS 部署指南](/mesosphere/dcos/1.13/installing/production/deploying-dcos/) 后再继续操作。
- 在升级 DC/OS 之前，请先查看 [版本注释](/mesosphere/dcos/1.13/release-notes/)。
...
_不能_ 在升级到新版本的同时更改群集配置。必须通过对已安装版本的修补进行群集配置更改。例如，您无法同时将群集从 1.11 升级到 1.12 并添加更多公共代理节点。您可以修补到 1.12，然后升级到 1.13 来添加更多公共代理节点，或者可以升级到 1.13，然后在升级后通过 [修补 1.13](/mesosphere/dcos/1.13/installing/production/patching/) 来添加更多公共代理节点。
...
- 升级之前抓取 [ZooKeeper 的快照](/mesosphere/dcos/1.13/installing/installation-faq/#q-how-do-i-backup-zookeeper-using-guano)。Marathon 支持回滚，但不支持降级。
...
- [备份您的群集](/mesosphere/dcos/1.13/administering-clusters/backup-and-restore/)。[enterprise type="inline" size="small" /]
- 可选：您可以将自定义 [节点和群集健康检查](/mesosphere/dcos/1.13/installing/production/deploying-dcos/node-cluster-health-check/) 添加到 `config.yaml`。
- 确认所有管理节点都处于运行良好状态：
...
2. 将旧的 `config.yaml` 合并为新的 `config.yaml` 格式。大多数情况下，区别会很小。
    ...
    * `config.yaml` 文件的语法可能与早期版本不同。有关当前 `config.yaml` 语法和参数的详细说明，请参阅 [文档](/mesosphere/dcos/1.13/installing/production/advanced-configuration/configuration-reference/)。
...
1. 复制并更新 DC/OS 1.12 `config.yaml` 和 `ip-detect` 文件到 bootstrap 节点上新的干净文件夹。
    ...
    * 在升级期间，您无法更改 `exhibitor_zk_backend` 设置。
    * DC/OS 1.12 `config.yaml` 文件的语法不同于先前版本。有关当前 `config.yaml` 语法和参数的详细说明，请参阅 [高级配置参考](/mesosphere/dcos/1.13/installing/production/advanced-configuration/configuration-reference/)。
...
1. 验证升级情况：
    ...
    1. 验证 `/opt/mesosphere/bin/mesos-master --version` 表示升级后的管理节点正在运行 [发行说明](/mesosphere/dcos/1.13/release-notes/) 中指定的 Mesos 版本，例如 `1.5.1`。
...
如果您的升级因为 [自定义节点或群集检查] 而失败(/mesosphere/dcos/1.13/installing/production/deploying-dcos/node-cluster-health-check/)，运行以下命令可了解更多详细信息：
```bash
