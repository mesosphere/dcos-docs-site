...
1. 使用 AWS CloudFormation Web 控制台启动 DC/OS 高级模板，并指定 DC/OS cloud_images AMI。验证群集是否已成功启动。如需更多信息，请参阅 [文档](/mesosphere/dcos/1.13/installing/evaluation/community-supported-methods/aws/)。
...
1. 按照 [本说明] 完成安装 (/mesosphere/dcos/1.13/installing/evaluation/community-supported-methods/aws/)。

## 故障排除

- 熟悉 DC/OS 服务启动 [流程](/mesosphere/dcos/1.13/overview/architecture/boot-sequence/)。
- 查看安装故障排除 [文档](/mesosphere/dcos/1.13/installing/troubleshooting/)。要进行故障排除，必须有所有群集节点的 [SSH 访问权限](/mesosphere/dcos/1.13/administering-clusters/sshcluster/)。
