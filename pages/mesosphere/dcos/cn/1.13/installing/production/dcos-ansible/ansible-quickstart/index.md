...
## 前提条件和设置
1. 1. 您需要安装 `2.9.0` 或以上版本的 [Ansible](https://docs.ansible.com/ansible/latest/installation_guide/intro_installation.html)。有关更多安装信息，请参考链接。
     使用 Mac，您可以 brew 安装：
     ```bash
...
请参阅 [最低系统要求](/mesosphere/dcos/cn/1.13/installing/production/system-requirements/) 以及支持的 CentOS 和 Redhat Enterprise Linux (RHEL) [您的节点的版本](/mesosphere/dcos/cn/version-policy/#dcos-platform-version-compatibility-matrix/)，尤其是在针对生产环境进行计划时。
...
## 下载内容
我们维护并托管 [Ansible Galaxy 上的 DC/OS Ansible 项目](https://galaxy.ansible.com/dcos/cluster) 以便消耗。目前，我们以及 RedHat 都建议使用 `ansible-galaxy` 工具将所有必要内容本地下载到您的机器上。有关 `ansible-galaxy` 工具和如何配置的更多信息，请参阅 [此处的项目文档](https://docs.ansible.com/ansible/latest/cli/ansible-galaxy.html)。
...
1. 在 `~/.ansible/collections/ansible_collections/dcos/cluster` 目录中，将清单和组变量示例文件重命名为新文件：
     ```bash
...
4. 同样在变量文件中，在  下设置以下值。`config`. Enterprise 用户在此添加您的许可证密钥。（可选：在 `ip_detect_contents` 或 `ip_detect_public_contents` 中为您的环境指定特殊脚本，您可以在此处找到默认值 [ip-detect](https://github.com/dcos/dcos-ansible/blob/master/roles/dcos_bootstrap/templates/onprem/ip-detect.j2) [ip-detect-public](https://github.com/dcos/dcos-ansible/blob/master/roles/dcos_bootstrap/templates/onprem/ip-detect-public.j2)):
    ```yaml
    # ...
    ...
    # more info about ip-detect can be found here: /mesosphere/dcos/1.13/installing/production/deploying-dcos/installation/#create-an-ip-detection-script
    # ip_detect_contents: |
    #   #!/bin/sh
...
![enterprise-Login-Page](/mesosphere/dcos/cn/1.13/img/dcos-ee-login.png)



如果您安装了 DC/OS 开源，请选择所需的 OAuth 提供程序。[oss type="inline" size="small" /]

![oss-login-page](/mesosphere/dcos/cn/1.13/img/dcos-oe-login.png)
