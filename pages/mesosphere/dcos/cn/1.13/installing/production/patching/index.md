...
- 要更新到较新的主要或次要版本（例如 1.11 到 1.13），请参阅 [升级](/mesosphere/dcos/1.13/installing/production/upgrading/) 的说明。
...
- 在修补 DC/OS 之前，请先查看 [版本注释](/mesosphere/dcos/1.13/release-notes/)。
...
有关不同安全模式的更多信息，请参阅安全 [模式](/mesosphere/dcos/1.13/installing/production/advanced-configuration/configuration-reference/#security-enterprise)。
...
- 所有主机（管理和代理节点）必须能够与所有其他主机通信，如 [网络安全](/mesosphere/dcos/1.13/administering-clusters/securing-your-cluster/#network-security) 所述。
...
- 您必须熟悉 DC/OS [生产安装](/mesosphere/dcos/1.13/installing/production/deploying-dcos/installation/) 说明。
- 修补之前要对 ZooKeeper 进行实时捕捉。Marathon 支持回滚，但不支持降级。
- *重要信息：*升级之前 [对 IAM 数据库进行快照](/mesosphere/dcos/1.13/installing/installation-faq/#q-how-do-i-backup-the-iam-database-enterprise)。这很容易做到，应该被视为必要的。
...
- [备份您的群集](/mesosphere/dcos/1.13/administering-clusters/backup-and-restore/)。
- **可选** 您可以将自定义 [节点和群集运行状况检查](/mesosphere/dcos/1.13/installing/production/deploying-dcos/node-cluster-health-check/#custom-health-checks) 添加到 `config.yaml`。
...
该程序修补 DC/OS 1.13 群集而不更改群集的 [安全模式](/mesosphere/dcos/1.13/installing/production/advanced-configuration/configuration-reference/#security-enterprise)。
...
1. 构建安装工具包。
    ...
    1. 运行 [nginx](/mesosphere/dcos/1.13/installing/production/deploying-dcos/installation/) 容器以为安装文件提供服务。

1. 转到 DC/OS 管理节点 [程序](/mesosphere/dcos/1.13/installing/production/patching/#masters) 完成安装。

# <a name="strict"></a>在严格模式下修补 DC/OS 1.13
该程序的在安全性严格 [安全模式](/mesosphere/dcos/1.13/installing/production/advanced-configuration/configuration-reference/#security-enterprise) 下修补到 DC/OS 1.13。
...
1. 验证补丁：
    ...
    1. 验证 `/opt/mesosphere/bin/mesos-master --version` 表示修补后的管理节点正在运行 [发行说明](/mesosphere/dcos/1.13/release-notes/) 中指定的 Mesos 版本，例如 `1.5.1`。
...
如果您的修补因为 [自定义节点或群集检查](/mesosphere/dcos/1.13/installing/production/deploying-dcos/node-cluster-health-check/#custom-health-checks) 而失败，运行以下命令可了解更多详细信息：
```bash
