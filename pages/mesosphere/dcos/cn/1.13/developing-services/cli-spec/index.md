...
<p class="message--important"><strong>重要信息：</strong>CLI 模块<a href="/mesosphere/dcos/cn/1.13/administering-clusters/multiple-clusters/">特定于群集</a>，并存储在 <code>~/.dcos/clusters/"cluster_id"/subcommands</code> 中。必须为每个群集安装 CLI 模块。例如，如果连接到群集 1 并安装 Spark 模块，则连接到也运行 Spark 的群集 2。在安装该群集的模块之前，Spark CLI 命令不可用。</p>
