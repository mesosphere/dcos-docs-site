...
也可以 [在无法访问任务时配置 Marathon 的行为](/mesosphere/dcos/1.13/deploying-services/task-handling/configure-task-handling/)。
...
鉴于代理被视为已消失且无法提供这些卷，如果任务配置为使用 [本地持久卷](/mesosphere/dcos/1.13/storage/persistent-volume)，这些将被放弃。将创建一个新任务作为替代，并使用新卷。
