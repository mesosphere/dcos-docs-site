...
以下 pod 定义指定名为  的虚拟（用户）网络。`dcos`. `networks:mode:container` 字段创建虚拟网络。`name` 字段为可选。如果您已使用 [我们的 AWS 模板](/mesosphere/dcos/cn/1.13/installing/evaluation/community-supported-methods/aws/) 安装 DC/OS，则默认虚拟网络名称为 `dcos`. <!-- Validated by suzanne 6-23-17 --> 
