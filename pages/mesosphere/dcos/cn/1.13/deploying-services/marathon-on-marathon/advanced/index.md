...
本专题描述了如何部署具有独立角色、保留、配额和安全功能的非本地 Marathon 实例  (Marathon on Marathon) 。高级非本地 Marathon 程序只能在您需要 [密钥](/mesosphere/dcos/cn/1.13/security/ent/secrets/) 或细粒度 ACL 时使用。否则，请使用 [基本程序](/mesosphere/dcos/cn/1.13/deploying-services/marathon-on-marathon/basic/)。
...
- DC/OS 和 DC/OS CLI [已安装](/mesosphere/dcos/cn/1.13/installing/)。
- [DC/OS Enterprise CLI 0.4.14 或更高版本](/mesosphere/dcos/cn/1.13/cli/enterprise-cli/#ent-cli-install)。
- 每个专用 DC/OS 代理可以通过网络访问的专用 Docker 注册表。可以遵循 [以下](/mesosphere/dcos/cn/1.13/deploying-services/private-docker-registry/) 关于如何在 Marathon 中设置，或使用其他选项的说明（如 [DockerHub](https://hub.docker.com/)、[Amazon EC2 容器注册表](https://aws.amazon.com/ecr/)和 [Quay](https://quay.io/)）。
- 自定义非本机 Marathon 镜像[部署在您的专用 Docker 注册表中](/mesosphere/dcos/cn/1.13/deploying-services/private-docker-registry#tarball-instructions)。通过 [支持门户](https://support.mesosphere.com) 提交故障单以获取企业 Marathon 镜像文件。
- 您必须以超级用户身份登录。
- 对群集的 SSH 访问。
...
| `${SERVICE_ACCOUNT}` | Marathon 用来和 DC/OS 中的其他服务进行通信的 [服务帐户](/mesosphere/dcos/cn/1.13/security/ent/service-auth/) 的名称。名称应仅包含字母、数字、`@`、`.`、`\`、`_` 和 `-`。例如 `"marathon_user_ee"` |
...
| `${SERVICE_ACCOUNT_SECRET}` | [密钥存储库](/mesosphere/dcos/cn/1.13/security/ent/secrets/)中的密钥路径，用于保存 Marathon 将使用的私钥以及要在 DC/OS 上进行身份认证的 `${SERVICE_ACCOUNT}` 帐户。此名称 **不得**包含一个领导的 `/`。有效示例： `"marathon_user_ee_secret"` |
| `${DOCKER_REGISTRY_SECRET}` | [密钥](/mesosphere/dcos/cn/1.13/security/ent/secrets/)的名称，用于保存从专用注册表中获取 Marathon Docker 镜像的凭据。此名称 **不得**包含一个领导的 `/`。有效示例： `"registry_secret"`。 |
...
1. 将 Marathon 企业镜像推送到您的专用注册表[（说明）](/mesosphere/dcos/cn/1.13/deploying-services/private-docker-registry#tarball-instructions)，名称为 `${MARATHON_IMAGE}:${MARATHON_TAG}`。
1. 将您的专用 Docker 凭据存储在密钥存储库 [（说明）](/mesosphere/dcos/cn/1.13/deploying-services/private-docker-registry/#referencing-private-docker-registry-credentials-in-the-secrets-store-enterprise)中，名称为 `${DOCKER_REGISTRY_SECRET}`。
...
本步骤创建了 Marathon [服务账户](/mesosphere/dcos/cn/1.13/security/ent/service-auth/) 。Marathon 将使用此帐户对其余 DC/OS 组件进行身份认证。稍后授予此帐户的权限将定义允许 Marathon 执行的操作。

Marathon 服务账户可能是可选或必填项，具体取决于您的 [安全模式](/mesosphere/dcos/cn/1.13/security/ent/#security-modes)。
...
1. 创建 2048 位 RSA 公私密钥对 (`${PRIVATE_KEY}` 和 `${PUBLIC_KEY}`) 并将值分别保存到当前目录中的单独文件中。
    ...
    使用以下命令，我们创建一个公私密钥对。公钥将用于创建 Marathon 服务帐户。我们将私钥存储在 [密钥存储库](/mesosphere/dcos/cn/1.13/security/ent/secrets/) 然后将其传递至 Marathon，供其进行自我授权。
...
所有 CLI 命令也可通过 [IAM API](/mesosphere/dcos/cn/1.13/security/ent/iam-api/) 执行。

授予服务帐户 `${SERVICE_ACCOUNT}` 权限，以启动将作为 Linux 用户 `nobody` 执行的 Mesos 任务。

要作为不同 Linux 用户执行任务，请替换 `nobody` 为该用户的 Linux 用户名。例如，如需以 Linux 用户 `bob` 身份启动任务，请替换 `nobody` 为以下的 `bob`。

请注意，`nobody` 和 `root` 用户是默认出现在所有代理上的；如果指定自定义用户 (例如，`bob`)，那么就需要在可以执行这些任务的每个代理上手动创建该用户 `bob`（例如，使用 Linux `adduser` 或类似实用程序）。
...
1. 创建一个自定义 JSON 配置，用于安装自定义非本机 Marathon 实例。JSON 文件内容因您的 [安全模式](/mesosphere/dcos/cn/1.13/security/ent/#security-modes) 而有所不同。
...
1. 输入您的用户名和密码，然后单击 **登录**。
    ...
    ![Marathon on Marathon](/mesosphere/dcos/1.13/img/mom-marathon-gui.png)
