...
DC/OS 代理节点在 [安装](/mesosphere/dcos/cn/1.13/installing/)过程中可被指定为[公共](/mesosphere/dcos/cn/1.13/overview/concepts/#public-agent-node) 或[专用](/mesosphere/dcos/cn/1.13/overview/concepts/#private-agent-node)。公共代理节点通过基础架构网络从群集外部访问 DC/OS 服务。默认情况下，服务在专用代理节点上启动，不可从群集外部访问。
...
1. 使用此命令验证应用程序是否已添加：
    ...
    您还可以使用 DC/OS [UI](/mesosphere/dcos/cn/1.13/gui/services/) 的 **服务** 选项卡查看已部署的应用程序 。
...
1. 配置边缘负载均衡器和服务发现机制。
    ...
    - 所有其他用户：您可以使用 [Marathon-LB](/mesosphere/dcos/cn/1.13/networking/marathon-lb/)，一种基于 HAProxy 的快速代理程序和负载均衡器。
