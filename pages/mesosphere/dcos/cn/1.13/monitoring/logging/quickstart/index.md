...
使用本指南开始使用 DC/OS 日志记录。此处使用的许多命令在 [CLI 命令参考] 中有更全面的描述(/mesosphere/dcos/1.13/cli/command-reference/dcos-task/)。
...
您可以使用 `dcos node log` 命令，查看任务或主机系统的日志。关于这些命令的更多信息可在 [CLI 命令参考] (/mesosphere/dcos/1.13/cli/command-reference/dcos-node/) 部分找到。
