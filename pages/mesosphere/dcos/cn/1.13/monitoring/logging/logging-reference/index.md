...
在 1.11 之前的 DC/OS 版本中，节点和组件日志由  管理。`journald`. 不过，由于 [journald 性能问题]，[Mesos 任务 journald 日志槽已被禁用]。(/mesosphere/dcos/cn/1.13/installing/production/advanced-configuration/configuration-reference/#mesos-container-log-sink). 因此，只能通过 [Mesos 任务沙盒文件 API](http://mesos.apache.org/documentation/latest/sandbox/) 来访问旧版本的容器日志文件。
