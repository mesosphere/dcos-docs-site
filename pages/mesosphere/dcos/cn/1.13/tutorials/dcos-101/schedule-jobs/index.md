...
1. 单击 **计划**，然后单击 **启用计划**，指定作业运行计划。
    ...
    - 选择 **CRON 计划**，以指定 `cron` 格式的计划。使用 [this crontab generator](http://crontab.gur)u获取帮助。您还可以设置 **时区** 以应用于 cron 计划。例如，您可能在不同时区中有节点，并希望使用标准化 UTC 时间或特定本地时区（如 America/New_York）运行作业。
