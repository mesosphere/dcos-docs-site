...
默认情况下，REX-Ray 将在连接 13 个 EBS 卷后失败。使用 [config 选项 useLargeDeviceRange 来扩展此限制][13]，这在 RexRay v0.11.4 中已被采用。

在某些较新的 EC2 实例类型上，EBS 卷作为非易失性内存主机控制器接口规范 (NVMe) 设备存在。对 NVME 的支持仅添加到 v0.11.4 版的 Rexray 中。您将需要采取以下先决步骤，才能使 RexRay 与 CentOS 上的 NVMe 设备一起使用（在较新的 CoreOS AMI 上，则不必要）：
...
有关外部卷的故障排除，请参阅代理或系统日志。如果您在 DC/OS 上使用 REX-Ray，还可以查阅 `systemd` 日志中的 `dcos-rexray.service` 单元记录。
...
[6]: /mesosphere/dcos/cn/1.13/deploying-services/creating-services/
