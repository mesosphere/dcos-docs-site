...
# <a name="core"></a>核心遥测
[DC/OS 信号](/mesosphere/dcos/cn/1.13/overview/architecture/components/#dcos-signal) 组件查询领导管理节点上的诊断服务 `/system/health/v1/report` 端点，并将此数据发送给 [区块](https://segment.com/docs/)，Mesosphere 然后用它来跟踪使用率度量标准和客户支持。
...
此信息从 [DC/OS 诊断](/mesosphere/dcos/cn/1.13/overview/architecture/components/#dcos-diagnostics) 组件收集。对于每个 `systemd` 单元，将收集以下信息，其中 `<UNIT_NAME>` 是组件名称：
...
## Mesos
此信息从 [Apache Mesos](/mesosphere/dcos/cn/1.13/overview/architecture/components/#apache-mesos) 组件收集。
...
## 包管理器
此信息从 [DC/OS 包管理器 (Cosmos) ](/mesosphere/dcos/cn/1.13/overview/architecture/components/#dcos-package-manager) 组件收集。
...
您也可选择退出遥测功能。如需更多信息，请参阅 [退出](/mesosphere/dcos/cn/1.13/installing/production/deploying-dcos/opt-out/)
