...
Mesos 输入插件由名为 `config.yaml` 的 `enable_mesos_input_plugin`. 文件中的选项控制。要禁用插件，`enable_mesos_input_plugin` 需要设置为 `false`. 有关如何为本地安装创建配置文件的说明，请参见 [此处](/mesosphere/dcos/cn/1.13/installing/production/deploying-dcos/installation/#create-a-configuration-file). 要修改现有本地群集上的配置文件，必须 [修补现有 DC/OS 版本](/mesosphere/dcos/cn/1.13/installing/production/patching/#modifying-dcos-configuration). 对于云安装，可以在 [此处]找到每个受支持的云提供商的配置和安装说明。(/mesosphere/dcos/cn/1.13/installing/evaluation/).
