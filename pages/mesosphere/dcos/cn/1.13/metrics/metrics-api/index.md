...
度量标准 API 由 [Telegraf] 支持(/mesosphere/dcos/1.13/overview/architecture/components/#telegraf)，后者在群集中的所有节点上运行。

要开始使用 DC/OS 度量标准组件以及了解如何使用度量 API，请参阅 [度量快速入门指南](/mesosphere/dcos/1.13/metrics/quickstart/)。
...
要确定群集的 URL，请参阅 [群集访问](/mesosphere/dcos/1.13/api/access/)。节点的代理 ID 为其 Mesos ID。
...
要验证 API 请求，请参阅 [获取认证令牌](/mesosphere/dcos/1.13/security/ent/iam-api/#obtaining-an-authentication-token) 和 [传递认证令牌](/mesosphere/dcos/1.13/security/ent/iam-api/#passing-an-authentication-token) 文档。
...
要为您的帐户分配权限，请参阅 [权限参考](/mesosphere/dcos/1.13/security/ent/perms-reference/) 文档。
