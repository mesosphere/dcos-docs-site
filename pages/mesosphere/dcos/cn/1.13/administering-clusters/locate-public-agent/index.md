...
1. 选中 **公共 IP** 列，确定要公开的代理节点面向公众的 IP 地址。
    ...
    在大多数情况下，查找代理节点面向公众的 IP 地址就足够了。但是，如果需要，您可以还查找管理节点的公共 IP 地址。如果需要查找管理节点的公共 IP 地址，则使用 `dcos node list` [command](#public-ip-cmd) 或 `net/v1/nodes` [API call](#public-ip-api)。
...
此命令输出显示在 DC/OS 群集上运行的相同公共代理的公共 IP 地址。例如：
