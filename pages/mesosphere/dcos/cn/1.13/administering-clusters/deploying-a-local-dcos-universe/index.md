...
您可以通过使用本地 [{{ model.packageRepo }}](https://github.com/mesosphere/universe) 在数据中心安装和运行 DC/OS 服务，无需互联网访问。您可以部署包含所有已认证包（基础）的本地 {{ model.packagerepo }}，也可以部署包含选定包（高级）的本地 {{ model.packagerepo }}。
...
1. （可选）使用以下命令删除群集对默认 {{ model.packageRepo }} 的引用。如果要将默认 {{ model.packageRepo }} 保留在适当位置，只需将本地 {{ model.packageRepo }} 添加为附加资源库，跳至下一步。您还可以从 DC/OS Web 界面中的 **设置** > **包资源库** 中删除对默认 {{ model.packageRepo }} 资源库的引用。
...
1. 通过键入 `exit` 或打开新终端提示关闭 SSH 会话。在每个代理节点上重复这些步骤。
...
1. 执行[已认证 {{ model.packageRepo }} 包]（＃已认证）中描述的所有步骤。
