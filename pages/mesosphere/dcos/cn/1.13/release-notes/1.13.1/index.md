...
[button color="light" href="https://downloads.dcos.io/dcos/stable/1.13.1/dcos_generate_config.sh"]下载 DC/OS 开源[/button]
...
- DC/OS 监控服务 (`dcos-monitoring`) 可配置成使用 DC/OS 存储服务 (DSS) 卷来存储时间序列数据。<!--(DCOS-47725)-->
    ...
    有关使用 DC/OS 监控服务的更多信息，请参见 [DC/OS 监控服务](/mesosphere/dcos/cn/services/beta-dcos-monitoring/). 有关使用 DC/OS 存储服务的更多信息，请参见 [DC/OS 存储服务](/mesosphere/dcos/cn/services/storage/latest/).
...
- 默认情况下，将启用 Mesos 输入插件收集的新卷和网络度量标准。<!--(DCOS-47722, DCOS-47719)-->
    ...
    在 DC/OS 中 1.13，`dcos-telegraf` 默认为自动收集 Mesos 度量标准。之前，您需要通过更新代理配置或通过将 `enable_mesos_input_plugin` 文件中的 `config.yaml` 参数设置为 `true`. 来手动启用度量标准插件。使用此版本，不再需要手动启用此功能。相反，此参数的默认值现在设置为 true。如果您想要禁用 Mesos 度量标准的自动收集，您可以将 `enable_mesos_input_plugin` 文件中的 `config.yaml` 参数设为 false。
...
- 支持通用安装工具上的动态管理节点。<!--(DCOS-45725)-->
    ...
    动态管理节点让您能够创建、销毁和恢复管理节点。使用此功能，您可以使用 Universal 安装工具来缩减或扩大 DC/OS 群集，不仅是能从目前受支持的代理节点，还能从管理节点——如果您认为必须这么做。如需更多信息，请参阅 [可更换的管理节点] 文档。(/mesosphere/dcos/cn/1.13/installing/evaluation/aws/aws-replaceable-masters/).
...
### 安装
- 修复在使用 `/tmp` 选项安装 `noexec` 目录时导致某些 DC/OS 组件崩溃的问题 (DCOS-53077)。
...
- 如果是在未被修改的状态下使用，则修复生成 `fault-domain-detect` 错误的示例 `command not found` 脚本中缺少的 echo 命令行 (DCOS-51792)。
...
- 修复系统重新启动后 `dcos-iam-ldap-sync` 服务未能正确启动的问题 (COPS-4455，COPS-4814，DCOS-48107，DCOS-53420）。
    ...
    在此版本中，DC/OS 身份和访问管理 LDAP 同步 `systemd` 单元不再依赖于在加载 `systemd` 配置时可用的 /opt/mesosphere 目录。
...
- 更新 `urllib3` 至版本 1.24.2，解决在 [CVE-2019-11324](https://nvd.nist.gov/vuln/detail/CVE-2019-11324) 中使用 CA 证书时为 Python 识别的安全漏洞 (DCOS-52210)。
    ...
    此漏洞仍处于分析中。但是，在 Python `urllib3` 库中检测到一个问题，其中，在某些情况下，所需的 CA 证书集与 CA 证书的 OS 存储不同。可以利用该问题在验证应该失败的情况下允许成功的 SSL 连接。
...
### 已弃用或停用的功能
- 在 DC/OS 1.13 中，DC/OS 历史服务已转变为已停用状态。历史服务计划在 DC/OS 1.14 中停用。您可以在 [Mesosphere DC/OS 功能成熟度生命周期](/mesosphere/dcos/cn/1.13/overview/feature-maturity/). 中找到每个功能成熟度状态的定义。<!--DCOS-50304, DCOS-51996-->
