...
DC/OS 2.0 需要 DC/OS CLI >= 0.8。安装时，[请遵循这些说明](/mesosphere/dcos/cn/2.1/cli/install/).
...
在完成登录程序后，您的 CLI 已准备好与您的群集进行交互。您会注意到，现在它有其他命令，例如，`marathon`、`node`、`package` 等。这些命令来自 [插件](/mesosphere/dcos/cn/2.1/cli/plugins/)、dcos-core-cli 和 dcos-enterprise-cli（如适用），它将作为设置命令的一部分自动安装。
