...
1. 要使用 DC/OS [身份和访问管理 (IAM) API](/mesosphere/dcos/cn/2.1/security/oss/iam-api/) 来添加服务账户，请首先通过 OpenSSL 生成 RSA 私钥。
...
要使用 [身份和访问管理 (IAM) API](/mesosphere/dcos/cn/2.1/security/oss/iam-api/) 来列出所有已配置服务账户，请执行以下命令：
...
要使用 DC/OS [身份和访问管理 (IAM) API](/mesosphere/dcos/cn/2.1/security/oss/iam-api/) 来更改服务账户的公钥，请在 `public-key.pem` 文件中提供一个新的公钥。然后，替换以下命令中的 `<uid>`，并执行命令：
...
要使用 DC/OS [身份和访问管理 (IAM) API](/mesosphere/dcos/cn/2.1/security/oss/iam-api/) 来移除本地用户账户，请用相应的值替换 `<uid>` 并执行以下命令：
