...
使用 [DC/OS CLI](/mesosphere/dcos/cn/2.1/cli/)，可以通过指定 `dcos-services` 登录提供商，以服务的身份登录。

要使用 DC/OS CLI 来测试服务登录，请通过 [auth login]`dcos-services` 命令指定 (/mesosphere/dcos/cn/2.1/cli/command-reference/dcos-auth/dcos-auth-login/) 登录提供商。在执行以下命令之前，先用相应的值替换 `<service-account-id>` 和 `<private-key-path>`
...
在服务登录期间，服务登录令牌会被发送到 DC/OS [身份和访问管理 (IAM) API](/mesosphere/dcos/cn/2.1/security/oss/iam-api/).
