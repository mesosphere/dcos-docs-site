...
有关如何在应用程序或 pod 定义中引用这些密钥的信息，请参阅[配置服务和 pod 以使用密钥](/mesosphere/dcos/cn/2.1/security/ent/secrets/use-secrets/)
...
密钥应包括路径，除非您希望允许所有服务访问其值。有关密钥路径的更多信息，请参阅[空间](/mesosphere/dcos/cn/2.1/security/ent/#spaces)
...
- 有关从 CLI 或 API 创建密钥所需的权限，请参阅[密钥存储库权限](/mesosphere/dcos/cn/2.1/security/ent/perms-reference/#secrets) 您配置的权限必须包括允许用户创建的密钥名称。每个密钥必须拥有一项权限。密钥名称和权限名称必须匹配。

- [已安装 DC/OS CLI](/mesosphere/dcos/cn/2.1/cli/install/) 以及 [已安装 DC/OS Enterprise CLI](/mesosphere/dcos/cn/2.1/cli/enterprise-cli/#ent-cli-install).
...
1. 单击右上方的 **+** 图标。
    ...
    ![新密钥](/mesosphere/dcos/cn/2.1/img/new-secret.png)

    图 1 - 新密钥图标

    如果您当前没有密钥，将显示 **创建密钥** 界面。单击 **创建密匙** 按钮。

    ![创建密钥](/mesosphere/dcos/cn/2.1/img/GUI-Secrets-Create-Secret.png)

    图 2 -“创建密匙”按钮

1. 在 **创建新密钥** 屏幕的 **ID** 框中，键入密钥的名称及其路径（若有）。

    ![密钥 ID 键对](/mesosphere/dcos/cn/2.1/img/GUI-Secrets-Create-New-Keypair.png)
...
1. 在 **Value** 框中键入或粘贴密钥。
    ![密钥 ID/值字段](/mesosphere/dcos/cn/2.1/img/GUI-Secrets-Create-New-Keypair.png)
...
返回到密匙屏幕，可以看到密匙已经部署。
   ...
   ![已部署密匙](/mesosphere/dcos/cn/2.1/img/GUI-Secrets-Secrets-Keypair-Deployed.png)
...
<p class="message--note"><strong>注意：</strong>在发出本部分中的 curl 命令之前，必须遵循 <a href="/mesosphere/dcos/2.1/security/ent/tls-ssl/get-cert/">获取 DC/OS CA 捆绑包</a> 中的步骤。</p>
...
1. 使用 `dcos auth login` 登录到 CLI。您可在 [CLI 参考] 中找到有关此命令的更多信息。(/mesosphere/dcos/cn/2.1/cli/command-reference/dcos-auth/dcos-auth-login/).
...
1. 在 **ID** 框中，提供密钥名称及其路径（如果有）。
    ...
    ![创建新密钥](/mesosphere/dcos/cn/2.1/img/GUI-Secrets-Create-New-Secret.png)
...
返回到密匙屏幕，可以看到密匙已经部署。
   ...
   ![已部署密匙](/mesosphere/dcos/cn/2.1/img/GUI-Secrets-Deployed.jpeg)
