...
- [手动密封后](/mesosphere/dcos/cn/2.1/security/ent/secrets/seal-store/)
- 断电后。
- 由于无法通过 Vault 实例访问底层存储（如 ZooKeeper）。

密封的密钥存储库无法从 Web 界面访问。无法使用 [密钥 API] 检索密码值。(/mesosphere/dcos/cn/2.1/security/ent/secrets/secrets-api/). 依赖于通过环境变量配置的值的服务可能无法部署。
...
密封的预期状态是持久的，因此，如果存在密封存储库的前提条件之一，即使在重新启动后，`dcos-secrets` 也会自动将其密封。只有[密封存储库](/mesosphere/dcos/cn/2.1/security/ent/secrets/seal-store/)中描述的步骤才会将其密封。
