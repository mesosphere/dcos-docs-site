...
1. 如果 `NameID` 的 `Subject` 似乎包含电子邮件地址，则 DC/OS 会使用电子邮件地址值。
...
此程序使用身份和访问管理 API (IAM API)。有关 IAM API 的更多详细信息，请访问 [IAM API 文档](/mesosphere/dcos/cn/2.1/security/ent/iam-api/).


1. 使用浏览器或 curl 向 `GET` 发出 `<your-cluster-URL>/acs/api/v1/auth/saml/providers` 请求。
...
4. 向 `GET` 发出 `<your-cluster-URL>/acs/api/v1/auth/saml/providers/{provider-id}/acs-callback-url` 请求，将 `{provider-id}` 替换为您在上一步中获得的提供程序 ID。
5. 此请求返回回调 URL。
...
3. 为帐户分配所需权限。有关分配权限的详细信息，请访问[权限](/mesosphere/dcos/cn/2.1/security/ent/perms-reference/)文档。
