...
服务或作业的默认 Linux 用户可能因安全模式和容器类型而异 请参阅 [Linux 用户](/mesosphere/dcos/cn/2.1/security/ent/#linux-users) 了解更多信息。
...
许多 {{ model.packageRepo }} 服务忽略其用户帐户的覆盖，在 `strict` 模式下除外。我们提供覆盖服务默认 Linux 用户的详细步骤，该服务在[服务账户]中支持此覆盖。(/mesosphere/dcos/cn/2.1/security/ent/service-auth/). 有关逐步说明，请参阅与感兴趣的服务相关部分。这些程序还说明如何配置服务以使用加密和服务账户。

请记得授予权限，以便对启动 `create` 服务的服务帐户用户执行 `dcos:mesos:master:task:user[:<linux-user-name>]` 资源上的 {{ model.packageRepo }} 操作。请参阅 [Mesos 权限](/mesosphere/dcos/cn/2.1/security/ent/perms-reference/#mesos-permissions)，了解更多信息。

# <a name="marathon-app-def"></a>通过 Marathon 应用定义覆盖默认 Linux 用户

Marathon 应用定义提供用于覆盖默认 Linux 用户的 `"user"` 密钥。**提示：**参考 [Marathon 文档](/mesosphere/dcos/cn/2.1/deploying-services/creating-services/)，了解有关编写 Marathon 服务的更多详细信息。

以下教程将展示所有权如何运作。在开始之前，请确保：

- 代理程序上已存在 Linux 用户帐户。
- 您已安装并登录到 [DC/OS CLI](/mesosphere/dcos/cn/2.1/cli/).
- 必须遵守 [下载根证书](/mesosphere/dcos/cn/2.1/security/ent/tls-ssl/get-cert/) 中的步骤才能发布此部分的 curl 命令。
- 您已授予对`create` DC/OS 服务帐户用户 `dcos:mesos:master:task:user:<linux-user-name>` 资源执行 `dcos_marathon` 操作的权限。
...
1. 创建 Marathon 应用定义，并使用信息名称保存，如 `myservice.json`. 以下服务会将其在用户名称下运行的用户名称写入日志，创建新文件，并从 dcos.io 获取 Mesosphere 徽标。
    ...
    ```json
    {
      ...
      "uris": [
          "/mesosphere/dcos/2.1/img/logos/mesosphere.svg"
      ]
    }
    ```
...
1. 使用 [Marathon API]部署服务。(/mesosphere/dcos/cn/2.1/deploying-services/marathon-api/).
...
<p class="message--note"><strong>注意：</strong>有关创建和部署作业的更多信息，请参阅<a href="/mesosphere/dcos/2.1/deploying-jobs/quickstart/">作业文档</a></p>
...
- 您已安装并登录到 [DC/OS CLI](/mesosphere/dcos/cn/2.1/cli/).
- 必须遵守 [下载根证书](/mesosphere/dcos/cn/2.1/security/ent/tls-ssl/get-cert/) 中的步骤才能发布此部分的 curl 命令。
- 您已授予对`create` DC/OS 服务帐户用户 `dcos:mesos:master:task:user:<linux-user-name>` 资源执行 `dcos_metronome` 操作的权限。
...
1. 创建 Metronome 作业定义，并使用信息名称保存，如 `myjob.json`.
    ...
    "run": {
      "artifacts": [
        {
          "uri": "/mesosphere/dcos/2.1/img/logos/mesosphere.svg"
        }
      ],
...
1. 使用 [Metronome REST API] 部署作业。(https://dcos.github.io/metronome/docs/generated/api.html).
