...
日志记录 API 由 [DC/OS 日志组件](/mesosphere/dcos/cn/2.1/overview/architecture/components/#dcos-log) 支持，后者在群集中的所有节点上运行。有关使用日志记录 API 的更多信息，请参阅 [日志记录](/mesosphere/dcos/cn/2.1/monitoring/logging/). 关于使用示例，请参阅 [日志记录 API 示例](/mesosphere/dcos/cn/2.1/monitoring/logging/logging-api-examples/).
...
在 1.11 之前的 DC/OS 版本中，节点和组件日志由  管理。`journald`. 不过，由于 [journald 性能问题]，[Mesos 任务 journald 日志槽已被禁用]。(/mesosphere/dcos/cn/2.1/installing/production/advanced-configuration/configuration-reference/#mesos-container-log-sink). 因此，只能通过 [Mesos 任务沙盒文件 API](http://mesos.apache.org/documentation/latest/sandbox/) 来访问旧版本的容器日志文件。
...
要确定群集的地址，请参阅 [群集访问](/mesosphere/dcos/cn/2.1/api/access/).
...
所有日志记录 API 路由均需要认证才能使用。要验证 API 请求，请参阅 [获取认证令牌](/mesosphere/dcos/cn/2.1/security/ent/iam-api/#/obtaining-an-authentication-token/) 和 [传递认证令牌](/mesosphere/dcos/cn/2.1/security/ent/iam-api/#/passing-an-authentication-token/).
...
用户也可以通过 dcos:superuser 权限来到达所有路由。要为您的账户分配权限，请参阅 [权限参考](/mesosphere/dcos/cn/2.1/security/ent/perms-reference/).
...
- `application/json`- JSON 格式的  请求日志。
- `text/event-stream`- 服务器发送事件格式的  请求日志。
...
Fluent Bit 的默认配置标签日志由带  的 systemd 单元传输。`host.<unit-name>`.
例如，`dcos-mesos-master.service` 的日志标记有 `host.dcos-mesos-master.service`.
