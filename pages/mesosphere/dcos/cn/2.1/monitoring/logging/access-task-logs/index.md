...
以下是对所需 [权限](/mesosphere/dcos/cn/2.1/security/ent/perms-reference/) 的概述：
...
1. 选择**组织**并选择**组**。
    ...
    ![新组](/mesosphere/dcos/cn/2.1/img/GUI-Organization-Groups-Add_Group_w_Plus_Button_Tooltip-1_12.png)

    图 1. 新用户组

1. 新建一个组。

    ![生产组](/mesosphere/dcos/cn/2.1/img/GUI-Organization-Groups-Create_New_Group_Modal-1_12.png)

    图 2. 创建新组屏幕

1. 选择组名，并从**权限**选项卡上单击**添加权限**。

    ![为生产组添加权限](/mesosphere/dcos/cn/2.1/img/GUI-Organization-Groups-Group_Detail_Page-1_12.png)
...
1. 单击**插入权限字符串**以切换对话框，然后粘贴到以下权限中，单击**添加权限**。
    ...
    ![添加权限](/mesosphere/dcos/cn/2.1/img/GUI-Organization-Groups-Add_Perms_Modal_Logging-1_12.png)
...
1. 从**组成员**选项卡，在搜索框中输入并选择组名。这将向单个用户授予组权限。
    ...
    ![添加 Alice 到安全组](/mesosphere/dcos/cn/2.1/img/GUI-Organization-Users-User_Alice_Add_prod_group-1_12.png)
...
1. 选择**单个容器**，并将您的服务定义为：
    ...
    ![定义嵌套服务](/mesosphere/dcos/cn/2.1/img/GUI-Services-Add_Service_Single_Container_Alice-1_12.png)

    图 7. 定义一个嵌套服务

1. 单击**查看并运行**和**运行服务**以完成安装。您现在应该能看到一个在组中运行的服务。

    ![在组中运行的服务](/mesosphere/dcos/cn/2.1/img/GUI-Services-Main_View_prod_group_Alice-1_12.png)
