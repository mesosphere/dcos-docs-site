...
使用 [dcos cluster](/mesosphere/dcos/cn/2.1/cli/command-reference/dcos-cluster/) 命令连接到多个 DC/OS 群集。
...
- 可用：群集在本地设置，且可访问
- 不可用：群集在本地设置，且无法访问
- 未配置：群集不在本地设置（即，群集被 [链接](/mesosphere/dcos/cn/2.1/administering-clusters/multiple-clusters/cluster-links/)至当前附加的群集上）。
