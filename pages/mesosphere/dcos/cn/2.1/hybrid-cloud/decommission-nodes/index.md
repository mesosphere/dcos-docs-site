...
<p class="message--warning"><strong>警告：</strong>仅在节点永远不会返回（例如，EC2 VM 销毁）时才应该停用节点 。节点停用后，对应的代理 ID 被内部标记为 GONE 并且不允许返回和在管理节点上重新注册。节点上运行的任何任务已过渡到 <code>TASK_GONE_BY_OPERATOR</code> 状态。</p>
