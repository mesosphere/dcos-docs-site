...
DC/OS 中应用程序或组定义的每个更改都是作为 Marathon 部署执行的。[部署](/mesosphere/dcos/cn/1.11/deploying-services/marathon-api/) 是一组操作，包括：
...
![依赖关系图](/mesosphere/dcos/cn/1.11/img/dependency.png)
...
- **运行状况检查**在每个应用程序中指定，并根据任务运行。如果任务的运行状况检查失败，DC/OS 将替换该任务。如需更多信息，请参阅 [文档](/mesosphere/dcos/cn/1.11/deploying-services/creating-services/health-checks/)。
