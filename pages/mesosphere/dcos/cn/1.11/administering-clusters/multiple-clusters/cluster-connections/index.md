...
使用 [dcos cluster](/mesosphere/dcos/cn/1.11/cli/command-reference/dcos-cluster/) 命令连接到多个 DC/OS 群集。
...
- 可用：集群在本地设置，且可访问
- 不可用：集群在本地设置，且无法访问
- 未配置：集群不在本地设置（即，集群被 [链接](/mesosphere/dcos/cn/1.11/administering-clusters/multiple-clusters/cluster-links/)至当前附加的集群上）。
