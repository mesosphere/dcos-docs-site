...
1. 将客户端密钥从 Google 粘贴到 **Client Secret** 字段。
    ...
    ![Google IdP 配置](/mesosphere/dcos/cn/1.11/img/oidc-google.png)
...
1. 为此用户分配适当的[权限](/mesosphere/dcos/cn/1.11/security/ent/perms-reference/)。

### <a name="using-cli"></a>使用 DC/OS CLI

**先决条件：**[已安装 DC/OS CLI](/mesosphere/dcos/cn/1.11/cli/install/)。
...
1. 您应该看到类似以下内容的消息。
    ...
    ![CLI IdP 认证令牌](/mesosphere/dcos/cn/1.11/img/cli-auth-token.png)
