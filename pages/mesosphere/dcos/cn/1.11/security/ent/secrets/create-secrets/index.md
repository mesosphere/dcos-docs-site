...
有关如何在应用程序或 pod 定义中引用这些密钥的信息，请参阅[配置服务和 pod 以使用密钥](/mesosphere/dcos/cn/1.11/security/ent/secrets/use-secrets/)。

# 创建密钥

以下部分解释了如何使用 Web 界面、CLI 和密钥 API，将密钥创建为键/值对和文件。

密钥应包括路径，除非您希望允许所有服务访问其值。有关密钥路径的更多信息，请参阅[空间](/mesosphere/dcos/cn/1.11/security/ent/#spaces)。
...
- 有关从 CLI 或 API 创建密钥所需的权限，请参阅[密钥存储库权限](/mesosphere/dcos/cn/1.11/security/ent/perms-reference/#secrets)。您配置的权限必须包括允许用户创建的密钥名称。每个密钥必须拥有一项权限。密钥名称和权限名称必须匹配。
...
1. 单击右上方的 **+** 图标。
    ...
    ![新密钥](/mesosphere/dcos/cn/1.11/img/new-secret.png)
...
1. 在 **Value** 框中键入或粘贴密钥。
    ...
    ![密钥 ID/值字段](/mesosphere/dcos/cn/1.11/img/create-secret.png)
...
如果您<a href="/mesosphere/dcos/cn/1.11/installing/production/advanced-configuration/configuration-reference/#security-enterprise">安全模式</a>为 <tt>permissive</tt> 或 <tt>strict</tt>，则在本部分中发出 curl 命令之前，必须遵循中的步骤<a href="/mesosphere/dcos/cn/1.11/security/ent/tls-ssl/get-cert/">获取 DC/OS CA 捆绑包</a>。如果您的<a href="/mesosphere/dcos/cn/1.11/installing/production/advanced-configuration/configuration-reference/#security-enterprise">安全模式</a>为 <tt>disabled</tt>，则必须在将其发出前从命令中删除 <tt>--cacert dcos-ca.crt</tt>。</p>
...
1. 使用 `dcos auth login` 登录到 CLI。您可在 [CLI 参考](/mesosphere/dcos/cn/1.11/cli/command-reference/dcos-auth/dcos-auth-login/) 中找到有关此命令的更多信息。
