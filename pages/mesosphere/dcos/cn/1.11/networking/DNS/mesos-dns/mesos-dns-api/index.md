...
要验证 API 请求，请参阅 [获取认证令牌](/mesosphere/dcos/cn/1.11/security/ent/iam-api/#obtaining-an-authentication-token) 和 [通过传递认证令牌](/mesosphere/dcos/cn/1.11/security/ent/iam-api/#passing-an-authentication-token)。
...
# 资源
Mesos-DNS 实现了一个简单的 REST API，用于通过 HTTP 进行服务发现。这些示例假设您具有 [与节点的 SSH 连接](/mesosphere/dcos/cn/1.11/administering-clusters/sshcluster/)。
