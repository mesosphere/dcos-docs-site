...
- 要更新到较新的主要或次要版本（例如 1.13 到 2.0），请参阅 [升级](/mesosphere/dcos/cn/2.0/installing/production/upgrading/) 的说明。
...
- 在修补 DC/OS 之前，请先查看 [版本注释](/mesosphere/dcos/cn/2.0/release-notes/)
...
## 支持的补丁路径
- 从任何当前版本到下一版本。例如，支持 2.0.4 到 2.0.5 的补丁。
- 从任何当前版本到同一版本。例如，支持 2.0.4 到 2.0.4 的补丁。这对于进行配置更改非常有用。
...
<table style="border-collapse: collapse;" Border = "1" Cellpadding = "5" Cellspacing = "5">
   <tr>
    <th Rowspan = "20" Align = "center"><strong>修补<br> 前版本</strong></div></th>
   <tr>
    <th></th>
    <th Colspan = "20" Align = "center"><strong>修补至</strong></th>
   </tr>
    <th></th>
    <th>2.0.1</th>
    <th>2.0.2</th>
...
**不能** 在修补到新版本的同时更改群集配置。必须通过对已安装版本的更新进行群集配置更改。例如，您无法同时将群集从 2.0.x 修补到 2.0.y 并添加更多公共代理节点。您可以更新为 2.0.x，然后修补为 2.0.y，或者可以修补到 2.0.y，再在修补后更新 2.0.y，从而添加更多公共代理。
...
- [`dns_search`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#dns-search)
- [`docker_remove_delay`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#docker-remove-delay)
- [`gc_delay`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#gc-delay)
- [`resolvers`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#resolvers)
- [`telemetry_enabled`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#telemetry-enabled)
- [`use_proxy`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#use-proxy)
    - [`http_proxy`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#use-proxy)
    - [`https_proxy`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#use-proxy)
    - [`no_proxy`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#use-proxy)
- [`enable_mesos_input_plugin`](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#enable-mesos-input-plugin)

安全模式（`security`）可以更改，但只能更改为更严格的安全模式。不支持安全降级。例如，如果群集处于 `strict` 模式，而您希望降级至 `permissive` 模式，则必须重新安装群集并终止所有运行的工作负载。

有关不同安全模式的更多信息，请参阅安全 [模式](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#security-enterprise)
...
- 所有主机（管理和代理节点）必须能够与所有其他主机通信，如 [网络安全] 所述。(/mesosphere/dcos/cn/2.0/administering-clusters/securing-your-cluster/#network-security).
...
- 您必须熟悉 DC/OS [生产安装](/mesosphere/dcos/cn/2.0/installing/production/deploying-dcos/installation/) 说明。
...
- [备份您的群集](/mesosphere/dcos/cn/2.0/administering-clusters/backup-and-restore/).
- **可选** 您可以将自定义 [节点和群集运行状况检查](/mesosphere/dcos/cn/2.0/installing/production/deploying-dcos/node-cluster-health-check/#custom-health-checks) 添加到 `config.yaml`.

## bootstrap 节点

选择您所需的安全模式，然后按照适用的补丁说明进行。

- [修补 DC/OS 2.0 而不更改安全模式](#current-security)
- [在严格安全模式下修补 DC/OS 2.0](#strict)

# <a name="current-security"></a>修补 DC/OS 2.0 而不更改安全模式
该程序修补 DC/OS 2.0 群集而不更改群集的 [安全模式](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#security-enterprise)。
...
1. 将旧的 `config.yaml` 合并为新的 `config.yaml` 格式。大多数情况下，区别会很小。
    ...
    <p class="message--note"><strong>注意：</strong>在修补期间，您无法更改 <code>exhibitor_zk_backend</code> 设置。<code>config.yaml</code> 的语法可能与早期版本不同。有关当前 <code>config.yaml</code> 语法和参数的详细说明，请参阅 <a href="/mesosphere/dcos/2.0/installing/production/advanced-configuration/configuration-reference/">文档</a>.</p>
...
1. 构建安装工具包。
    ...
    1. 运行 [nginx](/mesosphere/dcos/cn/2.0/installing/production/deploying-dcos/installation/) 容器以为安装文件提供服务。

1. 转到 DC/OS 管理节点 [程序](/mesosphere/dcos/cn/2.0/installing/production/patching/#masters) 完成安装。

# <a name="strict"></a>在严格模式下修补 DC/OS 2.0
该程序在严格 [安全模式](/mesosphere/dcos/cn/2.0/installing/production/advanced-configuration/configuration-reference/#security-enterprise) 下修补到 DC/OS 2.0。
...
- 群集必须是 [DC/OS 2.0 的新近修补版本](#current-security) 并在 [宽容安全模式](#permissive) 下运行，然后才能更新到严格模式。如果群集在修补到 DC/OS 2.0 之前以严格模式运行，则可以跳过该程序。
- 如果您在运行 Pod 或者已在自定义配置中启用 Mesos “HTTP 命令执行器” 功能，则必须在修补到严格模式之前，以 DC/OS 2.0 宽容安全模式重启这些任务。否则，在修补管理节点时，这些任务将会被重新启动。
...
1. 构建安装工具包。
    ...
    1. 下载 `dcos_generate_config.ee.sh` 文件。
    1. 生成安装文件。将以下命令中的 `<installed_cluster_version>` 替换为当前要修补的群集上正在运行的 DC/OS 版本，例如 `2.0.4`.
        ```bash
        dcos_generate_config.ee.sh --generate-node-upgrade-script <installed_cluster_version>
        ```
    1. 上一步骤的命令将在其输出的最后一行产生 URL，前缀为 `Node patch script URL:`. 记下此 URL 以供后续使用。它在本文档中被称为“节点补丁脚本 URL”。
    2. 运行 [nginx](/mesosphere/dcos/cn/2.0/installing/production/deploying-dcos/installation/) 容器以为安装文件提供服务。
...
1. 通过在管理节点上运行以下命令来验证补丁：
    ...
    1. 验证 `curl http://localhost:5050/metrics/snapshot` 的度量标准 `registrar/log/recovered` 的值为 `1`.
        ...
        <p class="message--note"><strong>注意：</strong>如果要从宽容模式升级到严格模式，此 URL 将为“curl https://...”，并且您需要有 JWT 才能访问。</p>
        [enterprise type="inline" size="small" /]

    1. 验证 `/opt/mesosphere/bin/mesos-master --version` 表示修补后的管理节点正在运行 [发行说明](/mesosphere/dcos/cn/2.0/release-notes/) 中指定的 Mesos 版本，例如 `1.9.1`.
...
如果您的修补因为 [自定义节点或群集检查](/mesosphere/dcos/cn/2.0/installing/production/deploying-dcos/node-cluster-health-check/#custom-health-checks) 而失败，运行以下命令可了解更多详细信息：
```bash
...
DC/OS 2.0 {{ model.packageRepo }} 中可用的包比旧版本 {{ model.packageRepo }} 中的要新。服务在安装 DC/OS 时不会自动修补，因为并非所有 DC/OS 服务都具有保持现有状态的修补路径。