...
当您定义并启动服务时，Marathon 将执行交给 Apaches&reg; Mesos&reg;。Mesos 为每个任务创建一个沙盒目录。sandbox 目录是每个代理节点上的目录，用作执行环境并包含相关的日志文件。`stderr` 和 `stdout` 流也会写入沙盒目录。
...
# REST API
您可以使用 REST API 部署基于 Docker&reg; 的简单应用程序。通过 Marathon，运行使用 Docker 镜像的应用程序非常简单。
