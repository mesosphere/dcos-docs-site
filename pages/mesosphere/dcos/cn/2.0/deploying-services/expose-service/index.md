...
1. 使用此命令将您的应用程序添加到 Marathon，其中 `myApp.json` 是包含 Marathon 应用定义的文件。
    ...
    如果添加成功，则没有输出。还可以使用 DC/OS [UI] 的 **服务** 选项卡添加应用程序。(/mesosphere/dcos/cn/2.0/gui/services/).
...
1. 使用此命令验证应用程序是否已添加：
    ...
    还可以使用 DC/OS [UI] 的 **服务** 选项卡查看已部署的应用程序。(/mesosphere/dcos/cn/2.0/gui/services/).
...
1. 配置边缘负载均衡器和服务发现机制。
    ...
    - AWS&reg; 用户：如果您通过使用 [AWS CloudFormation 模板](/mesosphere/dcos/cn/2.0/installing/evaluation/community-supported-methods/aws/)安装了 DC/OS，则会包含 ELB。但是，您则必须在公共 ELB 上重新配置运行状况检查，以将应用程序公开到应用定义中指定的端口（例如，端口 80）。
    - 所有其他用户：您可以使用 [Marathon-LB](/mesosphere/dcos/cn/services/marathon-lb/latest/)，一种基于 HAProxy&reg; 的快速代理程序和负载均衡器。

1. 转到公共代理节点，查看网站是否正在运行。有关如何查找公共代理 IP 的信息，请参阅 [文档](/mesosphere/dcos/cn/2.0/administering-clusters/locate-public-agent/).

    您应在浏览器中看到以下消息：

    ![Hello Brave World](/mesosphere/dcos/cn/2.0/img/helloworld.png)
