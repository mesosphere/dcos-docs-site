...
![Mesosphere DC/OS Enterprise 组件](/mesosphere/dcos/cn/2.0/img/dcos-components-1-13.png)
...
Mesos 作为分布式系统内核来管理资源和任务。Mesos 管理节点揭示调度器、执行器和操作界面，以便进行群集管理。Mesos 代理节点管理每个 [DC/OS 代理节点]上的单个执行器、任务和资源。(/mesosphere/dcos/cn/2.0/overview/concepts/#dcos-agent-node). Mesos Agent Public 是配置用于在 [DC/OS 公共代理节点] 上运行的 Mesos 代理节点(/mesosphere/dcos/cn/2.0/overview/concepts/#public-agent-node).
...
- [文档](/mesosphere/dcos/cn/2.0/networking/DNS/mesos-dns)
...
DC/OS 包管理器 (Cosmos) 安装和管理 [DC/OS 软件包存储库](/mesosphere/dcos/cn/2.0/administering-clusters/package-registry/) 中的 DC/OS 软件包，如 [Mesosphere {{ model.packageRepo }}](https://github.com/mesosphere/universe).
