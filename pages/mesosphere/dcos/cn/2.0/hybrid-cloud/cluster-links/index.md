...
您可以使用 DC/OS&trade; CLI [dcos cluster link](/mesosphere/dcos/cn/2.0/cli/command-reference/dcos-cluster/dcos-cluster-link/) 和 [dcos cluster unlink](/mesosphere/dcos/cn/2.0/cli/command-reference/dcos-cluster/dcos-cluster-unlink/) 命令和 [cluster link API](/mesosphere/dcos/cn/2.0/administering-clusters/multiple-clusters/cluster-link-api/) 添加和删除一个群集到另一个群集的链接。设置链接后，您可以使用 CLI 或 UI 轻松在群集之间切换。如果已经使用 SSO 提供程序设置链接，您不需要提供凭证即可切换群集。

您必须是超级用户或具有相应的群集链路 [权限](/mesosphere/dcos/cn/2.0/security/ent/perms-reference/#cluster-linker)才能查看、添加和删除链路并授予查看链路群集的权限。
...
1. 配置 [OpenID&reg; IdP](/mesosphere/dcos/cn/2.0/security/ent/sso/setup-openid/)。
    1. 确保在 Google&reg; Dev 控制台中的**授权 Javascript 源** 和 **授权重定向 URL** 字段中均提供两个群集 URL。
...
要查看所有链接的群集，运行 `dcos cluster list` 命令。如果群集已链接但未设置，则其状态为 `UNCONFIGURED`. 如果群集已链接和附加，其状态为 `AVAILABLE`. 另请参阅 [查看连接的群集](/mesosphere/dcos/cn/2.0/administering-clusters/multiple-clusters/cluster-connections/).
...
您可以使用 CLI 或 UI 在链接的群集之间切换。使用 CLI 切换群集时，新群集将成为 CLI 的活动群集。新群集成为您在 UI 中看到的群集。如果在 CLI 中切换群集，它不会更改 UI 中的群集；同样，在 UI 中切换，不会影响 CLI 中附加的群集。
...
1. 选择 **切换群集**。
    ...
    ![swi 集群](/mesosphere/dcos/cn/2.0/img/switch-cluster-1-12.png)

    图 2 - 群集切换

1. 单击要切换到的群集名称。

    ![swi 链接的集群](/mesosphere/dcos/cn/2.0/img/switch-linked-cluster.png)

    图 3 - 切换到链接的群集

如果您是超级用户，还可以在“链接群集”选项卡中切换到链接的群集。

1. 选择 **群集 -> 链接群集**。

1. 在切换目标群集的最右侧单击垂直椭圆，然后选择 **切换**。

   ![swi linked cluster2](/mesosphere/dcos/cn/2.0/img/switch-linked-cluster2.png)
...
1. 作为外部用户，使用 Google 凭证登录群集的 DC/OS UI `cluster-a`
   ...
   ![google 登录](/mesosphere/dcos/cn/2.0/img/google-login.png)
...
1. 使用 Google IDP 设置群集。
    ...
    响应要求您验证群集证书捆绑包的指纹，而且必须使用响应  接受该响应。`yes`.
