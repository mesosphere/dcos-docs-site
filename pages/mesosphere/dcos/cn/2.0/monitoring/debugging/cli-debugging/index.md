...
```
有关日志级别的更多信息，请参阅 [CLI 命令参考](/mesosphere/dcos/2.0/cli/command-reference/) 或运行 `dcos --help`。

# 调试被卡住部署的子命令

DC/OS CLI 提供一组调试子命令，用于排除卡住的服务或 Pod 部署。您也可以从 [DC/OS UI] 使用调试服务和 Pod(/mesosphere/dcos/2.0/monitoring/debugging/gui-debugging/)。
...
## 应用定义示例
如果您目前没有在部署时卡住的服务或 Pod，您可以使用以下两个 [Marathon 应用定义](/mesosphere/dcos/2.0/deploying-services/creating-services/) 来验证本节的说明。
...
[`dcos marathon debug list`](/mesosphere/dcos/2.0/cli/command-reference/dcos-marathon/dcos-marathon-debug-list/) 命令向您显示处于等待状态的所有服务。这让您能够只查看未在运行的服务。
...
一旦您知道哪些服务或 pod 在部署中停滞不前，请使用 [`dcos marathon debug summary /<app-id>|/<pod-id>`命令] (/mesosphere/dcos/2.0/cli/command-reference/dcos-marathon/dcos-marathon-debug-summary/)了解有关具体卡壳服务或 pod 的更多信息。
...
[`dcos marathon debug details /<app-id>|/<pod-id>` 命令](/mesosphere/dcos/2.0/cli/command-reference/dcos-marathon/dcos-marathon-debug-details/) 可以让您确切了解应当如何更改您的服务或 Pod 定义。
...
有关此命令的更多信息，请参阅 [CLI 命令参考部分](/mesosphere/dcos/2.0/cli/command-reference/dcos-marathon/dcos-marathon-debug-details/)。
