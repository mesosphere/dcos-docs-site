...
* [Amazon: EBS&reg;][2]
* [Azure&reg;: 关于 Azure 虚拟机的磁盘和 VHD][3]
* [Azure: Microsoft&reg; Azure 存储简介][4]

# 最佳实践

`Mount` 磁盘资源主要用于 Kafka&reg; 和 Cassandra&reg; 等有状态服务，这些服务可以从整个群集中可用的专用存储中受益。使用 `Mount` 磁盘资源的任何服务具有对保留资源的独占访问权限。然而，考虑服务的性能和可靠性要求仍然很重要。`Mount` 磁盘资源的性能基于底层存储的特性，并且 DC/OS 不提供任何数据复制服务。请考虑以下事项：
