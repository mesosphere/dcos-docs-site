...
对于 DC/OS 来说，登录是交换 [DC/OS 认证令牌](/mesosphere/dcos/2.0/security/oss/authentication/authentication-token/)用户凭据的过程。

用户必须获得 DC/OS 认证才能使用 DC/OS 群集。在 DC/OS 中，认证令牌的使用期限为五天。认证令牌到期后，用户必须再次登录。

DC/OS 处理多种用户类型。可通过 [IAM API](/mesosphere/dcos/2.0/security/oss/iam-api/)管理用户账户；参见 [用户管理](/mesosphere/dcos/2.0/security/oss/user-management/)。
