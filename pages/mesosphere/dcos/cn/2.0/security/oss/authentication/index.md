...
在 DC/OS 中，系统的唯一验证器是 [Admin Router](/mesosphere/dcos/2.0/overview/architecture/components/#admin-router)。它根据 [身份和访问管理 (IAM)](/mesosphere/dcos/2.0/overview/architecture/components/#dcos-iam)发出的信息来执行 DC/OS 认证令牌验证。
...
您可以通过以下任一方式禁用身份认证：
1. 通过 [高级安装](/mesosphere/dcos/2.0/installing/production/deploying-dcos/installation/) 禁用身份认证：您可以通过将此参数添加到配置文件 (`genconf/config.yaml`) 来禁用身份认证。
...
2. 通过 [AWS](/mesosphere/dcos/2.0/installing/evaluation/community-supported-methods/aws/) 上的云安装来禁用身份认证：您可以在 **指定详细信息** 步骤上将 `OAuthEnabled` 选项设置为 `false` 来禁用身份认证。
