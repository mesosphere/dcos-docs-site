...
- 外部 [LDAP 目录](/mesosphere/dcos/2.0/security/ent/ldap/)。
- [OpenID Connect 或 SAML 提供程序](/mesosphere/dcos/2.0/security/ent/sso/)。
...
您可以使用 `/ldap/importuser` [IAM API](/mesosphere/dcos/2.0/security/ent/iam-api/) 端点导入一组 LDAP 用户。

**前提条件：**

- 必须设置 `group-search` 配置密钥，如[配置 LDAP 组导入](#Configure-LDAP-group-import) 中所述。
- 现有组条目必须通过使用 `member`、`uniquemember` 或 `memberuid` 属性列出其成员。
- 在本部分中发出 curl 命令之前，必须遵循[获取 DC/OS CA 根证书](/mesosphere/dcos/2.0/security/ent/tls-ssl/get-cert/) 中的步骤。
