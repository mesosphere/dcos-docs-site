...
本教程演示如何在宽容[安全模式]下实现 DC/OS 服务的用户权限。(/mesosphere/dcos/cn/2.0/security/ent/#security-modes). 完成后，您将通过使用 DC/OS 权限获得多租户。
...
1. 从 **Services > Services > Create Group** 创建服务组。
    ...
    ![服务创建组](/mesosphere/dcos/cn/2.0/img/GUI-Services-No_Services_Running-1_12.png)

    图 1. 创建组页面

    在此示例中，创建了一个名为 `prod-a` 和一个名为 `prod-b` 创建组之后，您应该看到两个文件夹。您可以在此处为用户组部署服务并为每个单元设置权限。

    ![组文件夹](/mesosphere/dcos/cn/2.0/img/GUI-Services-List_View_Groups_Empty-1_12.png)
...
1. 创建用户和组并定义每个组所需的权限。
    ...
    1. 选择 **Organization > Users** 并创建新用户。在此示例中，创建了两个用户（`Cory` 和 `Nick`).
        ...
        ![创建用户 Cory](/mesosphere/dcos/cn/2.0/img/GUI-Organization-Users-Create_User_Modal-1_12.png)

        图 3. 创建新用户

        完成后，您应该看到两个用户。

        ![所有用户](/mesosphere/dcos/cn/2.0/img/GUI-Organization-Users-Users_List_View_w_Users-1_12.png)
    ...
    1. 选择 **New Group**。在本示例中，创建了两个组：
        ...
        ![prod-a group](/mesosphere/dcos/cn/2.0/img/GUI-Organization-Groups-Create_New_Group_Modal-1_12.png)
...
1. 为 prod-a-group 添加这些权限，然后单击 **Close**。
    ...
    ![prod-a-group](/mesosphere/dcos/cn/2.0/img/GUI-Organization-Groups-Add_Permission_String_w_Perms-1_12.png)

    图 6. 为  添加权限`prod-a-group`

    以下是添加后的权限视图：

    ![prod-a-group](/mesosphere/dcos/cn/2.0/img/GUI-Organization-Groups-Group_Detail_w_Permissions-1_12.png)
...
1. 选择 **Group Membership**，然后在搜索框中键入 `prod-a-group`，然后单击选择。
    ...
    ![prod-a-group](/mesosphere/dcos/cn/2.0/img/GUI-Organization-Groups-User_Add_Group_Membership-1_12.png)
...
1. 以 Cory 身份登录到 DC/OS UI。您可以看到，用户 Cory 只能访问 **Services** 和 **{{ model.packageRepo }}** 选项卡。同时，Cory 只能看到 **prod-a** 服务。
  ...
  ![prod-a-group](/mesosphere/dcos/cn/2.0/img/GUI-Services-Limited_User_Access_List-1_12.png)
...
1. 选择 **JSON Configuration** 并粘贴到以下应用定义中：
    ...
    ![JSON 视图](/mesosphere/dcos/cn/2.0/img/GUI-Services-Add_Service_With_JSON-1_12.png)
...
1. 在 Cory 或 Nick 登录时，单击 NGINX 启动图标，查看确认消息。
    ...
    ![NGINX](/mesosphere/dcos/cn/2.0/img/GUI-Services-Add_Service_Review_Install-1_12.png)
...
1. 注销当前用户，然后以具有[超级用户](/mesosphere/dcos/cn/2.0/security/ent/perms-reference/#superuser)权限的用户身份返回。您将看到，两个服务都在 `prod-a` 和 `prod-b-groups`. 中运行。
    ...
    ![prod-a-group](/mesosphere/dcos/cn/2.0/img/GUI-Services-Services_List_w_Groups-1_12.png)
