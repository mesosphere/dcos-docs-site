...
若要获取认证令牌，请将请求正文中 `superuser` 的用户名和密码传递给[身份和访问管理服务 API]`/auth/login` 的 (/mesosphere/dcos/cn/2.0/security/ent/iam-api/). 端点。它将返回认证令牌，如下所示。
...
使用 (/mesosphere/dcos/cn/2.0/cli/) 登录 [DC/OS CLI]`dcos auth login` 时，它会在本地存储认证令牌值。您可以在 `curl` 命令中将此值引用为变量（在下一部分中讨论）。或者，您可以使用以下命令获取认证令牌值：
