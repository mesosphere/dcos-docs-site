...
您可能需要手动密封密钥存储库，以保护其内容免受侵入者的侵害。密封的密钥存储库无法从 Web 界面访问。无法使用 [密钥 API](/mesosphere/dcos/2.0/security/ent/secrets/secrets-api/) 检索密码值。取决于密钥存储库中的值的服务可能无法部署。
...
密封的预期状态是持久的；在密封存储库后，重启 `dcos-secrets` 不会拆封它。只有 [拆封存储库](/mesosphere/dcos/2.0/security/ent/secrets/unseal-store/) 中所述的步骤才可以拆封它。
