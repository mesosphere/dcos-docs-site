...
# 说明
`dcos cluster link` 命令使您能够配置从一个群集指向一个或多个群集的单向链接。访问群集时，您可以查看与其连接的群集。您可以[附加](/mesosphere/dcos/cn/1.12/cli/command-reference/dcos-cluster/dcos-cluster-attach/) 已连接的群集，无需事先运行 `dcos cluster setup`。

**先决条件**

- 您使用 [`dcos cluster setup`](/mesosphere/dcos/cn/1.12/cli/command-reference/dcos-cluster/dcos-cluster-setup/)命令设置要被连接的群集，必须指定相同的身份认证提供程序。
