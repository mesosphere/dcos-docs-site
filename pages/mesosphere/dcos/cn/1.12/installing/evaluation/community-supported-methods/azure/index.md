...
1. 在部署的输出中查找 `MASTERFQDN`。要查找该值，单击 `Last deployment` 下面的链接（就是这里的 `4/15/2016 (Succeeded)`），您将看到：
    ...
    ![Deployment history](/mesosphere/dcos/1.12/img/dcos-azure-marketplace-step2a.png)

    图 1. 部署历史记录

1. 单击最新部署并复制 `MASTERFQDN` 到 `Outputs` 部分。

    ![Deployment output](/mesosphere/dcos/1.12/img/dcos-azure-marketplace-step2b.png)
...
1. 查找管理节点的网络安全组资源，
    ...
    ![Resource - Master Node Network Security Group](/mesosphere/dcos/1.12/img/dcos-azure-step2case1a.png)

    图 3. 管理节点网络安全组

1. 单击左侧的 **入站安全规则** 选项卡。

    ![Inbound Security Rules](/mesosphere/dcos/1.12/img/dcos-azure-step2case1b.png)

    图 4. 入站安全规则

1. 添加入站安全规则。

    ![Add Inbound Security Rules](/mesosphere/dcos/1.12/img/dcos-azure-step2case1c.png)

    图 5. 添加入站安全规则 

1. 查找管理节点的负载均衡器资源。

    ![Resource - Master Node Load balancer](/mesosphere/dcos/1.12/img/dcos-azure-step2case1d.png)

    图 6. 管理节点负载均衡器

1. 单击左侧的 **入站 NAT 规则** 选项卡，

    ![Inbound NAT Rules](/mesosphere/dcos/1.12/img/dcos-azure-step2case1e.png)

    图 7. 入站 NAT 规则

1. 添加入站 NAT 规则。

    ![Add Inbound NAT Rules](/mesosphere/dcos/1.12/img/dcos-azure-step2case1f.png)
...
![DC/OS dashboard](/mesosphere/dcos/1.12/img/dcos-gui.png)
...
[1]: /mesosphere/dcos/cn/1.12/security/ent/users-groups/
