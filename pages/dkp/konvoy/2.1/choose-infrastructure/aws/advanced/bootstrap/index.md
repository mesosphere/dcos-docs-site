...
<p class="message--note"><strong>WARNING: </strong>If you are running in an AWS environment with non-standard AWS Service Endpoints, C2S, SC2S for example, you must use flag --aws-service-endpoints. In this case, run `dkp create bootstrap` with the following parameters:
`dkp create bootstrap --aws-service-endpoints=${SigningRegion1}:${ServiceID1}=${URL},${ServiceID2}=${URL};${SigningRegion2}...`
ServiceIDs would be: `ec2` , `elasticloadbalancing` , `autoscaling` , `secretsmanager` , `sts`.</p>
