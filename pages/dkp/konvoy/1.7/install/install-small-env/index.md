...
* An existing `cluster.yaml` file.

<p class="message--note"><strong>NOTE: </strong>If you do not have an existing <code>cluster.yaml</code> file run the <code>konvoy init --provisioner docker</code> command. This command creates a <code>cluster.yaml</code> file.</p>
...
This cluster configuration deploys a default cluster in Docker (2 worker nodes and 1 control plane node) with the Kommander addon enabled and the logging stack disabled. For information on using Kommander, see this [documentation](https://docs.d2iq.com/dkp/kommander/1.4/introducing-kommander/).
