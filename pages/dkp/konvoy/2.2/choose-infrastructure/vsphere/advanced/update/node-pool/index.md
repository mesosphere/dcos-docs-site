...
There exist many reasons to update the worker node pool. This topic covers three of the most common:

1. Upgrade the Kubernetes version.
1. Update the machine image.
1. Update the machine hardware.

The worker node pool is described by a MachineDeployment resource, which references immutable vSphereMachineTemplate and KubeadmConfigTemplate resources. This topic explains how to create a new vSphereMachineTemplate and patch the MachineDeployment to update the node pool.
