...
1.  To get a list of names used in your vSphere account, use the _%%% is there a vSphere CLI?_ `aws` [CLI][download_aws_cli]. After downloading, use the following command:

    ```sh
    %%% need actual vSphere command, if one exists
...
1.  To create a cluster name that is unique, use the following command:
    ...
    ```text
    %%% need actual vSphere output, if one exists
...
1.  Ensure your vSphere credentials are up to date. Refresh the credentials:

    ```sh
    %%% does this command or its equivalent exist?
...
1.  As they progress, the controllers also create Events. List the Events using this command:
    ...
    ```text
    %%% need (shorter?) vSphere specific output
