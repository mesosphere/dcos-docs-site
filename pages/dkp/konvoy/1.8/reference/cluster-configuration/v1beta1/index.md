...
| indexType | Determines how to index a node pool. It can be one of the following. `named`: The node pool is referenced using its name. This means one can update or delete the node pool without affecting other node pools. `positional` (DEPRECATED): The node pool is referenced using its position in the node pool list. This means deleting the node pool would affect subsequent positional node pools in the node pool list. This type has been DEPRECATED in favor of `named` node pools. (default: `positional`) | string | false |
...
| Field | Description | Scheme | Required |
| ----- | ----------- | ------ | -------- |
| version | The version of Kubernetes to deploy. (default: `1.20.6`) | string | false |
