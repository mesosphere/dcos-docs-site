---
layout: layout.pug
navigationTitle:  dkp config set-workspace
title:  dkp config set-workspace
menuWeight: 10
excerpt: Set the name of the default Workspace to use in all commands when none is provided
notes: Automatically generated, DO NOT EDIT
enterprise: false
beta: false
---
<!-- vale off -->
<!-- markdownlint-disable -->

## dkp config set-workspace

Set the name of the default Workspace to use in all commands when none is provided

```
dkp config set-workspace [WORKSPACE] [flags]
```

### Options

```
  -h, --help   help for set-workspace
```

### Options inherited from parent commands

```
      --config string            Config file to use (default "/root/.kommander/config")
      --context string           The name of the kubeconfig context to use
      --kubeconfig string        Path to the kubeconfig file to use for CLI requests.
      --request-timeout string   The length of time to wait before giving up on a single server request. Non-zero values should contain a corresponding time unit (e.g. 1s, 2m, 3h). A value of zero means don't timeout requests.
  -v, --verbose int              Output verbosity (default -1)
```

### SEE ALSO

* [dkp config](/dkp/kommander/2.2/cli/dkp/config/)	 - Modify kommander's configuration

