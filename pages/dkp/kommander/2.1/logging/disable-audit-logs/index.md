...
By default, FluentBit pods are configured to collect `systemd` logs from the
...
`fluent-bit` AppDeployment to make FluentBit collect `systemd` logs.

To configure the FluentBit AppDeployment to collect `systemd` logs from a
...
default FluentBit configuration should work) or `/run/log/journal` (typically
used when `systemd-journald` is configured to use a volatile storage).

1. Extract the default Helm values used by the FluentBit App:
...
These items will make Kubernetes mount systemd logs into FluentBit pods.
...
This is needed to make FluentBit actually collect the mounted logs
