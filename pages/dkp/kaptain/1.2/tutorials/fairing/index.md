...
<p class="message--warning" id="WARN"><strong>WARNING: </strong>The Kubeflow Fairing API does <em>not</em> set the Docker image's entrypoint or command.
...
Leave out the `command` in the preprocessor, since Kubeflow Fairing does not set the entrypoint or executable command in the Docker image.
