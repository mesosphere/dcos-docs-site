...
You will see how to do that with Kubeflow Fairing, so that you do not have to leave your favourite notebook environment at all!
...
That shows there are 60,000 28×28 pixel grayscale images.
...
EPOCHS = 5
GPUS = 1
PYTORCH_IMAGE = "mesosphere/kubeflow:mnist-pytorch-1.0.1-0.6.0"  # modify this parameter if you like to use a custom image
```
...
```
    ...
    env: EPOCHS=5
    env: GPUS=1
    env: PYTORCH_IMAGE=mesosphere/kubeflow:mnist-pytorch-1.0.1-0.6.0
...
```
FROM mesosphere/kubeflow:1.0.1-0.6.0-pytorch-1.5.0-gpu
ADD mnist.py /
...
The image is available as `mesosphere/kubeflow:mnist-pytorch-1.0.1-0.6.0` in case you want to skip it for now.
