...
### TensorFlow
The following command copies the data sets to the present working directory.
...
cp ../training/spark/datasets/mnist.npz .
```
...
def get_datasets(buffer_size):
    datasets, ds_info = tfds.load(name="mnist", data_dir="datasets", download=True, with_info=True, as_supervised=True)
...
model = Model(
    ...
    image_name="mesosphere/kubeflow:sdk-demo-mnist-tensorflow-2.2.0",
...
```python
model.train(workers=2, gpus=0, hyperparameters={"--steps": "10", "--epochs": "5"})
```
...
model.tune(
    workers=1, hyperparameters=hyperparams, objectives=["accuracy"], objective_goal=0.99
)
```
...
```
    ...
    [I 201214 11:38:41 font_manager:1337] generated new fontManager
