...
For more information, see: the [Command reference](/1.9/cli/command-reference/).
...
- A container launched by using the [DC/OS Universal container runtime](/1.9/deploying-services/containerizers/).
...
For more information about the `dcos task exec` command, see the CLI command [reference](/1.9/cli/command-reference/).
...
In this example, a long running [job](/1.9/deploying-jobs/) is launched by using the `dcos job run` command, and the `dcos task exec` command is used to launch an interactive Bash shell inside the container of that job.
