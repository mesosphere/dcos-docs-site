...
1. Use secure copy to transfer the new file to your master, as shown below. Before executing the command, replace `<master-IP>` with the IP address of the master.
   ...
   **Tip:** If you used GPG to generate the custom GPG keypair as described in [Reinitializing the Secret Store with a custom GPG keypair](/1.9/security/ent/secrets/custom-key/) and you have multiple masters, use the IP address of the master that you used to generate the keypair.
...
1. If your GPG private key is not already loaded into GPG, go ahead and load it now.
   ...
   **Tip:** If you recently completed the [Reinitializing the Secret Store with your own GPG key](/1.9/security/ent/secrets/custom-key/) procedure, your private key should already be loaded in GPG. 
