...
1.  Verify you have a valid IP detect﻿⁠⁠⁠⁠ script, functioning DNS resolvers to bind the DC/OS services to, and that all nodes are synchronized with NTP.
    ...
    You must have a valid [ip-detect](/mesosphere/dcos/1.9/installing/oss/custom/advanced/) script. You can manually run `ip-detect` on all the nodes in your cluster or check `/opt/mesosphere/bin/detect_ip` on an existing installation to ensure that it returns a valid IP address. A valid IP address does not have:

    - extra lines
    - white space
    - special or hidden characters

    It is recommended that you use the `ip-detect` [examples](/mesosphere/dcos/1.9/installing/oss/custom/advanced/).
...
*   SSH to your master node and enter this command to view the logs from boot time:
 ...
 [1]: /mesosphere/dcos/1.9/installing/oss/custom/configuration/configuration-parameters/#exhibitor-storage-backend
 [2]: https://open.mesosphere.com/reference/mesos-master/
 [3]: /mesosphere/dcos/1.9/installing/oss/custom/configuration/configuration-parameters/#master-discovery
 [4]: /mesosphere/dcos/1.9/overview/architecture/boot-sequence/
 [5]: /mesosphere/dcos/1.9/installing/oss/custom/configuration/configuration-parameters/
