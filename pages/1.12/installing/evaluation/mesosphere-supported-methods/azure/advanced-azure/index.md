...
These inputs/variables include everything from specifying the operating system (CentOS, CoreOS, RHEL) to setting the garbage collection time of your DC/OS cluster.
...
| dcos_adminrouter_tls_cipher_suite | Indicates whether to allow web browsers to send the DC/OS authentication cookie through a non-HTTPS connection. (optional) [enterprise type="inline" size="small" /]| string | `` | no |
| dcos_agent_list | Used to list the agents in the config.yaml (optional) | string | `` | no |
| dcos_audit_logging | Enable security decisions are logged for Mesos, Marathon, and Jobs. (optional) [enterprise type="inline" size="small" /]| string | `` | no |
| dcos_auth_cookie_secure_flag | Allow web browsers to send the DC/OS authentication cookie through a non-HTTPS connection. (optional) [enterprise type="inline" size="small" /] | string | `` | no |
...
| dcos_bouncer_expiration_auth_token_days | Sets the auth token time-to-live (TTL) for Identity and Access Management. (optional) [enterprise type="inline" size="small" /] | string | `` | no |
| dcos_ca_certificate_chain_path | Path (relative to the $DCOS_INSTALL_DIR) to a file containing the complete CA certification chain required for end-entity certificate verification, in the OpenSSL PEM format. (optional) [enterprise type="inline" size="small" /] | string | `` | no |
| dcos_ca_certificate_key_path |  | string | `` | no |
| dcos_ca_certificate_path | Path (relative to the $DCOS_INSTALL_DIR) to a file containing a single X.509 CA certificate in the OpenSSL PEM format. (optional) [enterprise type="inline" size="small" /]| string | `` | no |
...
| dcos_customer_key | Sets the customer key. (optional) [enterprise type="inline" size="small" /]| string | `` | no |
...
| dcos_fault_domain_detect_contents | Fault domain script contents. Optional but required if no `fault-domain-detect` script present. [enterprise type="inline" size="small" /]| string | `` | no |
| dcos_fault_domain_enabled | Used to control if fault domain is enabled. [enterprise type="inline" size="small" /]| string | `` | no |
...
| dcos_license_key_contents | Used to privide the license key of DC/OS for Enterprise Edition. Optional if `license.txt` is present on bootstrap node. [enterprise type="inline" size="small" /] | string | `` | no |
...
| dcos_master_external_loadbalancer | Allows DC/OS to configure certs around the external load balancer name. If not used SSL verfication issues will arrise. (recommended) [enterprise type="inline" size="small" /]| string | `` | no |
...
| dcos_security | Set the security level of DC/OS. Default is permissive. (recommended) [enterprise type="inline" size="small" /]| string | `` | no |
...
| dcos_superuser_password_hash | Set the superuser password hash. (recommended) [enterprise type="inline" size="small" /]| string | `` | no |
| dcos_superuser_username | Set the superuser username. (recommended) [enterprise type="inline" size="small" /]| string | `` | no |
...
| dcos_zk_agent_credentials | Set the ZooKeeper agent credentials. (recommended) [enterprise type="inline" size="small" /]| string | `` | no |
| dcos_zk_master_credentials | Set the ZooKeeper master credentials. (recommended) [enterprise type="inline" size="small" /]| string | `` | no |
| dcos_zk_super_credentials | Set the ZooKeeper super credentials. (recommended) [enterprise type="inline" size="small" /] | string | `` | no |
