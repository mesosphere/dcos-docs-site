---
layout: layout.pug
title: Configuring DC/OS on AWS using the Universal Installer
excerpt:  Customization Guide for DC/OS on AWS using the Universal Installer
...
| bootstrap_associate_public_ip_address | [BOOTSTRAP] Associate a public ip address with their instances. | string | `true` | no |
...
| dcos_auth_cookie_secure_flag | [Enterprise DC/OS] Allows web browsers to send the DC/OS authentication cookie through a non-HTTPS connection. (optional) | string | `` | no |
...
| dcos_cluster_docker_credentials_enabled | Indicates whether to pass the `--docker_config` option to Mesos. (optional) | string | `` | no |
...
| dcos_master_external_loadbalancer | [Enterprise DC/OS] Allows DC/OS to configure certs around the External Load Balancer name. If not used SSL verification issues will arise. (recommended) | string | `` | no |
...
| private_agents_associate_public_ip_address | [PRIVATE AGENTS] Associate a public ip address with these instances | string | `true` | no |
