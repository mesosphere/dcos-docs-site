...
1.  Install DC/OS using the [custom advanced installation instructions](/1.12/installing/oss/custom/advanced/). Here are the GPU-specific configuration parameters:
    ...
    For more information, see the [configuration parameter documentation](/1.12/installing/oss/custom/configuration/configuration-parameters/#enable-gpu-isolation) and Mesos [Nvidia GPU Support documentation](http://mesos.apache.org/documentation/latest/gpu-support/#external-dependencies).

## AWS EC2 DC/OS Installation with GPUs

###  Prerequisites
- The AWS DC/OS advanced template [system requirements](/1.12/installing/production/system-requirements/).
- The `zen.sh` script copied to your local machine. The script and instructions are [here](/1.12/installing/oss/cloud/aws/advanced/).
...
1. Follow the instructions [here](/1.12/installing/oss/cloud/aws/advanced/) to create a cluster with advanced AWS templates, using the following GPU-specific configuration.
...
-  NVIDIA GPU support is only available for tasks launched using the [DC/OS Universal container runtime](/1.12/deploying-services/containerizers/).
...
In this example, an app is deployed with GPUs that specifies a Docker container and the [DC/OS Universal Container Runtime (UCR)](/1.12/deploying-services/containerizers/) (container type to `MESOS`).
