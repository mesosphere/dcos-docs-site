...
- COPS-4044, DCOS_OSS-4469 - With this release, the default value of zero (0) for the Docker GC environment variable `LOG_TO_SYSLOG` writes log messages to the `journald` logging facility on the host system. You can change the logging facility and level of detail recorded with the `$SYSLOG_FACILITY` and `$SYSLOG_LEVEL` environment variables. 

    However, you should not configure the LOG_TO_SYSLOG environment variable to one (1), which uses the `systemd` logging facility. Setting this environment variable to one writes information to the `syslog` facility inside the container. The container and logged information is destroyed when the GC operation is done.
...
You can restart the `systemd` process on the nodes affected to restore proper netowrk connectivity. 
...
- MARATHON-8413 - Versioning information for apps and pods in Java9 includes a timestamp in nanoseconds if the `Instant.now()` function is called. Previously, nanoseconds were not included in the version information format that was retreived for an applications and pods.
...
- DCOS-43591 - You can add labels to the metrics configuration file to define a "whitelist" for grouping metrics according to the metadata you specify. For example, by adding the `DCOS_SERVICE_NAME` label, you cam group the metrics for a specific scheduler.
...
## Security
[/enterprise]
DCOS-21998, DCOS-44367 - You can install DC/OS cluster nodes with custom RSA-based CA certificates that are signed using an Elastic Cloud (EC) based private key. Previously, a custom CA certificate signed using a trusted EC-based provate key would generate a transport security layer (TLS) security alert.
...
- Expanded DC/OS upgrade paths enable Mesosphere to skip specific [upgrade paths](https://docs.mesosphere.com/1.12/installing/production/upgrading/#supported-upgrade-paths) within a supported patch version of DC/OS (i.e upgrade from 1.11.1 => 1.11.5 in one move) and to skip upgrade paths between supported major to major versions of DC/OS (for example, enabling you to upgrade from 1.10.7 to 1.11.4 in one move).
