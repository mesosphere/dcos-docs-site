...
You must be a superuser or have the appropriate cluster link [permission](/1.14/security/ent/perms-reference/#cluster-linker) to view, add, and remove links and grant permissions to view linked clusters.
...
To view all linked clusters, run the `dcos cluster list` command. If a cluster was linked, but not set up, its status is `UNCONFIGURED`. If a cluster is linked and attached, its status is `AVAILABLE`. Also see [Viewing Connected Clusters](/1.14/administering-clusters/multiple-clusters/cluster-connections/).
