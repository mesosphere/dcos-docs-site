...
You may want to manually seal the Secret Store to protect its contents from an intruder. Sealed Secret Stores cannot be accessed from the web interface. Secret values cannot be retrieved using the [Secrets API](/1.14/security/ent/secrets/secrets-api/). Services that depend on values in the Secret Store may fail to deploy.
...
The intended status of the seal is persisted; after sealing the store, the restart of `dcos-secrets` will not unseal it. Only the steps depicted in [unsealing the store](/1.14/security/ent/secrets/unseal-store/) will do so.
