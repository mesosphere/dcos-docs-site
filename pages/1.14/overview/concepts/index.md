...
The [DC/OS graphical user interface (GUI)](/1.14/gui/) is an interface for remotely controlling and managing a DC/OS cluster from a web browser. The GUI is also sometimes called the DC/OS UI or DC/OS web interface.

<a name="dcos-cli"></a>

## DC/OS CLI

The [DC/OS command line interface (CLI)](/1.14/cli/) is an interface for remotely controlling and managing a DC/OS cluster from a terminal.
...
For more information, see [Network Security](/1.14/administering-clusters/) and [Adding Agent Nodes](/1.14/administering-clusters/add-a-node/).
...
- Public agent nodes are used primarily for externally facing reverse proxy load balancers, like [Marathon-LB](/services/marathon-lb/latest/) or [Edge-LB](/services/edge-lb/1.3/).
...
For more information, see [Converting Agent Node Types](/1.14/administering-clusters/convert-agent-type/).
...
- One method of allowing master nodes to change IPs involves running ZooKeeper with Exhibitor on the bootstrap machine. Other alternatives include using S3, DNS, or static IPs, with various tradeoffs. For more information, see the [exhibitor storage backend](/1.14/installing/production/advanced-configuration/configuration-reference/#exhibitor-storage-backend) parameter description.
- If a bootstrap machine is not required for managing master node IP changes or as an SSH jumpbox, it can be shut down after bootstrapping and spun up on demand to [add new nodes](/1.14/administering-clusters/add-a-node/) to the cluster.

For more information, see the [system requirements](/1.14/installing/production/system-requirements/).
...
For more information about deploying using a cloud template, see [Mesosphere installation methods](/1.14/installing/evaluation/).
...
For more information, see the [Mesos-DNS documentation](/1.14/networking/DNS/mesos-dns/).
