...
The default Linux user of a service or job can vary according to the security mode and the container type. See [Linux users](/mesosphere/dcos/1.11/security/ent/#linux-users) for more information.
...
Remember to grant permission to perform the `create` action on the `dcos:mesos:master:task:user[:<linux-user-name>]` resource to the service account user that the Universe service is launched with. See [Mesos Permissions](/mesosphere/dcos/1.11/security/ent/perms-reference/#mesos-permissions) for more information.

# <a name="marathon-app-def"></a>Overriding the default Linux user via Marathon app definition

Marathon app definitions provide a `"user"` key which you can use to override the default Linux user. **Tip:** Reference the [Marathon documentation](/mesosphere/dcos/1.11/deploying-services/creating-services/) for more details on writing Marathon services.
...
- You have installed and are logged into the [DC/OS CLI](/mesosphere/dcos/1.11/cli/).
...
1. Deploy the service using the [Marathon API](/mesosphere/dcos/1.11/deploying-services/marathon-api/).
...
**Note:** Refer to the [Jobs documentation](/mesosphere/dcos/1.11/deploying-jobs/quickstart/) for more information about creating and deploying jobs.
