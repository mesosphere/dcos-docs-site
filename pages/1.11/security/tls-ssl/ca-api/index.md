...
To get an authentication token, pass the user name and password of a user with the necessary permissions in the body of a request to the `/auth/login` endpoint of the [Identity and Access Management Service API](/1.11/security/iam-api/). It returns an authentication token as shown below.
...
Authentication tokens expire after five days by default. If your program needs to run longer than five days, you will need a service account. Please see [Provisioning custom services](/1.11/security/service-auth/custom-service-auth/) for more information.
  
# API reference

[api-explorer api='/1.11/api/certificate-authority.yaml']
