...
See [Configuring services and pods to use secrets](/1.11/security/secrets/use-secrets/) for information on how to reference these secrets in your app or pod definition.
...
Secrets should include paths, unless you want to allow all services to access its value. See [Spaces](/1.11/security/#spaces) for more information about secret paths.
...
### [DC/OS CLI](/1.11/cli/) or [Secrets API](/1.11/security/secrets/secrets-api/)

- See [Secret Store Permissions](/1.11/security/perms-reference/#secrets) for the permissions needed to create secrets from the CLI or API. The permissions you configure must include the name of the secret the user is allowed to create. You must have one permission per secret. The secret name and permission name must match.

- [DC/OS CLI installed](/1.11/cli/install/) and the [Enterprise DC/OS CLI installed](/1.11/cli/enterprise-cli/#ent-cli-install).
...
1. In the **ID** box, provide the name of your secret and its path, if any. Example, **developer/my-secret**.
    ...
    ![Secret ID/Value Fields](/1.11/img/secret-id-value.png)
...
1. When you have completed your entries, the secret should look something like the following.
    ...
    ![Creating the Secret](/1.11/img/create-secret.png)
...
**Note:** If your [security mode](/1.11/installing/custom/configuration/configuration-parameters/#security-enterprise-dcos-only) is `permissive` or `strict`, you must follow the steps in [Obtaining the DC/OS CA bundle](/1.11/networking/tls-ssl/get-cert/) before issuing the curl commands in this section. If your [security mode](/1.11/installing/custom/configuration/configuration-parameters/#security-enterprise-dcos-only) is `disabled`, you must delete `--cacert dcos-ca.crt` from the commands before issuing them.
