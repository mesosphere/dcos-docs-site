...
**Note:** The overlay replicated log is different from the [master's replicated log](http://mesos.apache.org/documentation/latest/replicated-log-internals/), which is stored at `/var/l/var/lib/dcos/mesos/master/replicated_log`. Removing the overlay replicated log will have no effect on the master's recovery semantics.
