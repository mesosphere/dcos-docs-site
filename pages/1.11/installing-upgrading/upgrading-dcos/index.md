...
- [`dns_search`](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#dns-search)
- [`docker_remove_delay`](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#docker-remove-delay)
- [`gc_delay`](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#gc-delay)
- [`resolvers`](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#resolvers)
- [`telemetry_enabled`](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#telemetry-enabled)
- [`use_proxy`](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#use-proxy)
    - [`http_proxy`](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#use-proxy)
...
See the security [mode](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#security-enterprise) for a description of the different security modes and what each means.
...
- Optional: You can add custom [node and cluster healthchecks](/1.11/installing-upgrading/custom/node-cluster-health-checks/#custom-health-checks) to your `config.yaml`.
...
This procedure upgrades a DC/OS 1.10 cluster to DC/OS 1.11 without changing the cluster's [security mode](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#security-enterprise).
...
1.  Merge the old `config.yaml` into the new `config.yaml` format. In most cases the differences will be minimal.
    ...
    *  The syntax of the `config.yaml` may be different from the earlier version. For a detailed description of the current `config.yaml` syntax and parameters, see the [documentation](/1.11/installing-upgrading/custom/configuration/configuration-parameters).
...
This procedure upgrades to DC/OS 1.11 in [permissive security mode](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#security-enterprise).

**Prerequisite:**

- Your cluster must be [upgraded to DC/OS 1.11](#current-security) and running in [disabled security mode](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#security-enterprise) before it can be upgraded to permissive mode. If your cluster was running in permissive mode before it was upgraded to DC/OS 1.10, you can skip this procedure.

**Important:** Any [custom node or cluster healthchecks](/1.11/installing-upgrading/custom/node-cluster-health-checks/#custom-health-checks) you have configured will fail for an upgrade from disabled to permissive security mode. A future release will allow you to bypass the healthchecks.
...
This procedure upgrades to DC/OS 1.11 in security strict [mode](/1.11/installing-upgrading/custom/configuration/configuration-parameters/#security-enterprise).
...
If your upgrade fails because of a [custom node or cluster check](/1.11/installing-upgrading/custom/node-cluster-health-checks/#custom-health-checks), run these commands for more details:
```bash
