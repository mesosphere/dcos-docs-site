...
- DCOS_OSS-4040 - The `dcos-diagnostic` bundle files are stored in `/var/lib/dcos/dcos-diagnostics/diag-bundles` and `/var/lib/dcos` is not a separate partition. It belongs to the root directory and there are many other critical data in the same directory on the masters, which could seriously damage a cluster if diagnostics bundle takes most of the space. The workaround is to move the destination directory (read volume) from root partition to a different partition than all these other directories which allows `dcos-diagnostics` bundles location to be configured. 
...
## Security
[/enterprise]
- DCOS_OSS-4742 - Currently, if the `validate_success` or `validate_error` function fails during gen unit tests; the user is not aware of the full output on why the assert failed. This release adds proper `AssertionError` reporting to resolve this issue. 
