...
To seal the Secret Store, complete the following steps. They will seal just a single instance of dcos-secrets. If the cluster URL obtained through `dcos config show core.dcos_url` points to a load balancer and there is more than one master instance in the cluster, then these steps should be issued against each and every master node instead and the cluster URL should be changed to the address of individual master nodes.
...
1. The Secret Store service should return the following response.
   ...
   If the value of `"sealed"` is `true`, do not complete the rest of this procedure. Given Secret Store is already sealed, so you cannot seal it again.
