...
- From the latest GA version of previous to the latest GA version of current. For example, if 1.8.8 is the latest and 1.9.0 is the latest, this upgrade would be supported.
    ...
    - [1.8 to 1.9](/1.10/administration/upgrading/)
    - [1.9 to 1.10](/1.10/administration/upgrading/)
...
- [`check_time`](/1.10/installing/custom/configuration/configuration-parameters/#check-time)
- [`dns_search`](/1.10/installing/custom/configuration/configuration-parameters/#dns-search)
- [`docker_remove_delay`](/1.10/installing/custom/configuration/configuration-parameters/#docker-remove)
- [`gc_delay`](/1.10/installing/custom/configuration/configuration-parameters/#gc-delay)
- [`resolvers`](/1.10/installing/custom/configuration/configuration-parameters/#resolvers)
- [`telemetry_enabled`](/1.10/installing/custom/configuration/configuration-parameters/#telemetry-enabled)
- [`use_proxy`](/1.10/installing/custom/configuration/configuration-parameters/#use-proxy)
    - [`http_proxy`](/1.10/installing/custom/configuration/configuration-parameters/#use-proxy)
    - [`https_proxy`](/1.10/installing/custom/configuration/configuration-parameters/#use-proxy)
    - [`no_proxy`](/1.10/installing/custom/configuration/configuration-parameters/#use-proxy)
...
See the security [mode](/1.10/installing/custom/configuration/configuration-parameters/#security-enterprise-dcos-only) for a description of the different security modes and what each means.
...
- Optional: You can add custom [node and cluster healthchecks](/1.10/installing/custom/node-cluster-health-check/#custom-health-checks) to your `config.yaml`.
...
This procedure upgrades a DC/OS 1.9 cluster to DC/OS 1.10 without changing the cluster's [security mode](/1.10/installing/custom/configuration/configuration-parameters/#security-enterprise-dcos-only).
...
1.  Merge the old `config.yaml` into the new `config.yaml` format. In most cases the differences will be minimal.
    ...
    *  The syntax of the `config.yaml` may be different from the earlier version. For a detailed description of the current `config.yaml` syntax and parameters, see the [documentation](/1.10/installing/custom/configuration/configuration-parameters/).
...
This procedure upgrades to DC/OS 1.10 in [permissive security mode](/1.10/installing/custom/configuration/configuration-parameters/#security-enterprise-dcos-only).

**Prerequisite:**

- Your cluster must be [upgraded to DC/OS 1.10](#current-security) and running in [disabled security mode](/1.10/installing/custom/configuration/configuration-parameters/#security-enterprise-dcos-only) before it can be upgraded to permissive mode. If your cluster was running in permissive mode before it was upgraded to DC/OS 1.10, you can skip this procedure.

**Important:** Any [custom node or cluster healthchecks](/1.10/installing/custom/node-cluster-health-check/#custom-health-checks) you have configured will fail for an upgrade from disabled to permissive security mode. A future release will allow you to bypass the healthchecks.
...
This procedure upgrades to DC/OS 1.10 in security strict [mode](/1.10/installing/custom/configuration/configuration-parameters/#security-enterprise-dcos-only).
...
If your upgrade fails because of a [custom node or cluster check](/1.10/installing/custom/node-cluster-health-check/#custom-health-checks), run these commands for more details:
```bash
...
[advanced-install]: /1.10/installing/custom/advanced/
