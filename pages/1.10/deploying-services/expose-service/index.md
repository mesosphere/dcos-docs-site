...
* DC/OS is [installed](/mesosphere/dcos/1.10/installing/)
* DC/OS CLI is [installed](/mesosphere/dcos/1.10/cli/install/)
...
1.  Create a Marathon app definition with the required `"acceptedResourceRoles":["slave_public"]` parameter specified. For example:
    ...
    For more information about the `acceptedResourceRoles` parameter, see the Marathon REST API [documentation](/mesosphere/dcos/1.10/deploying-services/marathon-api/).
...
1.  Add your app to Marathon by using this command, where `myApp.json` is the file containing your Marathon app definition.
     ...
     **Tip:** You can also add your app by using the **Services** tab of DC/OS [GUI](/mesosphere/dcos/1.10/gui/#services).
...
1.  Verify that the app is added with this command:
    ...
    **Tip:** You can also view deployed apps by using the **Services** tab of DC/OS [GUI](/mesosphere/dcos/1.10/gui/#services).

1.  Configure an edge load balancer and service discovery mechanism.

    - AWS users: If you installed DC/OS by using the [AWS CloudFormation templates](/mesosphere/dcos/1.10/installing/evaluation/), an ELB is included. However, you must reconfigure the health check on the public ELB to expose the app to the port specified in your app definition (such as port 80).
