...
render: mustache
---
1.  Before submitting your job, upload the artifact, such as a `jar` file, to a location visible to the cluster (for examples,  HTTP, S3, or HDFS). [Learn more](13).

1.  Run the job.
    Include all configuration flags before the `jar` url and the args for your {{ model.techShortName }} job after the `jar` url. Generally following the template `dcos spark run --submit-args="<flags> URL [args]` where `<flags>` can be things like `--conf spark.cores.max=16` and `--class my.aprk.App`, `URL` is the location of the application, and `[args]` are any arguments for the application.
...
{{ model.techShortName }} job settings are controlled by configuring [Spark properties][14].
...
To reuse {{ model.techShortName }} properties without cluttering the command line, the CLI supports passing a path to a local file containing {{ model.techShortName }} properties. Such a file is whitespace separated properties and values.
...
[Using the Secret Store](/services/spark/2.3.1-2.2.1-2/security/#using-the-secret-store) and
[Using Mesos Secrets](/services/spark/2.3.1-2.2.1-2/security/#using-mesos-secrets).

# DC/OS Overlay Network

To submit a {{ model.techShortName }} job inside the [DC/OS Overlay Network][16]:
...
Note that DC/OS Overlay support requires the [UCR][17], rather than the default Docker Containerizer, so you must set `--conf spark.mesos.containerizer=mesos`.
...
**Note:** If you kill a job before it finishes, the framework will persist as an `inactive` framework in Mesos for a period equal to the failover timeout. You can manually tear down the framework before that period is over by hitting
...
DC/OS {{ model.techName }} distributions 1.X are compiled with Scala 2.10.  DC/OS Apache Spark distributions 2.X are compiled with Scala 2.11.  Scala is not binary compatible across minor verions, so your Spark job must be compiled with the same Scala version as your version of DC/OS {{ model.techName }}.

The default DC/OS {{ model.techName }} distribution is compiled against Hadoop 2.6 libraries.  However, you may choose a different version by following the instructions in the [Customize Spark Distribution] (http://localhost:3000/services/spark/2.3.1-2.2.1-2/install/#customize-spark-distribution) section of the Installation page.
