...
```shell
dcos package uninstall --cli percona-mongo
dcos package install --cli percona-mongo -package-version="0.3.0-3.6.5"
```
1. Once the CLI subcommand has been updated, call the update start command, passing in the version. For example, to update DC/OS Percona Server for MongoDB Service to version `0.3.0-3.6.5`:
```shell
dcos percona-mongo update start --package-version="0.3.0-3.6.5"
```
...
```shell
dcos percona-mongo update start --options=options.json --package-version="0.3.0-3.6.5"
```
