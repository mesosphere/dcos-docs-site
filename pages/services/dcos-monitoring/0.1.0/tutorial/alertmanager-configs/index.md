...
Instead of putting plain text secrets in the configuration file, you can save the secrets in the secret store and refer to the secrets in the configuration file using special templates.

Currently, the following secrets in the configuration file are supported:

### Slack API URL

You might refer the Slack API URL (which is a secret) using a special template like the following:

```yaml
receivers:
- name: slack_general
  slack_configs:
  - api_url: '{{ template "secrets.slack_api_url" . }}'
```
