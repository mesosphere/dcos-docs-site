...
<tr> 
  <td align=justify style=color:white><strong>Important:</strong> You must specify all configuration values in the "options.json" file when performing a configuration update. Any unspecified values will be reverted to the default values specified by the DC/OS service. See the "Recreating options.json" section below for information on recovering these values.</td> 
</tr> 
</table>
...
The service deploys 3 nodes by default, as 3 nodes is the minimum node requirement for a Highly-Available [MongoDB Replica Set](https://docs.mongodb.com/manual/replication/). 

<table class=“table” bgcolor=#7d58ff>
<tr> 
  <td align=justify style=color:white><strong>Note:</strong> Only 1 (not recommended), 3, 5 or 7 nodes are recommended and supported.</td> 
</tr> 
</table>
...
<table class=“table” bgcolor=#7d58ff>
<tr> 
  <td align=justify style=color:white><strong>Note:</strong> Volume requirements (type and/or size) cannot be changed after initial deployment.</td> 
</tr> 
</table>
...
Run `dcos percona-server-mongodb pod restart mongo-rs-<NUM>`, e.g. `mongo-rs-2`.
...
<table class=“table” bgcolor=#858585>
<tr> 
  <td align=justify style=color:white><strong>Important:</strong> Nodes are not moved automatically. You must perform the following steps manually to move nodes to new systems. You can automate node replacement according to your own preferences.
</td> 
</tr> 
</table>
...
```shell
dcos package uninstall --cli percona-server-mongodb
dcos package install --cli percona-server-mongodb -package-version="0.4.1-3.6.8"
```
1. Once the CLI subcommand has been updated, call the update start command, passing in the version. For example, to update DC/OS Percona Server for MongoDB Service to version `0.4.1-3.6.8`:
```shell
dcos percona-server-mongodb update start --package-version="0.4.1-3.6.8"
```
...
```shell
dcos percona-server-mongodb update start --options=options.json --package-version="0.4.1-3.6.8"
```
