...
The LVM volume plugin integrates the LVM2 CSI plugin with DC/OS. When you create a new volume provider you can specify the ‘plugin’ field as ‘lvm’ in order to create a volume provider whose storage is backed by LVM2. Such a volume provider is called a ‘lvm’ volume provider.
...
Example volume provider configuration:

The spec.plugin field is set to ‘lvm’.
The node field is set to the ID of the Mesos agent running on the server.
The plugin-configuration field specifies the plugin-specific configuration given in this section. In this case the devices field declares that the /dev/xvdb and /dev/xvdc devices on the server corresponding to node should form part of a LVM2 volume group called ‘volume-group-1’.
```json
{
    "name": "volume-group-1",
