...
The service link for Kibana in the **Services** DC/OS web interface tab points to `<your-cluster-URL>/service/<kibana-service-name>`, which is initially broken with a redirect loop. To access Kibana when X-Pack is enabled, first authenticate on `<your-cluster-URL>/service/<kibana-service-name>/login`. That will redirect you to the Kibana UI, and after that the DC/OS service link will work.
