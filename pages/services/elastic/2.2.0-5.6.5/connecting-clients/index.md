...
1. Store and retrieve data.
  ...
  ```bash
  curl -s -u elastic:changeme -XPUT 'coordinator.{{ model.serviceName }}.l4lb.thisdcos.directory:9200/customer/external/1?pretty' -d '
  {
    "name": "John Doe"
  }'
  curl -s -u elastic:changeme -XGET 'coordinator.{{ model.serviceName }}.l4lb.thisdcos.directory:9200/customer/external/1?pretty'
  ```

**Note:** If you did not install any coordinator nodes, you should direct all queries to your data nodes instead:

  ```bash
  curl -s -u elastic:changeme 'data.{{ model.serviceName }}.l4lb.thisdcos.directory:9200/_cat/nodes?v'
  ```
...
The default framework name is `{{ model.serviceName }}`. If you customized the name your framework, you need to adjust the service address accordingly.
...
- Elasticsearch URL: This is a required configuration parameter. The default value `http://coordinator.{{ model.serviceName }}.l4lb.thisdcos.directory:9200` corresponds to the named VIP that exists when the Elastic package is launched with its own default configuration.
