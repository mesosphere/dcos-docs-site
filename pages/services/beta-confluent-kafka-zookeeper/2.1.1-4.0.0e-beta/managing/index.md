...
To make configuration changes via scheduler environment updates, perform the following steps (documented in full [here](https://docs.mesosphere.com/1.9/deploying-services/config-universe-service/):
...
```
dcos beta-confluent-kafka-zookeeper update start --options=options.json
```

7. To check the status of the update, run:

```
dcos beta-confluent-kafka-zookeeper update status
```
...
1. Remove the decommissioned IP and add a new IP to the placement rule whitelist by editing `NODE_PLACEMENT`:
    ...
    ```
2. Redeploy `zookeeper-1-server` from the decommissioned node to somewhere within the new whitelist: `dcos beta-confluent-kafka-zookeeper pods replace zookeeper-1`.
...
1. Run `dcos beta-confluent-kafka-zookeeper pods restart zookeeper-<NUM>`, e.g. `zookeeper-2`.
