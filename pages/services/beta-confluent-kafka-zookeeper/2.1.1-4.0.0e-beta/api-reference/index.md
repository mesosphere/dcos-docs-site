...
If you are using Enterprise DC/OS, the security mode of your installation may also require the `--cacert` flag when making REST calls. Refer to [Obtaining and passing the DC/OS certificate in cURL requests](https://docs.mesosphere.com/1.10/security/tls-ssl/#get-dcos-cert) for information on how to use the `--cacert` flag. If your security mode is disabled, do not use the `--ca-cert` flag.
...
```shell
curl -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/plans/
```

### CLI example

```shell
dcos beta-confluent-kafka-zookeeper plan list
```
...
```shell
curl -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/plans/deploy
```

### CLI example

```shell
dcos beta-confluent-kafka-zookeeper plan status deploy
```
...
```shell
curl -X POST -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/plans/deploy/start
```

### CLI example

```shell
dcos beta-confluent-kafka-zookeeper plan start deploy
```
...
```shell
curl -X POST -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/plans/deploy/interrupt
```

### CLI example

```shell
dcos beta-confluent-kafka-zookeeper plan pause deploy
```
...
```shell
curl -X PUT -H "Authorization:token=$auth_token" <dcos_surl>/service/confluent-kafka-zookeeper/v1/plans/deploy/continue
```

### CLI example

```shell
dcos beta-confluent-kafka-zookeeper plan resume deploy
```
...
```shell
curl -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/endpoints/
```
...
```shell
curl -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/endpoints/clientport
```
...
```shell
curl  -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/pod
```

### CLI Example

```shell
dcos beta-confluent-kafka-zookeeper pod list
```
...
```shell
curl  -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/pod/<node-id>/info
```

### CLI Example

```shell
dcos beta-confluent-kafka-zookeeper pod info zookeeper-0
```

HTTP Example

```shell
curl  -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/pod/zookeeper-0/info
```
## Replace a Node
...
```shell
curl -X POST -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/pod/<node-id>/replace
```

### CLI Example

```shell
dcos beta-confluent-kafka-zookeeper pod replace <node-id>
```
...
```shell
curl -X POST -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/pod/<node-id>/restart
```

### CLI Example

```shell
dcos beta-confluent-kafka-zookeeper pod restart <node-id>
```
...
```shell
curl -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/configurations/target
```

### CLI Example

```shell
dcos beta-confluent-kafka-zookeeper config target
```
...
```shell
curl -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/configurations
```

### CLI Example

```shell
dcos beta-confluent-kafka-zookeeper config list
```
...
```shell
curl -H "Authorization:token=$auth_token" <dcos_url>/service/confluent-kafka-zookeeper/v1/configurations/9a8d4308-ab9d-4121-b460-696ec3368ad6
```

### CLI Example

```shell
dcos beta-confluent-kafka-zookeeper config show 9a8d4308-ab9d-4121-b460-696ec3368ad6
```
...
```shell
curl -H "Authorization: token=$auth_token" "<dcos_url>/service/confluent-kafka-zookeeper/v1/state/properties/suppressed"
```

### CLI example

```shell
dcos beta-confluent-kafka-zookeeper debug state property suppressed
```
