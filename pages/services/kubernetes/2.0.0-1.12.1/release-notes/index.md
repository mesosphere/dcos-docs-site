...
* DC/OS Kubernetes `2.0.0-1.12.1` requires DC/OS 1.12.
* DC/OS Kubernetes `2.0.0-1.12.1` introduces breaking changes to the way the package works and is deployed.
  Therefore, it is not possible to upgrade an existing installation of DC/OS Kubernetes to `2.0.0-1.12.1`.
* Before installing `kubernetes-cluster` package `2.0.0-1.12.1`, the `kubernetes` package must be [installed and running](../install-and-customize).
