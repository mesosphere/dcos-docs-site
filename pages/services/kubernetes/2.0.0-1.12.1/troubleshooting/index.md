...
If it appears that there are enough resources available, verify there are no existing quotas for that installation or if there are, that it contains correct values.  

See [resources section](../install-and-customize#resources) of the "Install and Customize" page for more detail setting up your cluster.

## Scheduler is restarting

If the scheduler task is starting for your Kubernetes cluster but is in a "Failed" restart loop, verify all of the installation [prerequisites](../install-and-customize#prerequisites) have been met.
This might include improperly configured Service Account and/or Service Account secret or other issues.

The simplest way is to look at the Kubernetes cluster's scheduler logs for any error messages, below are a few examples of what you might see.
...
## Scheduler started but other tasks aren't being launched

If the scheduler is running but no other tasks are being launched and the DC/OS cluster is running in strict mode, verify that the service account was [created correctly](../install-and-customize).
...
In this case use the the `dcos kubernetes cluster debug pod` [CLI operations](../operations#debug-pod-1) to assist in debugging and recovering failed, flapping or otherwise misbehaving tasks.
...
**Note:** take precautions when dealing with unhealthy etcd tasks, see the [etcd section below](#troubleshooting-etcd) for more detail.
...
First drain one of the impacted Kubernetes nodes, forcing Kubernetes to reschedule any running pods onto other nodes:

```shell
kubectl drain <node name>
```

**Note:** Depending on the running workloads, you may need to include the flags `--delete-local-data` and/or `--ignore-daemonsets` when running the above command.

Next, `replace` the corresponding DC/OS pod that is running on the to be decommissioned agent, this will schedule the pod onto a different agent:

```shell
dcos kubernetes cluster debug pod replace <pod> --cluster-name <cluster-name>
```

After the replaced DC/OS pod is in `Running` state and the Kubernetes node's status is `Ready,SchedulingDisabled`, it is safe to uncordon the node, allowing for workloads to be scheduled onto it once again:

```shell
kubectl uncordon <node name>
```

**Note:** Kubernetes will not actively rebalance the existing workloads onto the newly uncordoned node, however any future pods will be scheduled there.

Repeat for all other kube nodes and DC/OS tasks that are scheduled on the same DC/OS agent replacing `<node name>`, `<pod>` and `<cluster-name>` with the appropriate values.

Finally, once all of the tasks have been successfully moved to different agents, it is safe to decommission the DC/OS agent:
...
In the case when HA mode is enabled, it is safe to run the below command and move the kube-control-plane task to a different agent:
...
Once you have confirmed that is the case, run the below command to move the etcd task to a different agent:
...
For instance, if an `etcd` process that has been added to an existing cluster crashes before actually being able to establish a connection with the other members, the cluster may become unstable or, under some circumstances, inoperable.
This document presents a number of best-practices that help reducing the chances of permanently losing an `etcd` cluster and the associated data. It is recommended to be familiar with the [Disaster Recovery](../disaster-recovery) document before reading this document.
...
1. **(Scenario 1)** When first creating the cluster (i.e. when first installing DC/OS Kubernetes);
1. **(Scenario 2)** When scaling up the cluster (i.e. when updating the value of `kubernetes.high_availability`);
1. **(Scenario 3)** When `kubernetes.high_availability` is `false` and the single member crashes (e.g. when the DC/OS agent where the member is running permanently fails);
1. **(Scenario 4)** When `kubernetes.high_availability` is `true` and one or more members crash (e.g. when there is a major outage of the DC/OS cluster).
...
Before re-installing DC/OS Kubernetes, make sure that all DC/OS agents are healthy, and that the DC/OS cluster itself is healthy (e.g. that there are no networking problems) and meets all the [pre-requisites](../install-and-customize).
...
To prevent this situation (but be prepared in case it arises), you can take a number of preventive measures.
...
Run the following command (replacing the `<SERVICE_NAME>` placehold as appropriate) to create a snapshot:
...
You should then fetch the `etcd-0-peer.db` file from inside the `etcd-0-peer` task and store it in a safe place (i.e. in their workstation or cloud storage).
To fetch the `etcd-0-peer.db` file from inside the `etcd-0-peer` task use the DC/OS UI or manually `scp` from the DC/OS agent.
...
The DC/OS Kubernetes cluster as a whole may experience some instability, and some other tasks (e.g. `kube-control-plane-0-instance`) may also restart, but existing `etcd` data will be safe.
...
For this reason, it is **STRONLY ADVISED** to periodically backup the DC/OS Kubernetes cluster(s) using `dcos kubernetes cluster backup --cluster-name=CLUSTER-NAME`, and to avoid running production workloads in a cluster where `kubernetes.high_availability` is set to `false`.
...
In this scenario, simply cleanup and re-launch a failed `etcd` member in a different DC/OS agent by running the following command (replacing `<SERVICE_NAME>` and `<etcd-task-name>` as appropriate):
