...
Once [the Kubernetes API is exposed correctly and kubectl setup](../connecting-clients), you will be able to access the Kubernetes Dashboard by running:
...
When accessed, and depending on whether you are running DC/OS Open or DC/OS EE (as well as on your browser's configuration) you may be presented a warning indicating that the TLS certificate being used by the Kubernetes Dashboard is not trusted.
...
In DC/OS Open, and as described in [Install and Customize](../install-and-customize), a certificate authority is automatically generated when installing the package.
...
In DC/OS Enterprise, and as described in [Install and Customize](../install-and-customize), the centralized [DC/OS CA](/1.12/security/ent/tls-ssl/) is used in order to provide TLS certificates for the Kubernetes cluster.
...
In this screen you should choose the "_Kubeconfig_" option, click the "_Choose kubeconfig file_" text box and pick the location of your kubeconfig file (typically, `$HOME/.kube/config`).
