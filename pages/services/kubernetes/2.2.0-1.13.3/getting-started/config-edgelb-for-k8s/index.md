...
- Configure Edge-LB on your cluster and deploy the configured pool for Kubernetes including:
    1. setting up a service account for Edge-LB
    1. installing Edge-LB, including the CLI plugin

## Set up and Install Edge-LB with service account

1. Add the Edge-LB repositories, replacing the download links with the [latest available binaries](https://support.mesosphere.com/s/downloads), this example links to DC/OS Edge-LB 1.2.3:
...
1. Create an Edge-LB service account:
...
1. Create an options JSON file to install Edge-LB with its service account. Here we create `edge-lb-options.json` and add the following configuration snippet:
...
1. Install the `edgelb` package on your DC/OS Enterprise cluster. From your CLI, enter:
...
1. Save the following Kubernetes/Edge-LB Service configuration as `edgelb.json`. Copy for your convenience:
    ...
    ```
    Paste in the file using your text editor.

1. Deploy the Edge-LB configured pool for your Kubernetes service. In your CLI, enter:
...
1. List your Edge-LB configured pools.
    ...
    and verify that your output looks similar to the following:
...
Now you have configured Edge-LB to connect to your Kubernetes clusters. In the last section, you will [test connections and view the Kubernetes Dashboard](/services/kubernetes/2.2.0-1.13.3/getting-started/test-connect/) via a web proxy on your browser.
