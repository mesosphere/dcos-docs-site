...
- Depending on your [security mode](/1.10/security/ent/#security-modes) in Enterprise DC/OS, you may [need to provision a service account](/service-docs/kafka/kafka-auth/) before installing Kafka. Only someone with `superuser` permission can create the service account.
...
**Note:** Alternatively, you can [install Kafka from the DC/OS web interface](/1.10/deploying-services/install/). If you install Kafka from the web interface, you must install the Kafka DC/OS CLI subcommands separately. From the DC/OS CLI, enter:
