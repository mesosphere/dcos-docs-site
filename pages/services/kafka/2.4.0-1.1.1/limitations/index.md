...
The `disk` configuration value is denominated in MB. We recommend you set the configuration value `log_retention_bytes` to a value smaller than the indicated `disk` configuration. See the [Configuration](/services/kafka/2.4.0-1.1.1/configuration/) section for instructions on customizing these values.
