...
- Packaging
    ...
    DC/OS Minio is packaged for deployment on DC/OS. DC/OS packages follow the [Universe schema](https://github.com/mesosphere/universe), which defines how packages expose customization options at initial installation. When a package is installed on the cluster, the packaging service (named ‘Cosmos’) creates a Marathon app that contains a rendered version of the marathon.json.mustache template provided by the package. For DC/OS Minio, this Marathon app is the scheduler for the service
...
Internally, DC/OS Minio Service treats “Deployment” as moving from one state to another state. By this definition, “Deployment” applies to many scenarios:

 - When Minio is first installed, deployment moves from a null configuration to a deployed configuration.
...
1. Run the command `dcos package install minio` in the DC/OS CLI or click Install for a given package on the DC/OS Dashboard.
...
3. Cosmos creates a Marathon app definition by rendering Minio's `marathon.json.mustache` file with the configuration options provided in the request, which represents Minio’s Scheduler. Cosmos queries Marathon to create the app.

4. Marathon launches the Minio’s scheduler somewhere in the cluster using the rendered app definition provided by Cosmos.

5. Minio’s scheduler is launched. From this point onwards, the SDK handles deployment.
...
4. Now that the Scheduler has registered as a Mesos Framework, it is able to start interacting with Mesos and receiving offers. When this begins, the scheduler will begin running the Offer Cycle and deploying `minio`. See that section for more information.
...
This is the flow for uninstalling `minio`.
...
**Steps:**
 ...
 2. Give the user permissions for the folder where you will install your service. In this example, we are creating a user called "developer", who will have access to the `/testing` folder. Select the group or user you created. Select ADD PERMISSION and then toggle to INSERT PERMISSION STRING. Add each of the following permissions to your user or group, and then click ADD PERMISSIONS.
    ...
    ```    
Install a service (in this example, `minio`) into a folder called `test`. Go to Catalog, then search for `beta-minio`.

Click CONFIGURE and change the service name to `/testing/minio`, then deploy.

The slashes in your service name are interpreted as folders. You are deploying `minio` in the `/testing` folder. Any user with access to the `/testing` folder will have access to the service.
