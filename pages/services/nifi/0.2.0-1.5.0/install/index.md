...
<p class="message--note"><strong>NOTE: </strong><br>1) The service folder location cannot be changed after initial install. Changing the service location would require installing a new instance of the service against the new location, then copying over any data as necessary to the new instance.<br>2) <code>cn dn node identity</code> parameter specify <code>initial node identity</code> for TLS which will be used inside authorizers.xml file for CN value. </p>
