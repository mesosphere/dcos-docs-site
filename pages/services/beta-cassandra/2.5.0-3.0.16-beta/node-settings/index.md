...
`MOUNT` volumes created by the DC/OS Storage Service are created for a specific volume profile. This profile needs to be referenced for this service to find these volumes.

To configure the volume profile:
*    **In DC/OS CLI options.json**: `disk_profile`: string
*    **DC/OS web interface**: `CASSANDRA_DISK_PROFILE`: `string`
