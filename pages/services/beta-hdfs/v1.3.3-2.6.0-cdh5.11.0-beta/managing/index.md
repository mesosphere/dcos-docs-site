...
Some changes, such as decreasing the number of nodes or changing volume requirements, are not supported after initial deployment. See [Limitations](https://docs.mesosphere.com/service-docs/confluent-kafka/2.0.1-3.3.0e/limitations/).
...
```bash
$ dcos beta-hdfs --name=hdfs describe > options.json
```
...
```bash
$ dcos beta-hdfs --name=hdfs update start --options=options.json
```
