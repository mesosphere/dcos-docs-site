...
1. If you installed X-Pack, go to
  ...
  ```
  and log in with `elastic`/`changeme`. [More information on installing X-Pack](/services/beta-elastic/2.1.2-5.6.5-beta/elastic-x-pack/).
...
- The Kibana X-Pack plugin is not installed by default, but you can enable it. See the [X-Pack documentation](/services/beta-elastic/2.1.2-5.6.5-beta/elastic-x-pack/) to learn more about X-Pack in the Elastic package. This setting must match the corresponding setting in the Elastic package (i.e., if you have X-Pack enabled in Kibana, you must also have it enabled in Elastic). 
