...
Install HDFS from the DC/OS web interface. Find the package in Universe and select **Install**.

Alternatively, install HDFS from the DC/OS CLI:
...
If you installed HDFS from the GUI, you must install the CLI separately.
...
```
ANote that once the service is deployed on the overlay network, it cannot be updated to use the host network.
