...
1. 运行作业。
 在 `jar` url 之前包含所有配置标记，在 `jar` url 之后包含 {{ model.techShortName }} 作业的自变量。通常遵循模板 `dcos spark run --submit-args=" <flags> URL [args]` where `<flags>` can be things like `--conf spark.cores.max=16` and `--class my.aprk.app`, `URL` is the location of the application, and `[args]` 是应用程序的任何自变量。
 ...
 运行作业成功。提交 ID：driver-20160126183319-0001
...
[使用密钥存储库](/services/spark/2.3.1-2.2.1-2/security/#using-the-secret-store) 和
[使用 Mesos 密钥](/services/spark/2.3.1-2.2.1-2/security/#using-mesos-secrets) 部分。
...
**注意：** 如果您在作业完成之前将其杀死，框架将继续作为 Mesos 中的 `inactive` 框架存留，存续时间等于故障切换超时时间。在超时之前，您可以通过点击
[Mesos 拆毁端点][18] 手动将框架拆毁。
