...
本主题介绍如何为 Kafka 配置 DC/OS 访问权限。根据您的 [安全模式](/latest/security/ent/#security-modes/)，Kafka 需要 [服务认证](/latest/security/ent/service-auth/) 才能访问 DC/OS。
...
**提示：** 您可以使用 [DC/OS 密钥存储库](/latest/security/ent/secrets/) 保护密钥对。
 ...
 **重要：** 这些命令使用 `kafka-role` 的默认 Kafka `role` 值 。如果您正在运行 Kafka 的多个实例，则用以下正确的名称替换 `kafka-role` 的实例：<name>(`-role`). For example, if you have a Kafka instance named `kafka2`, you would replace each role value in the code samples to `kafka2-role`。
