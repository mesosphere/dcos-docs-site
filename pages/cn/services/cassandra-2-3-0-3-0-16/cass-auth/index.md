...
本主题介绍如何为 Cassandra 配置 DC/OS 访问。根据您的 [安全模式](/1.9/security/ent/#security-modes/)，Cassandra 需要 [服务认证](/1.10/security/ent/service-auth/) 才能访问 DC/OS。
 ...
 **重要：** 这些命令使用 `cassandra-role` 的默认 Cassandra `role` 值。如果您正在运行 Cassandra 的多个实例，则用正确的名称 (` 替换 `cassandra-role` 的实例。<name>-role`). For example, if you have a Cassandra instance named `cassandra2`, you would replace each role value in the code samples to `cassandra2-role`。
