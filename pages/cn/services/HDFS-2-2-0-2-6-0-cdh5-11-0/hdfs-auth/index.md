...
本主题介绍如何为 HDFS 配置 DC/OS 访问。根据您的 [安全模式](/1.9/security/ent/#security-modes/)，HDFS 需要 [服务身份认证](/1.10/security/ent/service-auth/) 以访问 DC/OS。
 ...
 **重要信息：** 这些命令使用 `hdfs-role` 的默认 HDFS `role` 值。如果您正在运行多个 HDFS 实例，使用正确的名称替代 `hdfs-role` 实例 (`<name>-role`). For example, if you have a HDFS instance named `hdfs2`, you would replace each role value in the code samples to `hdfs2-role`。
