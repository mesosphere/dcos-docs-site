...
DNS 由 DC/OS 中的两个不同组件 `mesos-dns` 和 `dcos-dns` 提供。这些组件支持两个顶级域 (TLD) 名，即 `.mesos` 和 `.directory`。请阅读 [建议](#Recommendation) 部分和 [Mesos-DNS](/1.12/networking/DNS/mesos-dns/)，更好地了解这两个 TLD 的使用。
...
有关 Mesos DNS SRV 记录的完整描述，请参阅 [SRV 记录](/1.12/networking/DNS/mesos-dns/service-naming/#srv-records)。

- 对于由名为 `myservice` 的服务启动的名为 `mytask` 的任务，Mesos-DNS 生成一个 SRV 记录 `_mytask._protocol.myservice.mesos`，其中 `protocol` 为 `udp` 或 `tcp`。

- 有关 Mesos-DNS 中命名任务和服务的更多信息，请参阅 [任务和服务命名约定](/1.12/networking/DNS/mesos-dns/service-naming/#task-and-service-naming-conventions)。
