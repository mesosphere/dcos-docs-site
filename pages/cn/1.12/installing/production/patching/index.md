...
- 您的群集必须 [修补到 DC/OS 1.11](#current-security) 并在 [禁用安全模式] (1.11/install/production/advanced-configuration/configuration-reference/#security-enterprise) 下运行，才能修补为宽容模式。如果您的群集在补丁到 DC/OS 1.10，之前处于宽容模式，则可以跳过该程序。
...
1. 验证补丁：
 ...
 - 监控 Mesos UI 以验证修补的节点是否重新加入 DC/OS 群集以及任务是否已协调 (`http://<master-ip>/mesos`). If you are patching from permissive to strict mode, this URL will be `https://<master-ip>/mesos`。
