...
2) 将以下示例代码复制并粘贴到新文件中，并将其保存为本地文件夹中的 `main.tf`。
 ...
 您可以 [在此](/1.12/installing/evaluation/gcp/advanced-gcp/) 查找其他输入变量及其描述。

3) 如果需要，更改到您刚才创建 `dcos-tf-gcp-demo` 文件所在的 `main.tf` 文件夹。
...
5) 运行该执行计划并将其保存到静态文件中。例如，将输出保存在 `plan.out` 文件中。
...
有关使用更多高级配置选项在 GCP 上创建群集的更多信息，请参阅 [高级 GCP 群集配置选项](/1.12/installing/evaluation/gcp/advanced-gcp/)。
...
在官方 [DC/OS 升级](/1.12/installing/production/upgrading/) 文档中了解有关 Terraform 执行的升级程序的更多信息。
