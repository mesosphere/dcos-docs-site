...
- [已安装 DC/OS CLI](/1.12/cli/install/) 并以超级用户身份登录。
- [已安装 DC/OS Enterprise CLI 0.4.14 或更高版本](/1.12/cli/enterprise-cli/#ent-cli-install)。
- 必须 [获取根证书](/1.12/security/ent/tls-ssl/get-cert/) 才能在本部分发出 `curl` 命令。
...
使用 [DC/OS 密钥存储库](/1.12/security/ent/secrets/) 保护钥匙对。
...
1. [SSH 到您的节点](/1.12/administering-clusters/sshcluster/)。
...
如需更多信息，请参阅 [权限参考](/1.12/security/ent/perms-reference/)。

## 分配权限
使用 [权限参考](/1.12/security/ent/perms-reference/) 和日志输出，为您的服务分配权限。所有 CLI 命令也可通过 [IAM API] 执行(/1.12/security/ent/iam-api/)。
...
生成 [服务登录令牌] (/1.12/security/ent/service-auth/)，其中指定了服务帐户（`<service-account-id>`）和私钥（`<private-key>.pem`）。
...
服务成功登录后，将创建一个[认证令牌](/1.12/security/ent/service-auth/)。认证令牌应在后续对 DC/OS 端点的请求中使用。您可以将认证令牌引用为 shell 变量，例如：
