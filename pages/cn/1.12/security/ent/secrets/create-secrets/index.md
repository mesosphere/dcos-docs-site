...
有关如何在应用程序或 pod 定义中引用这些密钥的信息，请参阅[配置服务和 pod 以使用密钥](/cn/1.12/security/ent/secrets/use-secrets/)。
...
- 有关从 CLI 或 API 创建密钥所需的权限，请参阅[密钥存储库权限](/cn/1.12/security/ent/perms-reference/#secrets)。您配置的权限必须包括允许用户创建的密钥名称。每个密钥必须拥有一项权限。密钥名称和权限名称必须匹配。
