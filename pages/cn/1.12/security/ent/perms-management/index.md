...
可以使用 DC/OS Web 界面、[IAM HTTP API](/cn/1.12/security/ent/iam-api/) 或 [DC/OS Enterprise CLI](/cn/1.12/cli/enterprise-cli/) 对用户和组应用权限。每个接口提供管理访问控制条目 (ACE) 的方式。每个 ACE 包括以下信息：
...
有 [四个命令](/cn/1.12/cli/command-reference/dcos-security/#dcos-security-org) 用于管理 DC/OS Enterprise CLI 的权限。
...
[IAM HTTP API](/cn/1.12/security/ent/iam-api/) 提供管理用户和组权限的操作。
