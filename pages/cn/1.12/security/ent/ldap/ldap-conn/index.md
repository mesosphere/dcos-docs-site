...
1. 单击 **Add Directory**。
    ...
    ![添加目录对话框](/mesosphere/dcos/1.12/img/ldap-add-dir-conn.png)
...
1. 从 **Select SSL/TLS setting** 列表框中选择首选加密选项。
    ...
    *选中**对所有连接使用 SSL/TLS ** 复选框以使用[安全 LDAP（LDAPS）](http://social.technet.microsoft.com/wiki/contents/articles/2980.ldap-over-ssl-ldaps-certificate.aspx)。
    ...
    * 如果**尝试 StartTLS，如果未能**尝试通过 [StartTLS](https://tools.ietf.org/html/rfc2830) 将连接升级到 TLS，则继续不加密，如果升级到 TLS 失败，则继续不加密连接。
    ...
    <p class="message--note"><strong>注意：</strong>我们建议<strong>对所有连接使用 SSL/TLS</strong>，或 <strong>尝试 StartTLS，如果无法</strong>确保 SSL/TLS 或 StartTLS 加密，则中止；否则密码将以明文形式发送。</p>
...
1. 指定身份认证方法和参数，如[身份认证部分](/mesosphere/dcos/cn/1.12/security/ent/ldap/ldap-auth/)中所述。
