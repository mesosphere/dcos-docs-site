...
DC/OS Enterprise 支持 [通过 LDAP 进行基于目录的身份认证](/1.12/security/ent/ldap/)。用户和用户组可以从外部目录导入 DC/OS IAM。查看[管理用户和组](/1.12/security/ent/users-groups/)。
...
安装时可配置确切的时长。请参阅 [高级配置] (/1.12/installing/production/advanced-configuration/)下的 [配置参考] (/1.12/installing/production/advanced-configuration/configuration-reference/)。
