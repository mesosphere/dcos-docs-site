...
本教程演示如何在宽容[安全模式](/1.12/security/ent/#security-modes)下实现 DC/OS 服务的用户权限。完成后，您将通过使用 DC/OS 权限获得多租户。

**前提条件：**

- DC/OS Enterprise [已安装](/1.12/installing/)在宽容[模式](/1.12/security/ent/#security-modes)下。
...
1. 从 **Services > Services > Create Group** 创建服务组。
 ...
 ![服务创建组](/1.12/img/GUI-Services-No_Services_Running-1_12.png)

 图 1. 创建组页面

 在此示例中，创建了一个名为 `prod-a` 和一个名为 `prod-b`。创建组之后，您应该看到两个文件夹。您可以在此处为用户组部署服务并为每个单元设置权限。

 ![组文件夹](/1.12/img/GUI-Services-List_View_Groups_Empty-1_12.png)
...
1. 创建用户和组并定义每个组所需的权限。
 ...
 ![创建用户 Cory](/1.12/img/GUI-Organization-Users-Create_User_Modal-1_12.png)

 图 3. 创建新用户

 完成后，您应该看到两个用户。

 ![所有用户](/1.12/img/GUI-Organization-Users-Users_List_View_w_Users-1_12.png)
 ...
 ![prod-a group](/1.12/img/GUI-Organization-Groups-Create_New_Group_Modal-1_12.png)
...
1. 为 prod-a-group 添加这些权限，然后单击 **Close**。
 ...
 ![prod-a-group](/1.12/img/GUI-Organization-Groups-Add_Permission_String_w_Perms-1_12.png)

 图 6. 为“prod-a-group”添加权限

 以下是添加后的权限视图：

 ![prod-a-group](/1.12/img/GUI-Organization-Groups-Group_Detail_w_Permissions-1_12.png)
...
1. 选择 **Group Membership**，然后在搜索框中键入 `prod-a-group`，然后单击选择。
 ...
 ![prod-a-group](/1.12/img/GUI-Organization-Groups-User_Add_Group_Membership-1_12.png)
...
1. 以 Cory 身份登录到 DC/OS Web 界面。您可以看到，用户 Cory 只能访问 **Services** 和 **Universe** 选项卡。同时，Cory 只能看到 **prod-a** 服务。
 ...
 ![prod-a-group](/1.12/img/GUI-Services-Limited_User_Access_List-1_12.png)
...
2. 选择 **Services > Services**，然后单击加号 (**+**) 来部署服务。
 ...
 ![JSON 视图](/1.12/img/GUI-Services-Add_Service_With_JSON-1_12.png)
...
3. 针对 Nick 重复之前的步骤。确保指定 `"id": "/prod-b/nginx",`，例如：
...
4. 在 Cory 或 Nick 登录时，单击 NGINX 启动图标，查看确认消息。

 ![NGINX](/1.12/img/GUI-Services-Add_Service_Review_Install-1_12.png)
...
1. 注销当前用户，然后以具有[超级用户](/1.12/security/ent/perms-reference/#superuser)权限的用户身份返回。您将看到，两个服务都在 prod-a 和 prod-b 组中运行。

 ![prod-a-group](/1.12/img/GUI-Services-Services_List_w_Groups-1_12.png)
