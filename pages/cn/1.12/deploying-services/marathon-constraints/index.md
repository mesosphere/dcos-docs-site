...
使用 `@region` 和 `@zone` 字段名称配置 [故障域感知和容量扩展](/1.11/deploying-services/fault-domain-awareness/)。
...
字段名称为属性时，则支持所有 Marathon 算子。Marathon 支持文本、标量、范围和数集属性值。为确定标量、范围和数集，Marathon 将对格式值执行字符串比较。格式与 Mesos 属性格式匹配。范围和数集的格式分别为 `[begin-end,...]` 和 `{item,...}` 。例如，您可能有一个格式为 `[100-200]` 的范围和格式为 `{a,b,c}` 的数集。Regex 适用于 LIKE 和 UNLIKE 算子；如需匹配 ANY 值，请使用字符串 `.*`。
