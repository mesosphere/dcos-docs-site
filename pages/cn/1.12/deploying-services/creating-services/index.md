...
1. 使用 [DC/OS CLI](/1.12/cli/) 将服务添加到 DC/OS。
...
<p class="message--note"><strong>注意：</strong>默认情况下，抓取器不会使下载的文件可执行。在上述示例中， <code>cmd</code> 首先使文件可执行文件。</p>
...
1. 选择是否使用 Universal Container Runtime (UCR) 或 Docker Engine 运行时间。请参阅[使用容器化工具](/1.12/deploying-services/containerizers/)。
...
1. 使用 [Marathon API](/1.12/deploying-services/marathon-api/) 部署应用程序 `basic-3-docker`。请参阅 [验证 HTTP API 端点](/1.12/security/ent/iam-api/) 以了解有关如下命令中所需的 API 令牌的更多信息。
...
1. 确定[公共节点的 IP 地址](/1.12/administering-clusters/locate-public-agent/)。
