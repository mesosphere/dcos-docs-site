...
1. 使用 [自定义高级安装说明] 安装 DC/OS (/1.12/installing/oss/custom/advanced/)。以下是 GPU 特定的配置参数：
 ...
 有关更多信息，请参阅 [配置参数文档] (/1.12/installing/oss/custom/configuration/configuration-parameters/#enable-gpu-isolation) 和 Mesos [Nvidia GPU 支持文档] （http://mesos.apache.org/documentation/latest/gpu-support/#external-dependencies）。

## 带有 GPU 的 AWS EC2 DC/OS 安装

## 先决条件
- AWS DC/OS 高级模板 [系统要求](/1.12/installing/production/system-requirements/)。
- 复制到本地机器的 `zen.sh` 脚本。脚本和说明在 [此处](/1.12/installing/oss/cloud/aws/advanced/)。
...
1. 请根据 [此处](/1.12/installing/oss/cloud/aws/advanced/) 说明，使用以下 GPU 专用配置创建具有高级 AWS 模板的群集。
...
- NVIDIA GPU 支持仅适用于使用 [DC/OS 通用容器运行时间] 启动的任务(/1.12/deploying-services/containerizers/)。
...
1. 使用 DC/OS  CLI 启动应用程序：
 ...
 服务部署完成后，检查 `stdout` 内容，验证该服务是否采用 `nvidia-smi` 命令产生正确的输出。您会看到如下内容，并且每隔 5 秒重复一次。[通过 DC/OS  CLI](/1.12/monitoring/logging/quickstart/) 或在 DC/OS 仪表板上的服务 **健康** 页面访问日志。
...
## 基于 Docker 的应用定义
在本示例中部署了一个具有 GPU 的应用程序，用于指定 Docker 容器和 [DC/OS 通用容器运行时间 (UCR)](/1.12/deploying-services/containerizers/) （容器类型为 `MESOS`）。
