...
* [Redis](/1.12/tutorials/dcos-101/redis-package/) 已部署并在您的群集中运行。
 ...
 * 通过检查日志：
    ...
    dcos task log app1
    ```
 在这里，您应该看到 `app1` 在哪个节点和端口上运行以及显示 Redis 中密匙数的应用程序输出。不同运行之间甚至在应用程序生命周期内，节点和端口可能会有所不同，具体取决于群集中的事件。
...
在 [下一部分](/1.12/tutorials/dcos-101/service-discovery/)中，您将通过探索 DC/OS 中应用程序的不同选项，了解 DC/OS 服务发现。