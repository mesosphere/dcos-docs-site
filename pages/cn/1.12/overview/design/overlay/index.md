...
从 [不同主机上的容器到容器通信] 的数据包步骤中明显可知(/1.12/overview/design/overlay/#container-to-container-different-hosts) ，为了让 DC/OS 覆盖网络运行，有几个元数据需要预先配置到代理中，以便进行路由和切换以正常工作。在这里，我们将列出 DC/OS 覆盖网络所需的信息。
...
只有解决这些挑战，才能让 DC/OS 覆盖网络运行。我们将在下一节介绍 DC/OS 覆盖网络控制平面的软件架构。控制平面将对 [挑战] 一节列出的元数据(/1.12/overview/design/overlay/#challenges) 进行配置和编程，以使其正常运行。
