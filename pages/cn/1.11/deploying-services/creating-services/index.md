...
1. 使用 [DC/OS CLI](/1.11/cli) 将服务添加到 DC/OS。
...
**注意：** 默认情况下，抓取器不会使下载的文件可执行。在上述示例中， `cmd` 首先使文件可执行文件。
...
1. 选择是否使用 Universal Container Runtime (UCR) 或 Docker Engine 运行时间。请参阅[使用容器化工具](/1.11/deploying-services/containerizers/)。
...
1. 使用 [Marathon API](/1.11/deploying-services/marathon-api/) 部署应用程序 `basic-3-docker`。请参阅 [验证 HTTP API 端点](/1.11/security/ent/iam-api/) 以了解有关如下命令中所需的 API 令牌的更多信息。
...
1. 向下滚动到 **Marathon Task Configuration** 部分，并记录 PORTS 属性。
 ...
 ![容器端口](/1.11/img/container-port.png)

 图 1. 容器端口
   
1. 确定[公共节点的 IP 地址](/1.11/administering-clusters/locate-public-agent/)。
1. 导航至 <public-node-IP>:<port><public-node-IP>:<port>以查看 Docker 容器根目录的内容。
