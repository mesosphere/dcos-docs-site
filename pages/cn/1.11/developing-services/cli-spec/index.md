...
[DC/OS 命令行界面 (CLI)](/mesosphere/dcos/cn/1.11/cli/) 是管理集群节点、安装和管理包、检查集群状态，以及管理服务和任务的实用程序。DC/OS CLI 开放且可扩展：任何人都可以创建新子命令，可供最终用户安装。例如，[Spark DC/OS 服务][2] 提供了可用 Spark 的 CLI 扩展。安装后，您可以键入以下命令以提交 Spark 作业并查询其状态：
```
