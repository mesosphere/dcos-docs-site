...
有关参数描述和配置示例，请参阅 [文档](/cn/1.11/installing/production/advanced-configuration/configuration-reference/)。
...
- 在 **选择模板** 页面，从您的工作站上传 [Zen](/cn/1.11/installing/evaluation/community-supported-methods/aws/advanced/template-reference/#zen) 模板，然后单击 **下一步**。
...
2. 在 **指定详情**页面指定这些值，然后单击 **下一步**。
    ...
    * **堆栈名称** 指定群集名称。
    * **CustomAMI** 可选：指定 AMI ID。如需更多信息，请参阅 [使用自定义 AMI 进行安装](/cn/1.11/installing/production/system-requirements/custom-ami/)。
...
可以通过创建新的堆栈来添加更多代理节点。使用 [advanced-priv-agent.json](/cn/1.11/installing/evaluation/community-supported-methods/aws/advanced/template-reference/#private-agents) 或 [advanced-pub-agent.json](/cn/1.11/installing/evaluation/community-supported-methods/aws/advanced/template-reference/#public-agents) 模板。这些模板创建代理，然后作为 AutoScalingGroup 的一部分附加到 `PrivateAgentStack` 或 `PublicAgentStack`。
