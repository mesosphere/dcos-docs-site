...
1. 将此脚本添加到 bootstrap 节点的 `genconf` 文件夹。[更多信息](/mesosphere/dcos/cn/1.11/installing/production/deploying-dcos/installation/#create-a-fault-domain-detection-script)。

1. [安装 DC/OS ](/mesosphere/dcos/cn/1.11/installing/production/deploying-dcos/installation/#create-a-fault-domain-detection-script)。
...
在 Marathon 服务或 pod 定义中，可以使用 [布局约束](/mesosphere/dcos/cn/1.11/deploying-services/marathon-constraints/)，以便：
...
要增加容量，请 [添加新代理](/mesosphere/dcos/cn/1.11/administering-clusters/add-a-node/) 到集群的远程分域或分区，然后更新服务以在相应的一个或多个分域启动实例。
