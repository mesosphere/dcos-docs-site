...
DC/OS 由 Apache Mesos 提供支持，用作一组计算机的分布式内核，您可以将其视为一个单元，但您仍保持对每个单元的控制。在 DC/OS 中，系统的内核是任意数量公开可用和专用的 Mesos 管理节点和代理节点；故障的 Mesos 管理节点由备用管理节点透明地替代，并掌管主节点选举。管理节点处理有故障的代理节点和进程。
...
4. 实时度量“firehouse”开放供您喜爱的诊断和分析包使用。
