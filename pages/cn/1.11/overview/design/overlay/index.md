...
从 [不同主机上的容器到容器通信] 的数据包步骤中明显可知(/cn/1.11/overview/design/overlay/#container-to-container-different-hosts) ，为了让 DC/OS 覆盖网络运行，有几个元数据需要预先配置到代理中，以便进行路由和切换正常工作。在这里，我们将列出 DC/OS 覆盖网络所需的信息。
