...
1. 在 **Permissions Strings** 字段中复制并粘贴权限。根据您的[安全模式]选择权限字符串(/1.11/security/ent/#security-modes)。
 ...
 指定作业组 (`<job-group>`), 作业名 (`<job-name>`), 和操作 (`<action>`)。操作可以是 `create`, `read`, `update`, `delete`, or `full`。如需允许多于一个操作，使用逗号隔开他们，例如: `dcos:service:metronome:metronome:jobs:<job-group>/<job-name> read,update`。
