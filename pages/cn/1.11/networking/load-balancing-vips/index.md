...
您可以通过在应用定义中分配 [VIP](/mesosphere/dcos/cn/1.11/networking/load-balancing-vips/virtual-ip-addresses/) 来使用第 4 层负载均衡器。通过VIP创建一个任务或一组任务后，它们将自动可用于集群中的所有节点，包括管理节点。
...
- 必须使用支持的 [操作系统](/mesosphere/dcos/cn/1.11/installing/production/system-requirements/)。
...
#### 运行状况检查
使用 Mesos 进行运行状况检查。Mesos 运行状况检查显示在负载均衡层。Marathon 仅将**命令** [运行状况检查](/mesosphere/dcos/cn/1.11/deploying-services/creating-services/health-checks/) 转换为 Mesos 运行状况检查。您可以通过以下类似命令来模拟 HTTP 运行状况检查：
...
- [为应用程序分配 VIP](/mesosphere/dcos/cn/1.11/networking/load-balancing-vips/virtual-ip-addresses/)
