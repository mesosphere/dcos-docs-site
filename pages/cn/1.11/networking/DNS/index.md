...
每个 TLD 均由多个区域组成。每个 DC/OS 服务从这些不同区域获取多个 FQDN 条目。通过 Marathon 在 DC/OS 上启动的每个服务都以下列形式获得 FQDN，即 `<service-name>.mesos`。 而且, **所有** 在 DC/OS 上启动的正运行的服务会根据启动它的服务获得 FQDN ，格式为 `<service-name>.<group-name>.<framework-name>.mesos`。
